...
A business can sell digital or downloadable products. The products are stored privately using a storage service, such as S3, or other storage mechanism. When the customer buys this type of product, typically an email is sent to them where they can download the product.

Medusa doesn’t support digital products by default. However, it provides the customization capabilities necessary to implement it based on your use case, restriction free. You can create new entities that allow you to represent a digital product and link them to existing product-related entities.
...
export class ProductMedia extends BaseEntity {
  ...
  @ManyToOne(
    () => ProductVariant,
    (variant) => variant.product_medias
  )
...
To use this relation on the `ProductVariant` entity as well, extend the entity by creating the file `src/models/product-variant.ts` with the following content:

```ts title=src/models/product-variant.ts
...
If you’re using TypeScript, it’s recommended to create the file `src/index.d.ts` with the following content:
...
This ensures that your IDE’s autocomplete takes into consideration the new `product_medias` relation on the `ProductVariant` entity.

Next, you need to create a repository for the `ProductMedia` entity. To do that, create the file `src/repositories/product-media.ts` with the following content:

```ts title=src/repositories/product-media.ts
import { ProductMedia } from "../models/product-media"
import { 
  dataSource,
} from "@medusajs/medusa/dist/loaders/database"

export const ProductMediaRepository = dataSource
  .getRepository(ProductMedia)

export default ProductMediaRepository
```

Finally, you need to create a migration script that reflects the changes on the database schema.
...
export class ProductMediaCreate1693901604934 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      await queryRunner.query(`ALTER TABLE "product_media" ADD CONSTRAINT "FK_b38718bc6a3891beb6e620be706" FOREIGN KEY ("variant_id") REFERENCES "product_variant"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`)
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
      await queryRunner.query(`ALTER TABLE "product_media" DROP CONSTRAINT "FK_b38718bc6a3891beb6e620be706"`)
...
class ProductMediaService extends TransactionBaseService {
  ...
  async retrieve(
    ...
    const productMediaRepo = this.activeManager_
      .withRepository(
        this.productMediaRepository_
      )
