...
You can retrieve a single order edit by its ID by sending a request to the [Get Order Edit](https://docs.medusajs.com/api/store/#tag/OrderEdit/operation/GetOrderEditsOrderEdit) endpoint:
...
You can learn more about what fields to expect in the [API reference](https://docs.medusajs.com/api/store/#tag/OrderEdit/operation/GetOrderEditsOrderEdit).
...
1. Show the customer the available payment processors. These can be retrieved from the details of [the region of the order](https://docs.medusajs.com/api/store/#tag/Region/operation/GetRegionsRegion).
2. When the customer selects the payment processor, initialize the payment session of that provider in the payment collection. You can do that by sending a request to the [Manage Payment Sessions](https://docs.medusajs.com/api/store/#tag/Payment/operation/PostPaymentCollectionsSessions) endpoint, passing it the payment collection’s ID as a path parameter, and the payment processor's ID as a request body parameter:
...
2. Authorize the payment using the payment processor. The [Authorize Payment Session](https://docs.medusajs.com/api/store/#tag/Payment/operation/PostPaymentCollectionsSessionsSessionAuthorize) endpoint accepts the payment collection’s ID and the ID of the payment session as path parameters:
...
To confirm and complete the order edit, send a request to the [Complete Order Edit](https://docs.medusajs.com/api/store/#tag/OrderEdit/operation/PostOrderEditsOrderEditComplete) endpoint:
