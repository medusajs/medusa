...
function applyCollapsedStyle(el: HTMLElement, collapsed: boolean) {
  ...
  el.style.transform = `translateY(${collapsedStyles.top})`;
  el.style.opacity = collapsedStyles.opacity;
}
...
function useCollapseAnimation({
  ...
  useEffect(() => {
    ...
    function getTransitionStyles() {
      ...
      return {
        transition: collapsed ? 
          `height ${duration * 2}ms ${easing}, transform ${duration}ms ${easing}, opacity ${duration / 2}ms ${easing}` : 
          `height ${duration}ms ${easing}, transform ${duration * 2}ms ${easing}`,
        height: `${height}px`,
        transform: `translateY(${ExpandedStyles.top})`,
        opacity: ExpandedStyles.opacity
      };
    }
    ...
    function applyTransitionStyles() {
      ...
      el.style.transform = transitionStyles.transform
    }
