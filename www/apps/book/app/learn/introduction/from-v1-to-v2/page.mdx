...
While Medusa v1 supported Node.js v16+, Medusa v2 requires Node.js v20.19.0+ or v22.12.0+. So, make sure to update your Node.js version if it's older.
...
The Medusa Admin is now available out-of-the-box. It's built with [Vite v7](https://vite.dev/) and runs at `http://localhost:9000/app` by default when you start your Medusa application.
...
In Medusa v2, `bodyParser: false` still disables the body parser the same way. If you only need access to the raw request body instead of disabling parsing entirely, set `bodyParser.preserveRawBody` to `true` in the route's middleware configuration instead. For example:
