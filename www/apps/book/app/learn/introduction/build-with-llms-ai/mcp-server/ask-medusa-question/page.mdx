...
Every AI model already has opinions about Medusa from its training data, and for a framework that ships frequently, those opinions age badly. The problem isn't that an agent doesn't know Medusa; it's that it's confident about APIs, configuration keys, and method signatures that changed, were renamed, or belonged to Medusa v1. You find out when the code doesn't run, or worse, when it runs but does the wrong thing.
