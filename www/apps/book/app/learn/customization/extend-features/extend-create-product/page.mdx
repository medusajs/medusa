...
The [createProductsWorkflow](!resources!/references/medusa-workflows/createProductsWorkflow) used in the [Create Product API route](!api!/admin/products/create-product) has a `productsCreated` hook that runs after the product is created. You'll consume this hook to link the created product with the brand specified in the request parameters.
...
export const hook2Highlights = [
  ...
  ["6", "links", "Define an array to store the links in."],
  ["9", "push", "Add a link to be created."],
  ["19", "create", "Create the links."]
]
