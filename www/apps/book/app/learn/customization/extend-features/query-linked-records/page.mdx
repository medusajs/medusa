...
Medusa's existing API routes accept a `fields` query parameter that allows you to specify the fields and relations of a model to retrieve. So, when you send a request to the [List Products](!api!/admin/products/list-products), [Get Product](!api!/admin/products/get-a-product), or any product-related admin routes that accept a `fields` query parameter, you can specify in this parameter to return the product's brands.

<Note>

Learn more about using the `fields` query parameter to retrieve custom linked data models in the [Retrieve Custom Linked Data Models from Medusa's API Routes](../../../fundamentals/api-routes/retrieve-custom-links/page.mdx) chapter. For store routes, you'll need to override the allowed fields and relations to retrieve your custom linked data models.
...
Instead, you'll have to create a custom API route that uses [Query](../../../fundamentals/query/page.mdx#apply-filters-and-pagination-on-linked-records) to retrieve linked records with filters.

---

## Approach 2: Use Query to Retrieve Linked Records

You can also retrieve linked records using [Query](../../../fundamentals/query/page.mdx). Query allows you to retrieve data across modules with filters, pagination, and more. You can resolve Query from the Medusa container and use it in your API route or workflow.
...
export const highlights = [
  ["14", "resolve", "Resolve Query from the Medusa container."],
  ["18", "graph", "Run a query to retrieve brands with their linked products."],
  ["19", "entity", "The name of the model to query."],
  ["20", "fields", "The fields and relations to retrieve."]
]
...
For an alternative approach, refer to the [Query documentation](../../../fundamentals/query/page.mdx#apply-filters-and-pagination-on-linked-records).
