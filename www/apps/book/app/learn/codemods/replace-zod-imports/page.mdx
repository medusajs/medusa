...
Starting from [Medusa v2.14.0](https://github.com/medusajs/medusa/releases/tag/v2.14.0), Zod has been upgraded to v4, which introduces breaking changes to the Zod API. You may need to update your validation schemas accordingly. See the [Zod v4 upgrade guide](https://zod.dev/v4/changelog) for details.
...
Medusa v2.13.0+ exports Zod from `@medusajs/framework/zod`, and Medusa v2.14.0+ uses Zod v4 internally. Because Medusa manages Zod as part of its own dependencies, you should avoid manually pinning a `zod` version in your backend's `package.json`.

<Note title="Warning" type="warning">

Do not manually pin `zod` in your backend's `package.json`. Pinning a specific Zod version can conflict with the version Medusa manages internally, especially if other packages in the dependency tree expect Zod v4.

</Note>

If you encounter Zod version conflicts during installation or deployment, follow these steps:

1. Remove any `zod` entry from the `dependencies` or `devDependencies` in your backend's `package.json`.
2. Delete `node_modules` and your lock file:
...
This lets the package manager resolve a single consistent version of Zod that satisfies all packages in the dependency tree.

If you need Zod for Medusa Admin customizations (frontend code), you can install it separately in your admin package only — this keeps it isolated from the backend dependency graph.
