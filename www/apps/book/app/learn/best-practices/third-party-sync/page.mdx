...
A common performance pitfall when syncing data is retrieving more fields than necessary from third-party services or Medusa's [Query](../../fundamentals/query/page.mdx). This leads to increased data size, slower performance, and higher memory usage.
...
export const retryHighlights = [
  ...
  ["4", "RETRY_DELAY_MS", "Base delay for retries"],
  ["6", "fetchWithRetry", "Function to fetch with retry logic"],
  ["15", "fetch", "Fetch data from API"],
  ["27", "isRetryable", "Check if error is retryable"],
  ["36", "delay", "Calculate exponential backoff delay"],
]
...
export const timeoutHighlights = [
  ["3", "FETCH_TIMEOUT_MS", "Timeout duration for fetch requests"],
  ["13", "AbortController", "Create an `AbortController` for request timeout"],
  ["14", "setTimeout", "Set a timeout to abort the request"],
  ["24", "clearTimeout", "Clear the timeout on successful response"],
]
