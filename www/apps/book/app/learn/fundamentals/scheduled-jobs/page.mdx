...
export const highlights = [
  ["4", "greetingJob", "The scheduled job function to execute at a specified interval."],
  ["5", "container", "Receive the Medusa container as a parameter"],
  ["6", "context", "Receive context about the scheduled execution"],
  ["8", "logger", "Resolve the logger from the container"],
  ["13", "config", "The scheduled job's configurations."],
  ["14", "name", "The job's unique name"],
  ["15", "schedule", "The schedule to run the job on."]
]

```ts title="src/jobs/hello-world.ts" highlights={highlights}
import { MedusaContainer } from "@medusajs/framework/types"
import { ScheduledJobContext } from "@medusajs/framework/jobs"
...
You export an asynchronous function that receives the [Medusa container](../medusa-container/page.mdx) as the first parameter, and a `ScheduledJobContext` object as the second parameter. In the function, you resolve the [Logger utility](../../debugging-and-testing/logging/page.mdx) from the Medusa container and log a message.

The `ScheduledJobContext` object has the following property:

<Note>

The `context` parameter is available since Medusa [v2.16.1](https://github.com/medusajs/medusa/releases/tag/v2.16.1).

</Note>

- `scheduledFor`: A `Date` object indicating the timestamp for which the job was scheduled to run.
