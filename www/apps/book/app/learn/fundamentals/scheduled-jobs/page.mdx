...
Do not disable a scheduled job by commenting out or removing the `config` export while keeping the file in `src/jobs/`. The ESLint plugin enforces that all files in `src/jobs/` have a valid `config` export with `name` and `schedule` properties, and will report an error if the export is missing or incomplete.
