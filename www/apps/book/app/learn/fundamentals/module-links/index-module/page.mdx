...
Before [Medusa v2.21.0], you had to enable the `MEDUSA_FF_INDEX_ENGINE` flag to use the Index Module in the `/store/products` and `/admin/products` API routes. As of v2.21.0, the Index Module is no longer used and cross-module filters are applied through `query.graph` instead.
...
export const basicHighlights = [
  ["22", "id", "Filter products by their linked sales channel ID."]
]
...
So, whenever you retrieve carts or orders, either with the `index` method or with [Query's `graph` method](../query/page.mdx#select-specific-fields), request total properties only when you display them:
...
export const paginationHighlights = [
  ["19", "metadata", "Pagination details."],
  ["28", "pagination", "Apply pagination to the retrieved records."]
]
...
```ts title="src/api/custom/products/route.ts" highlights={[["22"]]}
import {
  MedusaRequest,
  MedusaResponse,
} from "@medusajs/framework/http"
...
export const workflowHighlights = [
  ["14", "query", "Resolve query in a workflow step."],
  ["18", "index", "Use the `index` method to retrieve data."],
]
