...
When the `MEDUSA_FF_INDEX_ENGINE` feature flag is enabled, the `/store/products` and `/admin/products` API routes use `query.index` for most filters instead of `query.graph`. Filters by `tags`, `categories`, or `options` always fall back to `query.graph` regardless of the flag.
