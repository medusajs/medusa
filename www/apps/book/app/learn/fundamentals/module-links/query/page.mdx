...
2. A Query configuration object. It accepts the following properties:
    ...
    5. `disallowed`: An array of strings or regular expressions. A string entry matches any field path segment of the same name. A regular expression is tested against each segment and also against the full dotted path, which lets you block a relation at a specific position (for example, `/\.orders(?:\.|$)/` blocks `orders` everywhere except at the root). Any matched field is removed before the query executes, making this a hard security boundary. Learn more in the [Disallowed Fields in API Routes](!docs!/learn/fundamentals/api-routes/disallowed-fields) chapter.
