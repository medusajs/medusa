...
You can filter by fields of linked data models from other modules when both modules share the same database. The linked field must be a non-computed DML column. For example:
...
In the example above, you filter variants by the `currency_code` field of their linked `price_set`. Medusa pushes the filter down as a SQL subquery, so only variants matching the condition are returned.
...
You can sort by fields of linked data models from other modules when both modules share the same database and the field is a non-computed DML column. For example:
