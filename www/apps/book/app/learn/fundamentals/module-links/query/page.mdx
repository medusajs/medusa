...
In the example above, each level in the `service_zone.fulfillment_set.location.*` path is a relation or link that Query resolves in order. You can also select specific properties at any level, such as `service_zone.fulfillment_set.location.id`, instead of using `.*`.
...
You can filter by a linked data model's property at any level of the chain. Refer to the [Apply Filters](#apply-filters) section for details.
...
This is available from [Medusa v2.17.3](https://github.com/medusajs/medusa/releases/tag/v2.17.3).

You can filter by fields of linked data models from other modules. For example:
...
In the example above, you filter variants by the `currency_code` field of their linked `price_set`. When possible, Medusa pushes the filter down as a SQL subquery. Otherwise, Medusa loads the related data and applies the filter in memory before returning the result.
...
You can sort by fields of linked data models from other modules. For example:
...
When possible, Medusa pushes the sort to SQL. Otherwise, Medusa loads the data and sorts in memory before returning the result.
...
2. A Query configuration object. It accepts the following properties:
    ...
    5. `disallowed`: An array of field path segments that are always stripped from the query, regardless of what the client requests. Any requested field whose path contains one of these segments is removed before the query executes. Unlike `allowed`, this is a hard security boundary that middleware cannot override. Use it to keep sensitive relations off unauthenticated endpoints.
