...
You can chain relations and links across multiple levels by separating each relation's name with a dot (`.`) in the `fields` array. A chain can traverse relations within the same module and links between different modules.

<Note title="Same-module relation depth">

Chaining relations that are defined within the same module is currently limited to one level deep. For example, if model `A` has a relation to model `B`, and `B` has a relation to model `C` (all in the same module), querying `a.b.c` will not return `b` or `c`. To work around this, run two separate queries: first retrieve `A` with `b.id`, then query `B` with `c.id` using the IDs from the first result.

</Note>

For example, the Fulfillment Module's `ShippingOption` belongs to a `ServiceZone`, which belongs to a `FulfillmentSet`, which [links to a stock location](!resources!/commerce-modules/fulfillment/links-to-other-modules#stock-location-module) in the Stock Location Module. To retrieve a shipping option along with its service zone, fulfillment set, and the fulfillment set's stock location in a single query:
...
export const queryLinkTableHighlights = [
  ...
  ["11", `"product.id"`, "Retrieve the ID of a product record linked to a `Post` record."],
  ["13", `"post.id"`, "Retrieve the ID of a `Post` record linked to a product record."]
]
...
- You pass in the `fields` property:
    - The link table's columns, such as `id`, `product_id`, and `post_id`. You can also select [custom columns](../custom-columns/page.mdx) that you defined in the link table.
...
<Note title="Sorting in API routes">
...
2. A Query configuration object. It accepts the following properties:
    ...
    5. `disallowed`: An array of strings or regular expressions. A string entry matches any field path segment of the same name. A regular expression is tested against each segment and also against the full dotted path, which lets you block a relation at a specific position (for example, `/\.orders(?:\.|$)/` blocks `orders` everywhere except at the root). Any matched field is always removed before the query executes, making this a hard security boundary that middleware cannot override.
