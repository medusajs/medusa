...
- `alias`: A custom alias to use when querying the linked records. By default, Medusa derives the alias from the data model's name. Available since Medusa [v2.17.2](https://github.com/medusajs/medusa/releases/tag/v2.17.2).
...
When you define a link using the object form, Medusa derives the query alias for the linked data model from the data model's name. You can override this alias by passing the `alias` property.

<Note>

This option is available since Medusa [v2.17.2](https://github.com/medusajs/medusa/releases/tag/v2.17.2).

</Note>

This is useful when you link the same module twice in different link definitions and need to differentiate the two links when querying.

For example:

export const aliasHighlights = [
  ["9", "alias", "Override the alias used when querying linked records."],
]

```ts highlights={aliasHighlights}
import BlogModule from "../modules/blog"
import ProductModule from "@medusajs/medusa/product"
import { defineLink } from "@medusajs/framework/utils"
...
In this example, you override the alias of the `post` data model to `featured_post`. When you query linked records, use `featured_post` as the relation name:

```ts
const { result } = await query.graph({
  entity: "product",
  fields: ["id", "featured_post.*"],
})
```
