...
- `alias`: A custom alias to use when querying the linked records. By default, Medusa derives the alias from the data model's name.
...
This option is available since Medusa [v2.16.1](https://github.com/medusajs/medusa/releases/tag/v2.16.1).
...
export const aliasHighlights = [
  ["10", "alias", "Override the alias used when querying linked records."],
]
...
In this example, you override the alias of the `post` data model to `featured_post`. When you query linked records, you use `featured_post` as the relation name:
