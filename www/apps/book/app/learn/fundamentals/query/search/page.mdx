...
For searching the `product` index, Medusa provides a [Search Products API route](!api!/store/products/search-products) that uses `query.search` under the hood. You can also use `query.search` in your custom API routes, such as to search other data models.
