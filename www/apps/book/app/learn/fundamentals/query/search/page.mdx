...
A search query runs against the [Search Module](!resources!/infrastructure-modules/search) to retrieve and rank documents based on relevance. For example, searching products with full-text search, filtering by price, and sorting by popularity.

Under the hood, the Search Module retrieves the results from the integrated [Search Module Provider](!resources!/infrastructure-modules/search/providers), such as [Medusa Search](!cloud!/search) or your custom third-party provider.

For searching from a storefront, Medusa provides a [Store Search API route](!api!/store/search/search-indexes) that uses `query.search` under the hood. You can also use `query.search` in your custom API routes, such as to search data models that the route doesn't expose.
...
```ts title="src/api/store/products/custom-search/route.ts" highlights={exampleHighlights} collapsibleLines="1-7" expandButtonLabel="Show Imports"
import {
  MedusaRequest,
  MedusaResponse,
...
<TypeList types={[
  {
    name: "$eq",
    type: "`any`",
    description: "Equals a literal value."
  },
  {
    name: "$ne",
    type: "`any`",
    description: "Doesn't equal a literal value."
  },
  {
    name: "$in",
    type: "`any[]`",
    description: "Is one of the given values."
  },
  {
    name: "$nin",
    type: "`any[]`",
    description: "Isn't one of the given values."
  },
  {
    name: "$lt",
    type: "`number` \| `Date`",
    description: "The number or date is less than the given value."
  },
  {
    name: "$lte",
    type: "`number` \| `Date`",
    description: "The number or date is less than or equal to the given value."
  },
  {
    name: "$gt",
    type: "`number` \| `Date`",
    description: "The number or date is greater than the given value."
  },
  {
    name: "$gte",
    type: "`number` \| `Date`",
    description: "The number or date is greater than or equal to the given value."
  },
  {
    name: "$exists",
    type: "`boolean`",
...
<TypeList types={[
  ...
  },
  {
    name: "distinct",
    type: "`string`",
    description: "A field to deduplicate on, so the provider returns at most one hit per distinct value. The count in `metadata.count` then counts the deduplicated hits.\n\nThe field must exist in the index, and it can't be an array or vector field."
  },
  {
    name: "min_score",
    type: "`number`",
