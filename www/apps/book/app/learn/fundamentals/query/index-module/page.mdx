...
The Index Module ingests the `amount`, `currency_code`, and `price_list_id` properties of the `Price` data model. So, you can sort products by their variants' price amounts with the `order` property of the `pagination` object.

<Note title="Calculated prices can't be sorted">

A variant's `calculated_price` isn't stored in the database. The [Pricing Module calculates it](!resources!/commerce-modules/pricing/price-calculation) when you retrieve the variant, based on the pricing context, price lists, and price rules. So, you can't sort products by their calculated price, either with the `index` method, Query's `graph` method, or the `order` query parameter of the `/store/products` API route.

The `/store/products` API route also doesn't accept `variants.prices.amount` in its `order` query parameter. To sort products by price in your storefront, create a custom API route that uses the `index` method, as shown below.

</Note>

For example, to retrieve products sorted by their USD prices from lowest to highest:

export const priceSortHighlights = [
  ["23", "currency_code", "Only consider USD prices."],
  ["31", "amount", "Sort by the variants' price amounts."]
]

```ts title="src/api/store/custom/products/route.ts" highlights={priceSortHighlights}
import {
  MedusaRequest,
  MedusaResponse,
} from "@medusajs/framework/http"
...
A product can have multiple variants, and each variant can have multiple prices. So, when you sort by `variants.prices.amount`:

- `ASC` sorts products by their lowest price amount.
- `DESC` sorts products by their highest price amount.

The sort only considers the prices that match your filters. In the example above, the `currency_code` filter limits the sort to USD prices, and products without a USD price aren't returned.

Keep in mind that the sort uses the price amounts stored in the database. It doesn't apply price rules or taxes, and it includes the prices of price lists, such as sale prices. If you need the sort order to match the calculated prices shown to customers, store the price you want to sort by in a search engine, as explained in the [Product Index Examples](!resources!/infrastructure-modules/search/product-index-examples) guide.
