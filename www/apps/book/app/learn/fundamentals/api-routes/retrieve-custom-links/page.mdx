...
In this example, you apply a middleware to the [List Products Store API Route](!api!/store/products/list-products).

The request object passed to middlewares has an `allowed` property that contains the fields and relations that can be retrieved. So, you modify the `allowed` array to include the `brand` field.

Push the plain field path, without a `*`, `+`, or `.*` prefix or suffix. To also retrieve a field nested under the link, such as `brand.name`, push that full path too.
