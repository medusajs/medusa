...
Medusa provides two middlewares to validate the request body and query parameters of incoming requests to your custom API routes:
...
`validateAndTransformBody` enforces strict schema validation. Fields present in the request body that are not defined in your Zod schema are rejected with an `unrecognized_keys` error. If you apply this middleware to a route that also has its own validation, make sure both schemas accept the same fields, or the request will always fail.
