...
RBAC is available as an experimental feature under the `rbac` feature flag. Learn how to enable it in the [Feature Flags guide](../../../debugging-and-testing/feature-flags/page.mdx).

</Note>

Beyond authentication, you can implement Role-Based Access Control (RBAC) to control what actions authenticated users can perform. RBAC uses policies and roles to define permissions.

### Apply Policies to API Routes

You can protect API routes using policies in your middleware configuration:
...
The `policies` array specifies the required permissions:

- **resource**: The resource type (e.g., `product`, `order`, `custom_resource`)
- **operation**: The operation type (e.g., `PolicyOperation.read`, `PolicyOperation.create`, `PolicyOperation.update`, `PolicyOperation.delete`)

### Check Permissions in Route Handlers

You can also manually check permissions within your API route handlers:
...
### Get User's Assignable Resources

For admin interfaces where users can assign roles or policies to others, use the assignable workflows to determine what they can assign:

```ts title="src/api/admin/assignable-roles/route.ts"
...
Learn more about RBAC in the [User Module RBAC guide](!resources!/commerce-modules/user/rbac).
