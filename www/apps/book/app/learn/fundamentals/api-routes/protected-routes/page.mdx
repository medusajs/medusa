...
3. An optional object of configurations accepting the following properties:
    - `allowUnauthenticated`: (default: `false`) A boolean indicating whether authentication is required. For example, you may have an API route where you want to access the logged-in customer if available, but guest customers can still access it too. Learn more about how the middleware handles invalid credentials in the [next section](#invalid-credentials-in-unauthenticated-routes).
...
### Invalid Credentials in Unauthenticated Routes

When you set `allowUnauthenticated` to `true`, the `authenticate` middleware never rejects a request. It verifies the credentials of the request, if any, then passes the request to the next handler in both of the following cases:

- The request carries no credentials at all.
- The request carries credentials that fail verification, such as an expired or a tampered JWT token.

In both cases, the middleware doesn't set the `auth_context` property of the request object. This is the intended behavior: the middleware's only job is to attach an authenticated actor when it can verify one, and the route decides what to do when it can't.

<Note title="Warning" type="warning">

Because both cases look the same to your route handler, don't treat a missing `auth_context` as proof that the request presented no credentials. If your route's logic differs between a signed-out request and an expired session, add the check explained next.

</Note>

#### Reject Invalid Credentials

To answer with a `401` error when a request presents credentials that fail verification, while still allowing requests that present no credentials, add a middleware that runs after the `authenticate` middleware.

Create the file `src/api/middlewares/reject-invalid-auth.ts` with the following content:

export const rejectInvalidAuthHighlights = [
  ["15", "authorization", "Check whether the request sent a bearer token."],
  ["19", "session", "Check whether the request sent a session cookie holding an authentication context."],
  ["21", "auth_context", "The `authenticate` middleware didn't verify the presented credentials."],
]

```ts title="src/api/middlewares/reject-invalid-auth.ts" highlights={rejectInvalidAuthHighlights}
import {
  AuthenticatedMedusaRequest,
  MedusaNextFunction,
  MedusaResponse,
...
export async function rejectInvalidAuth(
  req: AuthenticatedMedusaRequest,
  res: MedusaResponse,
  ...
  const hasToken = authHeader
    .toLowerCase()
    .startsWith("bearer ")
  const hasSession = Boolean(req.session?.auth_context)

  if ((hasToken || hasSession) && !req.auth_context) {
    return next(
      new MedusaError(
...
The middleware considers a request as presenting credentials if it has an `Authorization` header with a bearer token, or a session cookie holding an authentication context. If the request presents credentials but the `authenticate` middleware didn't set `auth_context`, the middleware rejects the request with a `401` error.

Then, apply the middleware after the `authenticate` middleware:

```ts title="src/api/middlewares.ts" highlights={[["17", "rejectInvalidAuth", "Run the check after authentication."]]}
import {
  defineMiddlewares,
  authenticate,
} from "@medusajs/framework/http"
import {
  rejectInvalidAuth,
} from "./middlewares/reject-invalid-auth"

export default defineMiddlewares({
  routes: [
    {
      matcher: "/custom/public*",
      middlewares: [
...
The order of the middlewares matters, since `rejectInvalidAuth` reads the `auth_context` that the `authenticate` middleware sets.

Now, a request with no credentials reaches the route as a guest, a request with valid credentials reaches the route with its `auth_context` set, and a request with expired or invalid credentials receives a `401` error. A storefront can then prompt the customer to log in again instead of showing them the guest experience.
...
You can access the logged-in customer’s ID in all API routes starting with `/store` using the `auth_context.actor_id` property of the `AuthenticatedMedusaRequest` object. You can then use [Query](../../query/page.mdx) to retrieve the customer details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.

For example:

export const customerHighlights = [
  ["13", "customerId", "Retrieve the logged-in customer's ID."],
  ["18", "query", "Retrieve the customer details."],
  ["25", "throwIfKeyNotFound", "Throw an error if the customer with\nthe specified ID is not found."],
]
...
You can access the logged-in admin user’s ID in all API routes starting with `/admin` using the `auth_context.actor_id` property of the `AuthenticatedMedusaRequest` object. You can then use [Query](../../query/page.mdx) to retrieve the user details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.

For example:

export const adminHighlights = [
  ["13", "userId", "Retrieve the logged-in admin user's ID."],
  ["18", "query", "Retrieve the user details."],
  ["25", "throwIfKeyNotFound", "Throw an error if the user with\nthe specified ID is not found."],
]
