...
- `matcher`: a string indicating the API route path to apply the middleware on. The string must be compatible with [path-to-regexp](https://github.com/pillarjs/path-to-regexp).
...
2. Pattern matchers. For example, `/custom/(products|collections)`.
