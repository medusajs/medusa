...
export const loaderHighlights = [
  ["9", "ModuleOptions", "Define a type for expected options."],
  ["17", "ModuleOptions", "Pass the option type as a type argument to `LoaderOptions`."],
  ["35", "clientDb", "Create a client instance that connects to the specified database."],
  ["41", "register", "Register custom resource in the container."],
  ["42", `"mongoClient"`, "The resource's key in the container."],
  ["43", "asValue(clientDb)", "The resource to register."]
]
...
export default async function mongoConnectionLoader({
  ...
  if (!options.connection_url) {
    throw new MedusaError(
      ...
      `[MONGO MDOULE]: connection_url option is required.`
    )
  }
  if (!options.db_name) {
    throw new MedusaError(
      MedusaError.Types.INVALID_ARGUMENT,
      `[MONGO MDOULE]: db_name option is required.`
    )
  }
  const logger = container.resolve(
    ContainerRegistrationKeys.LOGGER
