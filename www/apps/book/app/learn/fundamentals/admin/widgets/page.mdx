...
The widget component must be created as an arrow function. The [Admin ESLint Plugin](../eslint/page.mdx) enforces this and other widget development patterns automatically.
...
Plugins can register custom injection zones to extend the widget system beyond the core zones. This is useful when creating custom admin pages or extending existing pages with plugin-specific zones.

### Plugin-Defined Injection Zones

To register custom injection zones in a plugin, augment the `InjectionZoneRegistry` interface through declaration merging. Create an `index.d.ts` file in your plugin root with the following structure:

```ts title="my-plugin/index.d.ts"
declare module "@medusajs/admin-shared" {
  interface InjectionZoneRegistry {
    ...
    "my-plugin.custom-section.side.before": true
    "my-plugin.custom-section.side.after": true
  }
}
```

Make sure to include the declaration file in your plugin's package.json:

```json title="my-plugin/package.json"
{
  "files": [
    ".medusa/server",
    "index.d.ts"
...
### Using Custom Injection Zones

Once registered, custom injection zones can be used in widget configurations just like core zones:

```tsx title="src/admin/widgets/my-custom-widget.tsx"
import { defineWidgetConfig } from "@medusajs/admin-sdk"
...
export const config = defineWidgetConfig({
  zone: "my-plugin.product-page.before",
})
...
Custom injection zones should follow the pattern: `{plugin-name}.{page-context}.{position}`, where:

- `plugin-name`: Your plugin's identifier
- `page-context`: The page or section context
...
Custom injection zones are validated using a pattern check. If a zone doesn't match core injection zones, the system will show a warning but still allow the zone if it follows the expected pattern.
