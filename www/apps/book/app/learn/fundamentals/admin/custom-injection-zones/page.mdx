...
When you build a custom admin page in a plugin, use the [LayoutComposer](!resources!/admin-components/layouts/layout-composer) component from `@medusajs/dashboard/components` to structure the page's layout. This component also exposes injection zones based on the prefix you pass to its `widgetsZonePrefix` prop.
...
Refer to the [LayoutComposer](!resources!/admin-components/layouts/layout-composer) reference for more details on the component's props and layouts.
