...
<Note title="React Router v7">

Starting from [Medusa v2.19.0](https://github.com/medusajs/medusa/releases/tag/v2.19.0), the Medusa Admin uses React Router v7. If your project has its own `react-router-dom` dependency, update it to v7 to avoid installing two versions of React Router, which leads to a `useNavigate() may be used only in the context of a Router` error in your admin customizations.

React Router v7 also removed the `json` and `defer` utilities. Instead of `json`, return a `Response` object with a JSON body from your loader. Instead of `defer`, return a plain object holding the unresolved promises.
...
1. Define and export a [React Router loader](https://reactrouter.com/start/data/route-object#loader) function in the UI route's file. In this function, you can fetch and return data asynchronously.
2. In your UI route's component, you can use the [useLoaderData hook from React Router](https://reactrouter.com/api/hooks/useLoaderData) to access the data returned by the `loader` function.
...
In your UI route or any route file, you can export a `handle` object to define [route handles](https://reactrouter.com/start/data/route-object#handle). The object is passed to the loader and route contexts.
...
Refer to [React Router’s documentation](https://reactrouter.com) for components and hooks that you can use in your admin customizations.
