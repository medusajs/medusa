...
The `loaderData` prop replaces the `data` prop starting from [Medusa v2.19.0](https://github.com/medusajs/medusa/releases/tag/v2.19.0), which upgraded the Medusa Admin to React Router v7. In earlier versions, use the `data` prop instead.
...
This feature is available starting [Medusa v2.17.2](https://github.com/medusajs/medusa/releases/tag/v2.17.2).
...
The `loaderData` property replaces the `data` property starting from [Medusa v2.19.0](https://github.com/medusajs/medusa/releases/tag/v2.19.0), which upgraded the Medusa Admin to React Router v7. In earlier versions, use the `data` property instead.

</Note>

When you open the UI route at `http://localhost:9000/app/brands/123`, the browser tab's title will show the brand's name, such as `Acme - Medusa`.

<Note title="Tip">

If the `seo` function returns an empty `title` or doesn't return one, the dashboard falls back to the route's [sidebar label](#show-ui-route-in-the-sidebar), then its [breadcrumb](#set-ui-route-breadcrumbs), and finally the default `Medusa` title.
