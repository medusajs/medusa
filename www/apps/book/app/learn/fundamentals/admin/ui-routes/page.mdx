...
To set the page title of a UI route, export a `handle` object with a `seo` property in the UI route's file. It can return a string, or it can accept a `match` object whose `data` property holds the data returned by the `loader` function. This is useful for UI routes that accept path parameters, allowing you to set the page title dynamically.
...
export const seoHighlights = [
  ...
  ["37", "match", "The match object holds the data returned by the loader in its `data` property."],
]
...
Then, in the `handle.seo` function, you receive the `match` object whose `data` property holds the data returned by the `loader` function. You use this data to return the dynamic title.
