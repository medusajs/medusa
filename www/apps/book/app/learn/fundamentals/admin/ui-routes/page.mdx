...
Document title resolution from the route's `config` is available starting [Medusa v2.17.3](https://github.com/medusajs/medusa/releases/tag/v2.17.3).

</Note>

The Medusa Admin sets the browser tab title based on the current route. For custom UI routes, the title is resolved in the following order:

1. An explicit `seo` resolver exported from a `handle` object (for detail pages that surface an entity name).
2. The `label` from the route's `config` export (set via `defineRouteConfig`).
3. A `breadcrumb` function from a `handle` object that returns a plain string.

When you export a `config` with a `label`, that label is automatically used as the document title for the route. No extra code is required.

For example, the following UI route displays `Custom Route - Medusa` in the browser tab:
...
If you need a dynamic title (for example, showing an entity's name on a detail page), export a `handle` with an `seo` function:

export const seoHighlights = [
  ["18", "seo", "Return an object with a `title` string to set the document title."],
  ["19", "data", "The loader data is available on the match object."],
]
...
export const handle = {
  seo: (match: UIMatch<{ brand: { name: string } }>) => ({
    title: match.data?.brand?.name,
  }),
}
```
