...
**Environment Variables in Admin Customizations**: When developing admin customizations within a Medusa project, use `process.env.BACKEND_URL` instead of `import.meta.env.VITE_BACKEND_URL`. The Medusa admin builder automatically injects `process.env.BACKEND_URL` for admin customizations, while `VITE_BACKEND_URL` is used for standalone projects. In development, `process.env.BACKEND_URL` is typically set to `"/"` for relative requests.
