...
- Commerce Modules compute some totals, such as an order's `total`, and wrap them in a `BigNumber` instance. For example, if you retrieve an order's total with [Query](../../query/page.mdx), the `total` property is a `BigNumber` instance:
