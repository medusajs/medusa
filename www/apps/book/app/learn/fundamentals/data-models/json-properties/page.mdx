...
A JSON property in a data model stores any JSON-serializable value in a `jsonb` column.
...
By default, the TypeScript type of a `json` property is `Record<string, unknown>`. You can pass a type argument to use a different shape, including arrays:
...
To store an array of strings, use the [array property type](../properties/page.mdx#array) instead, which stores the value in a `text[]` column.

</Note>

### Accepted Values in JSON Property

JSON properties support any JSON-serializable value, including objects, arrays, strings, numbers, and booleans. When the property stores an object, its keys are strings and values can be one of the following types:
...
Only an empty string removes a property. If you pass `null` as the value of a property, Medusa stores `null` as the property's value instead of removing it.

</Note>

### Manage Metadata in Commerce Modules and Workflows

The merge behavior explained in this chapter also applies to the `metadata` property of data models in Medusa's Commerce Modules. This includes:

- Module service methods, such as the Product Module's `updateProducts`, the Order Module's `updateOrders` and `updateOrderLineItems`, and the Cart Module's `updateCarts` and `updateLineItems`.
- Workflows and steps that use these methods, such as `updateProductsWorkflow`, `updateOrderWorkflow`, `updateCartWorkflow`, and `updateCartsStep`.
- All API routes that are built on top of these workflows.

So, when you pass `metadata` to these API routes, methods, workflows, or steps:

- Medusa merges the top-level properties you pass with the existing `metadata`. Properties you don't pass remain unchanged.
- Medusa replaces nested objects entirely, as explained in the [Caveat: Updating Nested Objects](#caveat-updating-nested-objects) section. To change a single nested property, retrieve the record first, then pass the entire nested object with your changes.
- Medusa removes properties whose value is an empty string.

For example, to add a property to a product's `metadata` and remove another one using the `updateProductsWorkflow`:
...
After the update, the product's `metadata` has the new `erp_id` property, doesn't have the `old_tax_code` property, and keeps all other existing properties.
