...
This property type is used for product and product variant volumetric attributes like `weight`, `height`, `width`, and `length`.
...
export const relationshipSearchableHighlights = [
  ["4", "searchable", "Make the `variants` relationship searchable."]
]
