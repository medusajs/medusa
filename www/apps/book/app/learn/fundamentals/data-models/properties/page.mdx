...
Medusa stores along the `bigNumber` property a `raw_{property_name}` property that contains its raw value. Learn more in the [Big Number documentation](../big-numbers/page.mdx).
...
Enum values must be valid GraphQL names. Each value must match the pattern `[_A-Za-z][_0-9A-Za-z]*`: start with a letter or underscore, and contain only letters, digits, and underscores. Values must not be `true`, `false`, or `null`. For example, `"black"` and `"_301"` are valid, but `"301"`, `"a-b"`, and `"true"` are not. If you use invalid values, `medusa db:generate` fails with a GraphQL syntax error. Use `model.number()` and validate allowed values in your application logic if you need numeric-like identifiers.
...
export const relationshipSearchableHighlights = [
  ["6", "searchable", "Make the `variants` relationship searchable."]
]

```ts highlights={relationshipSearchableHighlights}
import { model } from "@medusajs/framework/utils"
