...
When you write a migration manually for a data model defined with [model.define](../page.mdx), make sure the created table includes the `created_at`, `updated_at`, and `deleted_at` columns. Medusa adds these [default properties](../properties/page.mdx#data-models-default-properties) to every data model, and the module's service relies on them in its queries. For example, the service filters out soft-deleted records using the `deleted_at` column, so queries fail at runtime if the column doesn't exist.
