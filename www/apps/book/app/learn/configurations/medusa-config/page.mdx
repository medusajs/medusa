...
<TypeList
  types={[
    ...
    },
    {
      ...
      description: `A function that returns a password (or a promise resolving to a password) called for each new database connection. Use this for short-lived credentials, such as AWS RDS IAM authentication tokens, that must be refreshed per connection. Available since v2.17.0.`
    },
    {
      name: "expirationChecker",
      type: "`() => boolean`",
      description: `A function that returns \`true\` when a connection's credentials have expired. When this returns \`true\`, the connection pool creates a new connection and calls \`dynamicPassword\` to obtain fresh credentials. Available since v2.17.0.`
    },
    {
