...
- [projectConfig](#project-configurations-projectConfig): Essential configurations related to the Medusa application, such as database, CORS, and cloud configurations.
...
The `projectConfig.cloud` configuration specifies Medusa Cloud-specific configurations for services like OAuth, emails, and payments when your application integrates with Medusa Cloud.

This configuration is an object that accepts the following properties:

#### cloud.environmentHandle

The `projectConfig.cloud.environmentHandle` configuration specifies the environment handle of the Medusa Cloud environment.
...
The `projectConfig.cloud.oauthJwksUri` configuration specifies the JWKS URI for the Medusa Cloud OAuth service. This is used to verify the JWT tokens issued by the Medusa Cloud OAuth service.
...
The `projectConfig.cloud.oauthAudience` configuration specifies the expected `aud` claim of id_tokens issued by the Medusa Cloud OAuth service.
...
Learn more about Medusa Cloud OAuth configuration in the [Cloud documentation](!cloud!/user).
