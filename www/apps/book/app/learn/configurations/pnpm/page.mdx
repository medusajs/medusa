...
`pnpm` can be used in monorepo setups with additional configuration. If you're setting up a monorepo, refer to the [Docker installation guide](../../installation/docker/page.mdx) for an example of a working pnpm monorepo configuration with proper workspace setup.

For simpler single-package projects, you can follow the steps in this guide. For monorepos, we recommend using the configuration shown in the Docker guide or consider using `yarn` or `npm` if you prefer a more straightforward setup.
