...
- The value of `COOKIE_SECRET` and `JWT_SECRET` must be a randomly generated secret. These are required in production. If not set, an error is thrown when starting the Medusa application.
