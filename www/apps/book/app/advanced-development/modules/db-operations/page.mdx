...
2. A public method that calls the transactional method. You use on it the `InjectManager` decorator imported from `@medusajs/utils` that injects the method with a [forked entity manager](https://mikro-orm.io/docs/identity-map#forking-entity-manager).
...
The function receives a transactional entity manager as a parameter. Use it to perform the database operations.
...
1. `transaction`: Set the transactional entity manager passed to the function. This is useful if your method must be a nested transaction:
