...
export const highlights = [
  ["6", "links", "Special `links` property that holds the `HelloModule`'s data models."],
  ["7", "links", "Special `links` property that holds the `ProductModule`'s data models."],
]
...
export default defineLink(
  ...
  HelloModule.links.product
)
```

The `defineLink` function accepts two parameters, which are the the data models to form the link between them.

You retrieve those data models by importing the module and accessing the special `links` property, which is an object of the module's data models.
