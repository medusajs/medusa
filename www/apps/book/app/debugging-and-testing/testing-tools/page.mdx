...
} else if (process.env.TEST_TYPE === "modules") {
...
"scripts": {
  ...
  "test:modules": "npm run build && NODE_OPTIONS=--experimental-vm-modules TEST_TYPE=modules jest --runInBand --forceExit"
},
```
...
- `test:modules` to run integration tests for modules available in any `__tests__` directory under `src/modules`.
