...
This defines a `createBrandStep`. In the step, you resolve the Brand Module's main service and use its `createBrands` method, which accepts one or more brands to create.
...
The compensation function resolves the Brand Module's main service and uses its `deleteBrands` method to delete the brand created by the step. The `deleteBrands` method accepts an ID or an array of IDs of brands to delete.
