...
import { readSpecFromBinding } from "./read-spec-from-binding"

export default async function readSpecDocument(filePath: string) {
  const fileContent =
    (await readSpecFromBinding(filePath)) ??
    (await workerCompatibleFetch<string>({
      url: filePath,
      responseTransformer: async (res) => {
        if (!res.ok) {
          throw new Error(`Failed to fetch spec: ${filePath} (${res.status})`)
        }
