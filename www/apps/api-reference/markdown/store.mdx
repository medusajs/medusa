...
<H1 className={"scroll-m-[184px] lg:scroll-m-[264px] !mb-0"} id="introduction">Medusa V2 Store API Reference</H1>

<ViewAsMarkdown path="/store" className="mb-1" />
...
To obtain a JWT token, send a request to the [authentication route](https://docs.medusajs.com/api/store/auth/authenticate-customer) passing it the customer's email and password in the request body.
...
If you're not using the JS SDK, send a request to the [session authentication API route](https://docs.medusajs.com/api/store/auth/set-authentication-session). To view the cookie session ID, pass the `-v` option to the `curl` command.
...
Create a publishable API key either using the [admin REST APIs](https://docs.medusajs.com/api/admin/api-keys/create-api-key), or using the [Medusa Admin](!user-guide!/settings/developer/publishable-api-keys#create-publishable-api-key).
...
An API route can restrict the fields and relations that you can retrieve:

- Allowed fields and relations are the only ones you can retrieve in the API route. Every Store API route has an allowed list, so you can only retrieve the fields and relations that it includes. A field is permitted only if its full path is in the list, so selecting a relation doesn't grant access to the fields nested under it.
- Disallowed fields and relations are the ones you can never retrieve in the API route, even if you pass them in the `fields` query parameter.

If an API route has any of these restrictions, they're listed in this reference under the Allowed and Disallowed collapsibles. Medusa removes restricted fields from the response silently, so the request still succeeds with a `200` status code.

To learn more about these restrictions and how to override them, refer to the [Allowed Fields](!docs!/learn/fundamentals/api-routes/allowed-fields) and [Disallowed Fields](!docs!/learn/fundamentals/api-routes/disallowed-fields) documentation.
...
To select a specific field, pass a `.<field>` suffix instead of the `*` prefix. For example, `variants.manage_inventory`.
...
```js title="Select relation's fields"
sdk.store.product.list({
  fields: "variants.manage_inventory,variants.allow_backorder,options.values"
})
```
...
```bash title="Select relation's fields"
curl 'localhost:9000/store/products?fields=variants.manage_inventory,variants.allow_backorder,options.values'
```
...
This returns the variants and options of each product, but the variants only have their `id`, `manage_inventory`, and `allow_backorder` fields, and the options only have their `id` and `values` fields. The `id` is always included.

<Note>

Each Store API route accepts only the field paths listed in its allowed fields. So, the nested paths you pass must appear in that list. Learn more in the [Allowed and Disallowed Fields](#allowed-and-disallowed-fields) section.
...
You can chain more relations to select deeper levels, such as `*items.variant.product`. On Store API routes, the route's allowed fields must include the full nested path, such as `variants.prices`. Referencing the parent relation alone doesn't grant access to what's nested under it.
...
curl 'localhost:9000/store/products?fields=*variants.prices'
```
...
You can retrieve custom linked data models in the `fields` query parameter. For example, if you linked a `Brand` to a `Product`, you can pass `brand` in the `fields` query parameter to retrieve the brand of each product.

Since every Store API route restricts the fields and relations you can retrieve, you must first add the custom linked data model to the route's allowed list. To learn how to override the allowed fields and relations, refer to the [Retrieve Custom Linked Data Models from Medusa's API routes](!docs!/learn/fundamentals/api-routes/retrieve-custom-links) documentation.
...
On Store API routes, the field you sort by must also be in the route's allowed list. If it isn't, the request fails with a `400` error, such as `Order field X is not valid`. This differs from the `fields` query parameter, which removes disallowed fields silently instead of returning an error.
