...
<SourceCodeLink link="https://github.com/medusajs/medusa/blob/b41f6828baae00ff6728c7539b1184015ca4dff2/packages/core/core-flows/src/order/workflows/create-order-change-actions.ts#L25" />
...
export async function POST(
  ...
  const { result } = await createOrderChangeActionsWorkflow(req.scope)
    .run({
      input: [{
        "action": "RECEIVE_DAMAGED_RETURN_ITEM"
      }]
    })
