...
This method is used to delete options. Unlike the [delete](../product.IProductModuleService.delete/page.mdx) method, this method won't completely remove the option. It can still be accessed or retrieved using methods like [retrieve](../product.IProductModuleService.retrieve/page.mdx) if you pass the `withDeleted` property to the `config` object parameter.

The soft-deleted options can be restored using the [restore](../product.IProductModuleService.restore/page.mdx) method.
