...
The `create-medusa-app` CLI tool simplifies the process of creating a new Medusa project. It creates a monorepo containing your Medusa backend application and, optionally, a storefront. It also allows you to setup a [Medusa plugin project](#create-a-medusa-plugin-project).
...
If `pnpm dlx create-medusa-app` fails with a build approval error before the project exists, see [Build approval errors before project creation](!resources!/troubleshooting/pnpm#build-approval-errors-before-project-creation).
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        ...
        Installs the storefront in the `apps/storefront` directory within your project monorepo. If this option is not specified, the storefront directory is removed from the project.
    ...
    <Table.Row>
      ...
      <Table.Cell>
        ...
        Specify the Medusa version to install in the project. The specified version will be used for every `@medusajs/*` package in the Medusa and storefront projects.
...
│   │   ├── package.json
│   │   └── medusa-config.ts
│   └── storefront/       # Storefront (if --with-nextjs-starter is used)
│       ├── src/
│       ├── package.json
│       └── next.config.js
...
Your Medusa backend will be located in the `apps/backend` directory, and the optional storefront in `apps/storefront`.
...
To install a specific version of Medusa, use the `--version` option. This will install the specified version for every `@medusajs/*` package in the Medusa and storefront projects. For example:
