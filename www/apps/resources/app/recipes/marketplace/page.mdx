...
This recipe provides the general steps to follow to implement a marketplace in your Medusa application.
...
export const mainServiceHighlights = [
  ...
  ```ts title="src/modules/marketplace/service.ts" highlights={mainServiceHighlights}
...
export const userSubscriberHighlights = [
  ...
  ```ts title="src/subscribers/user-created.ts" highlights={userSubscriberHighlights}
  ...
  To test it out, run the `build` command and use the `medusa user` command to create a user:
...
export const productSubscriberHighlights = [
  ...
  ```ts title="src/subscribers/product-created.ts" highlights={productSubscriberHighlights}
  import { SubscriberArgs, type SubscriberConfig } from "@medusajs/medusa"
  import { ModuleRegistrationName } from "@medusajs/modules-sdk"
  import { 
...
export const productRoutesHighlights = [
  ...
  ```ts title="src/api/admin/marketplace/products/route.ts" highlights={productRoutesHighlights}
  import { 
...
While this section showcases the implementation, the `order.created` event is still not emitted in Medusa V2.

</Note>

An order may contain items from different stores. To ensure that users can only view and manage their orders, create a subscriber that listens to the `order.created` event and handles splitting the order into multiple orders based on the items’ stores.
...
export const orderSubscriberHighlights = [
  ...
  ```ts title="src/subscribers/order-created.ts" highlights={orderSubscriberHighlights}
  import type { 
    SubscriberArgs, 
    SubscriberConfig
  } from "@medusajs/medusa"
  ...
  This adds a subscriber to the `order.created` event. In the subscriber, you:
...
export const orderRoutesHighlights = [
  ...
  ```ts title="src/api/admin/marketplace/orders/route.ts" highlights={orderRoutesHighlights}
  import { 
    AuthenticatedMedusaRequest, 
    MedusaResponse
  } from "@medusajs/medusa"
...
<CardList items={[
  ...
  {
    href: "/v2/storefront-development/tips",
