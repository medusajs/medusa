...
export const retrieveProductHighlights = [
  ...
  ["23", "graph", "Retrieve the vendor's products using the Query."]
]
...
This adds a `GET` API route at `/vendors/products` that, using the Query, retrieves the list of products of the vendor and returns them in the response.
...
- [How to use the Query](!docs!/advanced-development/modules/query)
