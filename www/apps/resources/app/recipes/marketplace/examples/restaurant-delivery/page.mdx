...
Then, create the file `src/modules/restaurant/index.ts` that holds the following module definition:
...
1. Use the `/auth/{actor_type}/{provider}` route to obtain an authentication token for registration. `{actor_type}` is the custom user type, such as `driver`, and `{provider}` is the provider used for authentication, such as `emailpass`.
...
2. Set the actor type of the user’s authentication identity (created by the `/auth/{actor_type}/{provider}` API route). For this step, you’ll use the `setAuthAppMetadataStep` step imported from the `@medusajs/core-flows` package.
...
```bash
curl -X POST 'http://localhost:9000/auth/restaurant/emailpass' \
--data-raw '{
...
```bash
curl -X POST 'http://localhost:9000/auth/driver/emailpass' \
-H 'Content-Type: application/json' \
--data-raw '{
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Creates the links returned by the previous step between the order and delivery. This is imported from `@medusajs/core-flows`.",
...
In this snippet, you resolve the Workflow Engine Module’s main service. Then, you use the `subscribe` method of the service to subscribe the the `handleDeliveryWorkflow` ’s execution. You indicate the execution using the transaction ID stored in the delivery.
