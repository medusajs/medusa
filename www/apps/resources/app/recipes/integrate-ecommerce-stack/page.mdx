...
This recipe provides the general steps to follow to integrate your ecommerce stack with Medusa.
...
export const serviceHighlights = [
  ...
  ```js title="medusa-config.js" highlights={[["5", "ERP_API_KEY", "The environment variable holding the API key of the ERP system."]]}
  const modules = {
    erpModuleService: {
      resolve: "./dist/modules/erp",
      options: {
...
export const workflowHighlights = [
  ...
  ["54", "update", "Update the product in Medusa to remove the ERP product's ID."]
    ...
    ```ts title="src/workflows/create-product.ts" highlights={workflowHighlights}
    import {
      createStep,
      StepResponse,
      createWorkflow
