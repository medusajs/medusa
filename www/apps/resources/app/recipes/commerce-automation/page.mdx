...
This recipe provides the general steps to follow to implement a B2B store with Medusa.
...
export const restockModuleService = [
  ...
  ```ts title="src/modules/restock-notification/service.ts" highlights={restockModuleService}
  ...
  const modules = {
    "restockNotificationModuleService": {
      resolve: "./dist/modules/restock-notification",
      definition: {
        isQueryable: true
      }
    },
  ...
  ```ts title="src/api/store/restock-notification/route.ts"
  import type { 
    MedusaRequest, 
    MedusaResponse
...
export const subscriberHighlights = [
  ...
  ```ts title="src/subscribers/inventory-item-update.ts" highlights={subscriberHighlights}
  import {
    SubscriberArgs,
...
<CardList items={[
  ...
  },
  {
    ...
    text: "Check out available notification provider modules.",
    startIcon: <BoltSolid />,
    showLinkIcon: false
  },
  {
...
To handle events within an order flow and automate actions, create a subscriber. For example, create a subscriber that listens to the `order.created` event and automatically creates a fulfillment if predefined conditions are met.
...
1. Create a subscriber that listens to the `order.created` event.
...
<Details summaryContent="Example: Add customer to VIP group">
  ...
  Here’s an example of a subscriber that listens to the `order.created` event and checks whether the customer should be added to the VIP customer group based on their number of orders:
...
export const newsletterHighlights = [
  ...
  ```ts title="src/subscribers/send-products-newsletter.ts" highlights={newsletterHighlights}
  import type {
    SubscriberArgs,
    SubscriberConfig,
  } from "@medusajs/medusa"
