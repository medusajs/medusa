...
3. [useQueryGraphStep](/references/helper-steps/useQueryGraphStep) to retrieve the order's details. [Query](!docs!/learn/fundamentals/query) is a tool that allows you to retrieve data across modules.
...
The list subscriptions API route should allow clients to retrieve subscriptions with pagination. An API route can be configured to accept pagination fields, such as `limit` and `offset`, then use them with [Query](!docs!/learn/fundamentals/query) to paginate the retrieved data.
...
Refer to the [Request Query Configuration](!docs!/learn/fundamentals/query#request-query-configurations) documentation to learn more about this middleware and the query configurations.
...
export const createSubscriptionOrdersJob1Highlights = [
  ...
  ["19", "limit", "The number of subscriptions per page."],
  ["20", "pagesCount", "The total number of pages."],
  ["23", "beginningToday", "Get the date for today at `00:00:00`."],
  ["29", "endToday", "Get the date for today at `23:59:59`."],
  ["39", "$gte", "The `next_order_date` must be greater than or equal to `beginningToday`."],
  ["40", "$lte", "The `next_order_date` must be less than or equal to `endToday`."],
]

```ts title="src/jobs/create-subscription-orders.ts" highlights={createSubscriptionOrdersJob1Highlights} collapsibleLines="1-8" expandMoreLabel="Show Imports"
...
export const expireSubscriptionOrdersJobHighlights = [
  ["17", "page", "The current page number"],
  ["18", "limit", "The number of subscriptions per page."],
  ["19", "pagesCount", "The total number of pages."],
  ["22", "beginningToday", "Get the date for today at `00:00:00`."],
  ["28", "endToday", "Get the date for today at `23:59:59`."],
  ["38", "$gte", "The `expiration_date` must be greater than or equal to `beginningToday`."],
  ["39", "$lte", "The `expiration_date` must be less than or equal to `endToday`."],
  ["49", "expireSubscription", "Set the subscriptions as expired."]
]

```ts title="src/jobs/expire-subscription-orders.ts" highlights={expireSubscriptionOrdersJobHighlights} collapsibleLines="1-7" expandMoreLabel="Show Imports"
import { MedusaContainer } from "@medusajs/framework/types"
import SubscriptionModuleService from "../modules/subscription/service"
import { SUBSCRIPTION_MODULE } from "../modules/subscription"
import moment from "moment"
...
{/* eslint-disable @medusajs/no-service-mutations-in-api-route */}
