...
const ProductMediaListPage = () => {
  ...
  return (
    <Container>
      ...
      {!loading && !productMedias.length && <div>No Digital Products</div>}
          ...
          <Table.Body>
            {productMedias.map((product_media) => (
              <Table.Row key={product_media.id}>
                ...
                <Table.Cell>
                  {product_media.variant && (
                    <Link to={`/products/${product_media.variant.product_id}`}>
                      View Product
                    </Link>
                  )}
                </Table.Cell>
...
import { Button, Container, Input, Label, Select, Drawer } from "@medusajs/ui"
...
export default async function handleOrderPlaced({
  ...
  for (const item of order.items) {
    const productMedias = await digitalProductModuleService.listProductMedias({
    ...
    const downloadUrls = await Promise.all(
      productMedias.map(async (productMedia) => {
        ...
        return (await fileModuleService.retrieveFile(productMedia.file_key)).url
      })
    )
