...
Finally, create the file `src/api/routes-config.ts` with the following content:
...
Next, add to the file `src/api/routes-config.ts` the `multer` middleware on this API route:
