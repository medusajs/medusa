...
- [Learn more about Query](!docs!/learn/fundamentals/query)
...
export const retrieveDigitalProductsHighlights = [
  ["20", "productVariants", "Retrieve the product variants of the deleted product."],
  ["23", "withDeleted", "Include deleted product variants in the result."],
  ["26", "graph", "Retrieve the digital products associated with the product variants."],
  ["27", "DigitalProductVariantLink.entryPoint", "Pass the link as an entry point."],
  ["34", "digitalProductIds", "Extract the IDs of the digital products."],
  ["36", "digitalProductIds", "Return the digital product IDs."],
]
...
3. Check whether the Medusa order has fulfillments. If so, mark the order's fulfillment as delivered using `markOrderFulfillmentAsDeliveredWorkflow` from Medusa's core workflows. The `no_notification` parameter is set to `true` to prevent duplicate notifications since the customer already received the digital products via the custom notification step.
...
### Allow Retrieving Digital Products

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, to retrieve digital products from the `/store/products` route, add the following route object to the array passed to `defineMiddlewares` in `src/api/middlewares.ts`:
