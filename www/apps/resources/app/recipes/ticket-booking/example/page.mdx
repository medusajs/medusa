...
The Medusa [Framework](!docs!/learn/fundamentals/framework) facilitates customizing Medusa's core features for your specific use case, such as ticket booking.
...
4. Retrieve the created venue with its rows using the `useQueryGraphStep`.
    - This step uses [Query](!docs!/learn/fundamentals/query) under the hood. It allows you to retrieve data across modules.
...
Refer to the [Query](!docs!/learn/fundamentals/query#request-query-configurations) documentation to learn more about query request configurations.
