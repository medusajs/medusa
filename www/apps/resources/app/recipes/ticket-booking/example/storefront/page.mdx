...
### Allow Retrieving Ticket Products

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, to retrieve ticket products from the `/store/products` route, add the following route object to the array passed to `defineMiddlewares` in the Medusa application's `src/api/middlewares.ts` file:
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("ticket_product")],
    },
  ],
})
```
