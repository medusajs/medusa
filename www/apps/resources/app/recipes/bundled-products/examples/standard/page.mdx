...
export const getBundledProductsRouteHighlights = [
  ...
  ["12", "graph", "Retrieve the bundles."],
  ["14", "queryConfig", "Pass the query configurations."],
  ["17", "json", "Return the bundles in the response."]
]
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container. Then, you call its `graph` method to retrieve the bundles.
...
export const bundleProductsRouteHighlights = [
  ["11", "id", "The ID of the bundle, received as a path parameter."],
  ["15", "currency_code", "The currency code to retrieve the prices in."],
  ["15", "region_id", "The region ID to retrieve the prices in."],
  ["32", "context", "Pass the context necessary to retrieve correct prices."]
]
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container.
...
To retrieve the correct price for each variant, you also pass a [Query Context](!docs!/learn/fundamentals/query/query-context) with the region ID and currency code that are passed as query parameters. This ensures that the prices are shown accurately to the customer.
...
### Allow Retrieving Bundles with Products

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, to retrieve bundles from the `/store/products` route, add the following route object to the array passed to `defineMiddlewares` in `src/api/middlewares.ts`:
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("bundle")],
    },
  ],
})
```
