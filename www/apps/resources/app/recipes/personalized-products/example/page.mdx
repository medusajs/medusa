...
1. Retrieve the region's currency code using the `useQueryGraphStep` step. This is necessary to calculate the variant's price in the correct currency.
    - The `useQueryGraphStep` uses [Query](!docs!/learn/fundamentals/query) to retrieve data across modules.
...
If you're not using custom pricing, you can skip this step and keep on using Medusa's existing [Add-to-Cart API route](!api!/store/carts/add-line-item).
...
export const validatePersonalizedProductHighlights = [
  ["7", "validate", "Consume the `validate` hook of the `addToCartWorkflow`."],
  ["9", "query", "Resolve Query from the Medusa container."],
  ["12", "variants", "Retrieve the product variants being added to the cart."],
  ["21", "", "Validate that the product is personalized."],
  ["24", "", "Validate that the height and width metadata are set."],
]
...
1. Resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container to retrieve data across modules.
