...
In this guide, all file paths are relative to the `apps/backend` directory of your Medusa project.
...
Learn more about data model indexes in [this documentation](!docs!/learn/fundamentals/data-models/indexes).
...
export const validateCustomerWishlistHighlights = [
  ["18", "graph", "Retrieve the wishlist based on the specified customer ID."],
  ["26", "", "Throw an error if the customer already has a wishlist."],
  ["34", "graph", "Retrieve the customer based on the specified customer ID."],
  ["33", "", "Throw an error if the customer doesn't exist."],
]
...
In the step function, you use [Query](!docs!/learn/fundamentals/query) to retrieve the wishlist based on the specified customer ID. If a wishlist exists, you throw an error, stopping the workflow's execution.
...
In the `apps/backend` directory, run the following command to start the development server:
...
In this route handler function, you use [Query](!docs!/learn/fundamentals/query) to retrieve the wishlist of the authenticated customer. For each wishlist, you retrieve its items, and the product variants of those items.
...
export const validateVariantWishlistHighlights = [
  ["23", "isInWishlist", "Check whether the variant is already in the wishlist."],
  ["38", "graph", "Retrieve the variant's details along with the sales channels that its product is available in."],
  ["50", "variantInSalesChannel", "Throw an error if the variant isn't in the wishlist's sales channel."]
]
...
export const wishlistCountRouteHighlights = [
  ["22", "graph", "Retrieve the product and its variants."],
  ["30", "", "If product doesn't exist, throw an error."],
  ["37", "count", "Retrieve the number of wishlists that the product is in."]
]
...
export const widgetHighlights = [
  ...
  ["37", "zone", "Inject the widget into a product's details page."]
]
...
3. You should see the widget you created showing the number of wishlists the product is in at the end of the page.
