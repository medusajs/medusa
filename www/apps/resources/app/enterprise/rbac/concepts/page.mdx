...
An assignment's `scope` and `scope_id` restrict the role to a single entity instance. For example, an assignment whose `scope` is `vendor` and whose `scope_id` is `ven_123` grants its policies on requests acting within that vendor.
...
export const scopedHighlights = [
  ["12", "scope", "Restrict the role to this vendor."],
    ...
    assignments: [
      {
        ...
        reference: "user",
        reference_id: "user_123",
        scope: { type: "vendor", id: "ven_123" },
      },
    ],
    granting_actor_id: "user_456",
    granting_scope: { type: "vendor", id: "ven_123" },
  },
})
```
...
In the above example, the user assigning the role can only grant it for the `ven_123` vendor, since their effective policies are resolved in that scope. They can't assign the role for another vendor, since they don't have access there.

If you omit `granting_scope`, the workflow resolves the granting actor's effective policies across all their assignments, whatever scope each one has. A user with a role for one vendor can then grant that role for another, so pass `granting_scope` whenever the grant happens within a scope.
...
Medusa also applies the policies to the fields a request asks for. If the actor can't read a related resource, Medusa removes the fields of that resource from the response instead of rejecting the request. This requires the separate `rbac_filter_fields` feature flag, as explained in the [Protect API Routes](../protect-routes/page.mdx#field-level-filtering) guide.
