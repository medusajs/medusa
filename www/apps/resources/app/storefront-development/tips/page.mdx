...
The JS SDK sends requests to Medusa’s Store API routes, which is enough for listing and filtering by a field’s exact value. For a search experience with full-text search, facets, and sorting, use the [InstantSearch adapter](../../instantsearch/page.mdx) instead. It’s the recommended approach since [Medusa v2.21.1](https://github.com/medusajs/medusa/releases/tag/v2.21.1), and it requires the [Search Module](../../infrastructure-modules/search/page.mdx) configured with a provider in your Medusa application.
