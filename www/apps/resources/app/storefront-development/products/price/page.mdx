...
When you retrieve products either with the [List Products] or [Retrieve Products] API routes, you must pass at least one of the following query parameters to retrieve the correct product variant price:
...
export const priceHighlights = [
  ...
  ["135", "", "If there's a computed price but no selected variant, show a `From` prefix to the price."],
  ["136", "price", "Display the computedd price."]
]
    ...
    <div>
      ...
      {product && (
        <>
          ...
          {selectedVariant && <span>Selected Variant: {selectedVariant.id}</span>}
          {price && (
            <span>
              {isSale && "SALE"}
              {!selectedVariant && "From: "}
