...
A cart's currency is set from the region you associate it with. If you pass a `currency_code` when creating a cart, it must match the region's currency — otherwise, an error is thrown. To avoid errors, omit `currency_code` and let Medusa set it from the region automatically.
...
Medusa doesn't reject a product variant added to the cart when its product isn't available in the cart's sales channel. Sales channel availability filters the products you retrieve, but Medusa doesn't enforce it when adding items to the cart. Refer to the [Enforce Sales Channel Availability in Carts](../../../commerce-modules/cart/sales-channel-availability/page.mdx) guide to learn how to add that validation.
