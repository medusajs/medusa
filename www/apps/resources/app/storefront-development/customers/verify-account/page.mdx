...
9. The verification page sends a `POST` request to the `/auth/verification/confirm` [Confirm Verification](!api!/store/auth/verify-the-customers-email) API route to verify the customer's email using the `token` query parameter in the URL.
...
Next, create a page in your storefront at the path that the verification link points to (for example, `/verify-account`). The page reads the `token` from the query string and sends it to the [Confirm Verification API route](!api!/store/auth/verify-the-customers-email) to verify the customer's email.
