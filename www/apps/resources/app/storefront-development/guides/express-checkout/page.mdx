...
Don't add a `method` or `methods` key to the middleware's object. Medusa runs a method-scoped middleware after it validates the query parameters, so pushing to `req.allowed` in it has no effect.
