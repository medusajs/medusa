...
### Allow Cart Variant Option Fields

Medusa restricts the fields that you can request from a Store API route through an allowed-fields list. Later in this guide, you'll retrieve a cart with its line items' variant options and its products' options, which the cart routes don't allow by default. Medusa omits any field that isn't allowed from the response.

To allow those fields, create the file `src/api/middlewares.ts` in the Medusa application with the following content:
...
The Store Cart API routes expand at most three levels of relations, so you can't request `items.variant.options.option` to get an option's title. Instead, you allow `items.product.options`, which holds the same titles at three levels, and match each variant option to its product option in the storefront. Learn more in the [storeRelationsLimit](!docs!/learn/configurations/medusa-config#httpstorerelationslimit) configuration.
...
In the `useEffect` hook, you use the JS SDK to send a request to the [List Products](!api!/store/products/list-products) API route, passing the following query parameters:
...
In the `useEffect` hook, you use the JS SDK to send a request to the [List Shipping Options of a Cart](!api!/store/shipping-options/list-shipping-options-for-cart) API route, passing the cart's ID as a query parameter. The API route returns a list of shipping options, which you set in the `shippingOptions` state variable.
...
So, to retrieve the prices of calculated shipping options, you use Medusa's [Calculate Shipping Option Price](!api!/store/shipping-options/calculate-shipping-option-price) API route.
...
In the `useEffect` hook, you use the JS SDK to send a request to the [List Payment Providers](!api!/store/payment-providers/list-payment-providers) API route, passing the cart's region ID as a query parameter.
...
You define a `placeOrder` function that uses the JS SDK to complete the cart by sending a request to the [Complete Cart API route](!api!/store/carts/complete-cart). If the cart is successfully completed, you unset the cart and redirect the customer to the confirmation page, which you'll implement later.
