...
1. Retrieve the available shipping options for the cart using the [List Shipping Options API route](!api!/store/shipping-options/list-shipping-options-for-cart) and show them to the customer.
2. For shipping options whose `price_type=calculated`, you retrieve their calculated price using the [Calculate Shipping Option Price API Route](!api!/store/shipping-options/calculate-shipping-option-price).
...
3. When the customer chooses shipping option(s), you use the [Add Shipping Method to Cart API route](!api!/store/carts/add-shipping-method) to add the cart's shipping method(s). You can add a single shipping method or multiple methods in one request.
...
- Retrieve the available shipping options of the cart to allow the customer to select from them using the [List Shipping Options API route](!api!/store/shipping-options/list-shipping-options-for-cart).
- For each shipping option, you retrieve its calculated price from the Medusa application using the [Calculate Shipping Option Price API Route](!api!/store/shipping-options/calculate-shipping-option-price).
- Once the customer selects a shipping option, you send a request to the [Add Shipping Method to Cart API route](!api!/store/carts/add-shipping-method) to update the cart's shipping method using the selected shipping option.
...
Multiple shipping methods support is available since Medusa [v2.16.0](https://github.com/medusajs/medusa/releases/tag/v2.16.0).

</Note>

The [Add Shipping Method to Cart API route](!api!/store/carts/add-shipping-method) supports adding multiple shipping methods to a cart in a single request by passing an array of shipping method objects instead of a single object.
...
When calculating a shipping option's price using the [Calculate Shipping Option Price API Route](!api!/store/shipping-options/calculate-shipping-option-price), or when setting the shipping method using the [Add Shipping Method to Cart API route](!api!/store/carts/add-shipping-method), you can pass a `data` request body parameter that holds data relevant for the fulfillment provider.
