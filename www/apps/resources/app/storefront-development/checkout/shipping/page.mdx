...
3. When the customer chooses shipping option(s), you use the [Add Shipping Method to Cart API route](!api!/store#carts_postcartsidshippingmethods) to add the cart's shipping method(s). You can add a single shipping method or multiple methods in one request.
...
Multiple shipping methods support is available since Medusa [v2.15.6](https://github.com/medusajs/medusa/releases/tag/v2.15.6).

</Note>

The [Add Shipping Method to Cart API route](!api!/store#carts_postcartsidshippingmethods) supports adding multiple shipping methods to a cart in a single request by passing an array of shipping method objects instead of a single object.
...
- Removes any existing shipping methods that conflict with the new ones based on shipping profile
