...
When the user enters their email, use the [Update Cart API route](!api!/store/carts/update-a-cart) to update the cart with the email.
...
After the customer enters and submits their email, you send a request to the [Update Cart API route](!api!/store/carts/update-a-cart) passing it the email in the request body.
