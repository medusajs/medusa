...
- A captured payment is refunded, unless another concurrent completion attempt already placed an order for the same cart. In that case, the refund is skipped so that the successful order keeps its payment.
