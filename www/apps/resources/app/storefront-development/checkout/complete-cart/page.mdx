...
To complete the cart, send a request to the [Complete Cart API route](!api!/store/carts/complete-cart). For example:
...
Check `type === "order"` to confirm cart completion. Don't assume the payment was charged based on calling the complete cart endpoint alone.
...
- Send a request to the [Complete Cart API route](!api!/store/carts/complete-cart) once all actions with the third-party payment provider are performed.
