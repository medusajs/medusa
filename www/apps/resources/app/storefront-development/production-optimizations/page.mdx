...
Each Store API route restricts the fields that you can request through an allowed-fields list. If you request a field that the route doesn't allow, Medusa omits it from the response without returning an error.

To allow extra fields, such as a custom relation, override the route's allowed fields as explained in the [Allowed Fields](!docs!/learn/fundamentals/api-routes/allowed-fields) documentation.
 ...
 *   fields: '*items, *items.variant, *items.product, shipping_methods'
 * });
 * 
 * // In a component
