...
- `expires_at`: An optional expiration date and time for the gift card. When `null`, the gift card never expires.
...
A gift card with a `null` value for `expires_at` never expires. When `expires_at` is set, the Loyalty Plugin enforces expiry at the following points:

- **Adding to cart**: If a customer tries to apply an expired gift card code, the `addGiftCardToCartWorkflow` throws an error and the gift card is not added.
- **Checkout**: When the customer places the order, the `confirmCartCreditLinesWorkflow` checks all gift cards applied to the cart. If any have expired since being applied, it throws an error before debiting balances.
- **Cart refresh**: The `refreshCartGiftCardsWorkflow` silently drops expired gift cards from the cart so the cart can still be refreshed. Their credit lines and links are removed first.
