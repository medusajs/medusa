...
To retrieve the gift cards of a cart with [Query](!docs!/learn/fundamentals/query), pass `gift_cards.*` in `fields`:
...
To retrieve the gift cards of an order with [Query](!docs!/learn/fundamentals/query), pass `gift_cards.*` in `fields`:
...
To retrieve the order line item of a gift card with [Query](!docs!/learn/fundamentals/query), pass `line_item.*` in `fields`:
...
To retrieve the store credit account of a gift card with [Query](!docs!/learn/fundamentals/query), pass `store_credit_account.*` in `fields`:
