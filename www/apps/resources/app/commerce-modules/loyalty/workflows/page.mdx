...
Apply a gift card to a cart as a credit line. This workflow validates that the gift card has not expired before adding it:
...
This hook is available since Medusa [v2.20.2](https://github.com/medusajs/medusa/releases/tag/v2.20.2).

</Note>

`addGiftCardToCartWorkflow` exposes a `validate` hook that runs after the gift card's built-in validations and before any credit line is created. Use it to add custom validation logic. Throw an error inside the hook to reject the gift card and stop the workflow.
...
addGiftCardToCartWorkflow.hooks.validate(
  async ({ input, cart, giftCards }, { container }) => {
    ...
    if (giftCards.some((gc) => gc.currency_code !== cart.currency_code)) {
      throw new Error("Gift card currency does not match cart currency.")
    }
  }
)
```

The hook receives the workflow `input`, the resolved `cart`, and the resolved `giftCards` array. You can resolve services from `container` to perform database lookups or any other custom checks.
...
Refresh gift card credit lines on a cart to account for balance or cart total changes. Expired gift cards are silently removed from the cart during a refresh rather than causing an error, so the cart can continue to be updated:
...
Confirm and debit gift card balances during checkout. This workflow also validates that none of the gift cards applied to the cart have expired before debiting any balances:
...
```ts
import { linkGiftCardsToOrderWorkflow } from "@medusajs/loyalty-plugin/workflows"
