...
In this example, you create a shipping option whose default price is `$10`. When the total of the cart or order using this shipping option is greater than `$100`, the shipping option's price becomes free. When the cart's `weight_total` is `5000` or more, the shipping option's price becomes `$15`.
...
A cart also has a `weight_total` property, which is the sum of the weights of its items. So, if a shipping option has the following price:
...
The shipping option's price is applied when the cart's `weight_total` is greater than or equal to `5000`.

<Note title="What weight_total represents">

The cart's `weight_total` is the sum of its line items' weights, where each line item's weight is its `unit_weight` multiplied by its quantity. A line item's `unit_weight` is inherited from the `weight` property of its product variant. If no weight is set on the item or its variant, Medusa calculates the line item's weight as `0`.
