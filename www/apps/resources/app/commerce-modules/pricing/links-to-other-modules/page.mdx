...
When you retrieve variant prices through `variants.prices.*` using Query, only prices that are not part of a price list are returned. Prices belonging to a price list (those with a non-null `price_list_id`) are excluded. Refer to the [Get Variant Prices guide](../../product/guides/price/page.mdx) for how to retrieve price-list prices.
