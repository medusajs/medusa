...
To do that, you'll consume the [productsCreated](/references/medusa-workflows/createProductsWorkflow#productscreated) hook of the [createProductsWorkflow](/references/medusa-workflows/createProductsWorkflow). This workflow is executed in the [Create Product Admin API route](!api!/admin/products/create-product)
...
Also, make sure to replace `{shipping_profile_id}` with an existing shipping profile's ID. You can retrieve one either from the Medusa Admin dashboard or using the [List Shipping Profiles API Route](!api!/admin/shipping-profiles/list-shipping-profiles).
...
To retrieve the `custom_name` property when you're retrieving the product through API routes, such as the [Get Product API Route](!api!/admin/products/get-a-product), pass in the `fields` query parameter `+custom.*`, which retrieves the linked `Custom` record's details.
...
### Retrieve in Store API Routes

The Store API routes only return the fields in their allowed fields, and `custom` isn't one of them, so Medusa removes it from the response without an error. To retrieve it through a route like the [List Products API Route](!api!/store/products/list-products), add it to the route's allowed fields in `src/api/middlewares.ts`:
...
You can then pass `+custom.*` in the `fields` query parameter of the Store Product API routes.
...
You can also retrieve the `Custom` record linked to a product in your code using [Query](!docs!/learn/fundamentals/query).
...
Learn more about how to use Query in [this guide](!docs!/learn/fundamentals/query).
...
The `updateProductsWorkflow` is executed by the [Update Product API route](!api!/admin/products/update-a-product), which accepts the `additional_data` parameter to pass custom data to the hook.
