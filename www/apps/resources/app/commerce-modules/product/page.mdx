...
## Built-in Product Workflows

Medusa provides built-in workflows to manage products, available through the `@medusajs/medusa/core-flows` package. Here are some commonly used ones:

### Export Products Workflow

Use the `exportProductsWorkflow` to export products to a CSV file:

```ts title="src/workflows/export-example.ts"
...
// Export products from a specific sales channel
const { result } = await exportProductsWorkflow(container)
  .run({
    input: {
      select: ["id", "title", "handle"],
...
This workflow is used by the [Export Products Admin API Route](!api!/admin#products_postproductsexport) and sends a notification to admin users when the export is complete, allowing them to download the CSV file.

For more product workflows, refer to the [Product Workflows Reference](!resources!/commerce-modules/product/workflows).
