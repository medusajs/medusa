...
To retrieve the shipping profile of a product with [Query](!docs!/learn/fundamentals/query), pass `shipping_profile.*` in `fields`:
...
To retrieve the inventory items of a product variant with [Query](!docs!/learn/fundamentals/query), pass `inventory_items.inventory.*` in `fields`. The `inventory_items` relation holds link records, so use `inventory_items.inventory` to access the actual `InventoryItem` data:
...
To retrieve the price set of a variant with [Query](!docs!/learn/fundamentals/query), pass `price_set.*` in `fields`:
...
To retrieve the sales channels of a product with [Query](!docs!/learn/fundamentals/query), pass `sales_channels.*` in `fields`:
...
To retrieve the translations of a product with [Query](!docs!/learn/fundamentals/query), pass `translations.*` in `fields`:
