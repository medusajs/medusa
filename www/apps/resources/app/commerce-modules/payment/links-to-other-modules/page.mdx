...
To retrieve the cart associated with the payment collection with [Query](!docs!/learn/fundamentals/query), pass `cart.*` in `fields`:
...
To retrieve the customer associated with an account holder with [Query](!docs!/learn/fundamentals/query), pass `customer.*` in `fields`:
...
To retrieve the order of a payment collection with [Query](!docs!/learn/fundamentals/query), pass `order.*` in `fields`:
...
To retrieve the regions of a payment provider with [Query](!docs!/learn/fundamentals/query), pass `regions.*` in `fields`:
