...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      A Stripe Payment Method Configuration ID that controls which payment methods are presented to customers. Useful when you manage payment methods from the Stripe Dashboard or need different sets for different scenarios (subscriptions, one-time, Connect). Refer to [Stripe's docs](https://docs.stripe.com/payments/payment-method-configurations).
