...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      `paymentMethodConfiguration` (v2.18.0+)
      
      </Table.Cell>
      <Table.Cell>
      
      A string indicating the ID of a Stripe [Payment Method Configuration](https://docs.stripe.com/payments/payment-method-configurations) (PMC) to apply a Dashboard-managed set of payment methods to the intent. This option is only used when `payment_method_types` is not set.
