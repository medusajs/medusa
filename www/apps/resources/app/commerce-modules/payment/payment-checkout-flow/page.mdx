...
- [Create and Initialize Payment Session](!api!/store/payment-collections/initialize-payment-session): Create a payment session for the selected payment provider in the Medusa application, and initialize the session in the third-party payment provider. Medusa validates that the chosen provider is enabled in the cart's region and throws an error if it is not.
