...
- `resolve`: A string indicating the package name of the payment module provider or the path to it relative to the `src` directory.
- `options`: An optional object of the payment module provider's options. The object must have the following property:
  - `config`: An object whose key is the ID of the payment provider, and its value is an object of options to pass to the module provider.