...
In the Medusa application, you need to use [Query](!docs!/learn/fundamentals/query) to retrieve the available payment providers in a region.
...
When creating a payment session through `createPaymentSessionsWorkflow`, Medusa validates that the chosen payment provider is enabled in the cart's region. If the provider is not enabled in the region, the workflow throws an error. This validation only applies to payment collections linked to an active (not yet completed) cart; payment collections on orders are not affected.
