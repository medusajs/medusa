...
You can also edit an order's locale using the [Update Order](!api!/admin/orders/update-order) API route.
...
An order item exposes two metadata fields:

- `metadata`: Custom data stored on the versioned [OrderItem](/references/order/models/OrderItem) record. This metadata is specific to a version of the item in the order.
- `line_item_metadata`: Custom data stored on the underlying [OrderLineItem](/references/order/models/OrderLineItem) record. This metadata belongs to the line item itself, independent of versioning.

<Note>

`line_item_metadata` is available starting [Medusa v2.16.0](https://github.com/medusajs/medusa/releases/tag/v2.16.0).
