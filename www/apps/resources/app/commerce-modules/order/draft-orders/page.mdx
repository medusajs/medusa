...
A draft order is stored in the database as a regular order. It is represented by the [Order data model](/references/order/models/Order) with the following property:
...
Once a draft order is finalized and ready for processing, it can be converted to a regular order by changing its `status` property to `pending`.
