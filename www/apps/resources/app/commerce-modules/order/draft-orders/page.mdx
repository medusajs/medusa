...
### Promotions in Draft Orders

You can add promotions to draft orders using the `addDraftOrderPromotionsWorkflow`. Once a promotion is attached to a draft order, Medusa recomputes adjustments for that promotion whenever items or shipping methods change.

<Note title="Warning" type="warning">

Automatic promotions (those with `is_automatic: true`) are not auto-discovered for draft orders. Medusa only evaluates promotions that have already been explicitly added to the draft order. To apply a promotion to a draft order, add it using `addDraftOrderPromotionsWorkflow`.

</Note>

This is different from store cart checkout flows, where automatic promotions are discovered and applied without requiring a promotion code.
...
This behavior is available since Medusa [v2.20.0](https://github.com/medusajs/medusa/releases/tag/v2.20.0).
...
This behavior is available since Medusa [v2.19.0](https://github.com/medusajs/medusa/releases/tag/v2.19.0).
