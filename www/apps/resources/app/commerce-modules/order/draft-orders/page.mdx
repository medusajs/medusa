...
The `fetchShippingOptionForDraftOrderWorkflow` exposes a `setPricingContext` hook you can use to inject additional context (for example, a `location_id`) before the fulfillment provider calculates the price. The context is merged with the order's `currency_code`.
