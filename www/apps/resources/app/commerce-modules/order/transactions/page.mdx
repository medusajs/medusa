...
To retrieve an order's refunds, you can retrieve the order's transactions using [Query](!docs!/learn/fundamentals/query) and filter the transactions by their `reference` property.
