...
An order is associated with the customer that placed it. Medusa defines a link module that builds a relationship the `Order` and `Customer` data models.

![A diagram showcasing an example of how data models from the Order and Customer modules are linked](https://res.cloudinary.com/dza7lstvk/image/upload/v1716555220/Medusa%20Resources/order-customer_lazrqz.jpg)
...
An order's payment is stored in a payment collection. Medusa defines a link module that builds a relationship the `Order` and `PaymentCollection` data models.
...
An order's line item is associated with the purchased product and its variant. Medusa defines a link module that builds a relationship the `LineItem`, `Product`, and `ProductVariant` data models.
...
An order is associated with the promotion applied on it. Medusa defines a link module that builds a relationship the `Order` and `Promotion` data models.
...
An order is associated with the customer's region. Medusa defines a link module that builds a relationship the `Order` and `Region` data models.
