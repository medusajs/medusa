...
- **Email Verification**: Require email verification for new registrations and authentications. Learn more in the [email verification guide](./email-verification/page.mdx).
- **Multi-Factor Authentication (MFA)**: Add an extra layer of security with time-based one-time passwords (TOTP), backup recovery codes, and configurable disable policies. Learn more in the [Auth Module Service reference](/references/auth/IAuthModuleService/methods/auth.IAuthModuleService.startAuthMfa).
