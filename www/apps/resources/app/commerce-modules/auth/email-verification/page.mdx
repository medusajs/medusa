...
To enable email verification, set the `require_verification` property of the [Emailpass Auth Module Provider](../auth-providers/emailpass/page.mdx) in `medusa-config.ts`:

export const configHighlights = [
  ["16", "require_verification", "Array of actor types that require email verification. Include `customer` to require verification for customers."],
]
...
With this configuration, customers will be required to verify their email address after registering. You can specify multiple actor types in the array if needed. This includes previously registered customers as well.

<Note>

Before [Medusa v2.15.6](https://github.com/medusajs/medusa/releases/tag/v2.15.6), `require_verification` was a boolean option. To migrate from the previous version, change `require_verification: true` to `require_verification: ["customer"]`.
