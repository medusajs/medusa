---
sidebar_label: "Link Provider to Existing Customer"
tags:
    - auth
    - how to
    - server
...
export const metadata = {
  title: `Link an Additional Auth Provider to an Existing Customer`,
}

# {metadata.title}

In this guide, you'll learn how to allow an existing customer to authenticate with an additional authentication provider, such as letting a customer who registered with the Emailpass provider also log in with Google or a Magic Link.

<Prerequisites
  items={[
    {
      text: "Existing customer.",
      link: "/commerce-modules/customer",
    },
    {
      text: "Custom authentication provider registered for the customer actor type.",
      link: "/references/auth/provider",
    },
  ]}
/>

## Why an Additional Provider Doesn't Resolve to the Customer

Each authentication provider creates its own [auth identity](../auth-identity-and-actor-types/page.mdx) for a customer. The auth identity holds an `app_metadata` property that associates it with an actor, such as a customer, using the key `{actor_type}_id`:
...
When a customer registers with the Emailpass provider, its auth identity has the `customer_id` key set to the associated customer's ID. So, authenticating with Emailpass resolves to that customer.

However, when the same customer authenticates with a different provider for the first time, that provider creates a *separate* auth identity. This new auth identity has an empty `app_metadata`, so it isn't associated with any customer yet:
...
Since the new auth identity isn't associated with a customer, Medusa treats the authentication as if a new customer is registering, rather than an existing customer logging in. So, authenticating with the new provider returns a token that has an `auth_identity_id` but no `actor_id`. As a result, requests to protected routes, such as `GET /store/customers/me`, are rejected with a `401` error.

To allow the new provider to resolve to the existing customer, you must set the `{actor_type}_id` key of the new auth identity's `app_metadata` to the existing customer's ID. You do that with the [setAuthAppMetadataStep](/references/medusa-workflows/steps/setAuthAppMetadataStep).
