...
The Auth Module automatically applies default MFA configuration, including setting the `mfa.encryption_key` to the `AUTH_MFA_ENCRYPTION_KEY` environment variable. If you created your Medusa application after v2.15.5, this environment variable is already set in your `.env` file.

You only need to explicitly configure MFA options if you want to override the defaults. For basic usage, the Auth Module works out of the box without additional MFA configuration.
...
- `encryption_key`: A string indicating the encryption key used for MFA. By default, this is automatically set to the `AUTH_MFA_ENCRYPTION_KEY` environment variable. You only need to specify this option if you want to override the default.
  - If the `AUTH_MFA_ENCRYPTION_KEY` environment variable is not set and you haven't explicitly configured this option, you'll get a "MFA encryption key is required to use MFA methods" error when trying to enroll or verify an MFA factor.
