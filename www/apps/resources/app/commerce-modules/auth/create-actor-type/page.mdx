...
To do that, create the file `src/api/routes-config.ts` with the following content:

export const middlewareHighlights = [
  ["11", "authenticate", "Require the user to be authenticated to access the `/manager` API route when sending a `POST` request."],
  ["12", "allowUnregistered", "The user doesn't need to be registered to access the API route."],
  ["19", "authenticate", "Require the user to be authenticated as a manager when accessing `/manager/me` API routes."]
]

```ts title="src/api/routes-config.ts" highlights={middlewareHighlights}
import { 
  defineRoutesConfig,
  authenticate
} from "@medusajs/medusa"
