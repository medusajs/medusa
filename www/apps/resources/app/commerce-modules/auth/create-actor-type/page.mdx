...
export const deleteHighlights = [
  ...
  ["44", "shouldKeepAuthIdentity", "Only keep the auth identity if it is still associated with other actor types."],
  ["50", "setAuthAppMetadataStep", "Remove only the manager association from the shared auth identity."],
  ["57", "deleteAuthIdentityStep", "Delete the auth identity entirely when the manager is the sole actor type."]
]
