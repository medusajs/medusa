...
export const deleteHighlights = [
  ["31", "manager_id", "If your actor type has a different name, such as vendor, change it to be `{actor_type}_id`."],
  ["43", "shouldKeepAuthIdentity", "Only keep the auth identity if it is still associated with other actor types."],
  ["58", "setAuthAppMetadataStep", "Remove only the manager association from the shared auth identity."],
  ["68", "deleteAuthIdentityStep", "Delete the auth identity entirely when the manager is the sole actor type."]
]
