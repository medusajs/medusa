...
You can register additional MFA providers in the `mfa.providers` array of the Auth Module's options. Custom MFA providers must implement specific interfaces and include a static `identifier` property.

#### Regular MFA Providers

For regular MFA factors (like TOTP), implement the `AuthMfaProvider` interface with these methods:
- `start` - Initialize enrollment for the factor
- `verifySetup` - Verify the factor during enrollment
- `canVerifyForAuthIdentity` - Check if the identity has this factor enabled
- `verify` - Verify a code during authentication

Every MFA provider must have a static `identifier` property. The provider's ID will be stored as `mfa_{identifier}`:

```ts
class CustomMfaProvider implements AuthMfaProvider {
  static identifier = "my-custom-mfa"
  
  // Implement required methods...
}
```

#### Recovery Code Providers

For recovery code providers, implement the `RecoveryCodeAuthMfaProvider` interface with additional methods for generating and managing recovery codes.

Recovery code providers must have their static `identifier` property set to `"recovery_code"`:

```ts
class CustomRecoveryProvider implements RecoveryCodeAuthMfaProvider {
