...
1. Generate a token using the [Get Reset Password Token API route](#get-reset-password-token-route).
    - The API route emits the `auth.password_reset` event, passing the token in the payload.
    - You can create a subscriber that listens to the event and send a notification to the user. The notification should have a URL that accepts the token as a query parameter.
2. Pass the token to the [Reset Password API route](#reset-password-route) to reset the password.
    - The URL in the user's notification can direct them to a frontend URL, which sends a request to the Reset Password API route.
...
The route accepts a `token` query parameter, which is the token generated using the [Get Reset Password Token route](#get-reset-password-token-route).
