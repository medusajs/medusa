...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      verification (for example `["customer"]`).

      Before [Medusa v2.16.0](https://github.com/medusajs/medusa/releases/tag/v2.16.0), 
      this was a boolean option. To migrate, change `require_verification: true` 
      to `require_verification: ["customer"]` (or other actor types as needed).
