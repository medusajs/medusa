...
In this guide, you'll learn how to retrieve cart totals in your Medusa application using [Query](!docs!/learn/fundamentals/query).
...
- `weight_total`: The sum of all line items' weights. Each line item's weight is its `unit_weight` multiplied by its quantity.
...
- `unit_weight`: The weight of a single unit of the line item, inherited from the `weight` property of its product variant. This field is not calculated and is stored in the database.
...
- `weight_total`: The line item's weight, calculated as its `unit_weight` multiplied by its quantity.
