...
- If `automatic_taxes` is disabled, Medusa only calculates tax lines when you send a request to the [Calculate Cart Taxes API route](!api!/store#carts_postcartsidtaxes).
...
To force Medusa to recalculate a cart's tax lines, even when `automatic_taxes` is disabled, send a request to the [Calculate Cart Taxes API route](!api!/store#carts_postcartsidtaxes). This also triggers a fresh calculation from your third-party tax provider, if you use one.
