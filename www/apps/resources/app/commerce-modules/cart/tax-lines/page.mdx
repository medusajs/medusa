...
Tax lines have the following notable fields:

- `code`: The tax code.
- `rate`: The tax rate.
- `provider_id`: The ID of the tax provider that calculated the tax line.
- `data`: Holds additional data returned by the tax provider, such as jurisdiction-level rate breakdowns.
- `metadata`: Holds custom key-value data.
