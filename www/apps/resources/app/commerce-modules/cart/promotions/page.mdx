...
```ts collapsibleLines="1-6" expandButtonLabel="Show Imports"
import {
...
Creating an adjustment without a `code` is supported since Medusa [v2.19.1](https://github.com/medusajs/medusa/releases/tag/v2.19.1). In previous versions, the `code` property was required in the create input types, even though the underlying data model allowed omitting it.
