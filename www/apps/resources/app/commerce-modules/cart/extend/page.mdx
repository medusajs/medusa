...
To do that, you'll consume the [cartCreated](/references/medusa-workflows/createCartWorkflow#cartcreated) hook of the [createCartWorkflow](/references/medusa-workflows/createCartWorkflow). This workflow is executed in the [Create Cart API Route](!api!/store/carts/create-cart).
...
### Allow Retrieving Custom Fields in API Routes

The Store API routes only return the fields that they allow in the `fields` query parameter. Since the `custom` link isn't part of the Store Cart routes' allowed fields, Medusa removes it from the response without an error.

So, before you can retrieve the linked `Custom` record through the Store Cart API routes, you must add `custom` to the allowed fields of those routes.

To do that, add a new route middleware configuration object in `src/api/middlewares.ts`:
...
This middleware pushes `custom` to the allowed fields of the Store Cart API routes, so Medusa keeps the field when a request asks for it.

<Note title="Important">

The middleware must be global, so don't set a `method` property in the route object, even though some of the Store Cart API routes accept `POST` requests. Medusa runs a middleware scoped to a method after it validates the `fields` query parameter, so it has no effect.
...
After you allow the `custom` field, you can retrieve the `custom_name` property when you're retrieving the cart through API routes, such as the [Get Cart API Route](!api!/store/carts/get-a-cart). To do that, pass in the `fields` query parameter `+custom.*`, which retrieves the linked `Custom` record's details.
...
You can also retrieve the `Custom` record linked to a cart in your code using [Query](!docs!/learn/fundamentals/query).
...
To test it out, send a `POST` request to `/store/carts/:id` to update a cart, passing `custom_name` in `additional_data`. The request also passes `+custom.*` in the `fields` query parameter, which the [middleware you added earlier](#allow-retrieving-custom-fields-in-api-routes) allows:
