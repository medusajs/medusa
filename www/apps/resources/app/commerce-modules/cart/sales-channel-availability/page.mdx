...
If your store has only one sales channel, the List Products API route skips the sales channel filter as an optimisation and returns all products, including those with no sales channel link. The retrieve route (`GET /store/products/:id`) always applies the filter. This means a product that has no sales channel assigned can appear in list results but return a 404 on retrieve. To avoid this inconsistency, make sure every product is assigned to at least one sales channel.
