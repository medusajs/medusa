...
To retrieve the `custom_name` property when you're retrieving the promotion through API routes, such as the [Get Promotion API Route](!api!/admin/promotions/get-a-promotion), pass in the `fields` query parameter `+custom.*`, which retrieves the linked `Custom` record's details.
...
Medusa doesn't have a Store Promotion API route. A storefront retrieves a cart's promotions through the [Store Cart API routes](!api!/store/carts) instead. Those routes only return the fields in their allowed fields, and `promotions.custom` isn't one of them, so Medusa removes it from the response without an error. To retrieve it, add it to the route's allowed fields in `src/api/middlewares.ts`:
...
export default defineMiddlewares({
  routes: [
    ...
    {
      matcher: "/store/carts",
      middlewares: [allowFields("promotions.custom")],
    },
  ],
})
```
...
You can then pass `+promotions.custom.*` in the `fields` query parameter of the Store Cart API routes.
...
You can also retrieve the `Custom` record linked to a promotion in your code using [Query](!docs!/learn/fundamentals/query).
...
The `updatePromotionsWorkflow` is executed by the [Update Promotion API route](!api!/admin/promotions/update-a-promotion), which accepts the `additional_data` parameter to pass custom data to the hook.
