...
This feature is available since Medusa [v2.18.1](https://github.com/medusajs/medusa/releases/tag/v2.18.1).

</Note>

When creating a fulfillment through the [Create Fulfillment API route](!api!/admin/orders/create-fulfillment), you can pass an `additional_data` field in the request body. Unlike the `data` field, `additional_data` is not persisted on the fulfillment record — it is only forwarded as-is to the fulfillment provider's `createFulfillment` method.
...
In the fulfillment provider's `createFulfillment` method, the fifth argument receives this object:
...
By default, when creating a fulfillment for an order, Medusa uses the order's shipping address as the delivery address sent to the fulfillment provider.
...
The merged address is forwarded to the fulfillment provider but the order's shipping address is not mutated.
