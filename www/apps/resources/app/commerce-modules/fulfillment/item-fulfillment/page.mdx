...
**Important:** Tracking information is not stored directly on the Fulfillment object. Properties like `tracking_links` or `tracking_urls` do not exist on the Fulfillment model. Instead, tracking information is stored in the related FulfillmentLabel models accessed via `fulfillment.labels`.

</Note>

### Accessing Tracking Information

To access tracking information for a fulfillment, you must retrieve the related labels:
...
// ✅ Correct - Access through labels relation
...
When working with fulfillments in workflows, make sure to retrieve the labels relation:
...
// Access tracking information through labels
const trackingUrls = fulfillment.labels.map(label => label.tracking_url)
```
