...
Many of the service factory's generated methods allow passing filters to perform an operation, such as update or delete records matching the filters.
...
To filter a record's property by an exact value, pass it as the property's value.
...
To find records with a property that doesn't match one or more values, pass an object with a `$nin` property. Its value is an array of multiple values that a recorder's property shouldn't match.
...
To perform a `like` filter on a record's property, pass an object with a `$like` property. Its value is the string to use when applying the `like` filter.
...
To filter a record's property to be within a range, pass an object with any of the following properties:
...
To use an `or` condition, use the `$or` property, whose value is an array of filters.
