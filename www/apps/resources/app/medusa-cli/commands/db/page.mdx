...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      Skip prompts when migrating search indexes and execute only safe actions. Drop actions are skipped.
      ...
      Prompts are shown for drop actions, by default.
    ...
    <Table.Row>
      ...
      Skip prompts when migrating search indexes and execute all actions, including drops.
...
Create and alter the search indexes declared under `search/`. Indexes that no definition declares any more are dropped after a confirmation prompt.
...
`db:migrate` runs this command automatically when the Search Module is enabled. Use `db:migrate:search` directly when you want to manage search indexes independently.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      Skip prompts and execute only safe actions. Drop actions are skipped entirely.
    ...
    <Table.Row>
      ...
      Skip prompts and execute all actions, including drops. Dropping an index permanently deletes all documents it holds.
