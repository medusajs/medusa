...
When deploying the admin as a standalone application on a separate domain, it will be served at the path set in `admin.path` (default: `/app`). To serve it at the root instead, set `admin.path` to `/` in `medusa-config.ts`:
