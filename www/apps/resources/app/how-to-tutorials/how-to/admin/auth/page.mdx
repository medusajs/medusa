...
- The user's auth identity was authenticated by the custom Authentication Module Provider. You use [Query](!docs!/learn/fundamentals/query) to retrieve the user's auth identity with its provider identities, then check whether the custom provider is one of the providers. If not, you throw an unauthorized error.
