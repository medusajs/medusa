...
If your organization uses an identity provider that implements OpenID Connect, such as Okta or Microsoft Entra ID, use the [OIDC Auth Module Provider](../../../../enterprise/sso/page.mdx) instead. It handles the authentication, and the Medusa Admin dashboard renders its login button and creates the admin user for you. It's an [enterprise feature](../../../../enterprise/page.mdx), so it requires a license key.

Follow this guide if you integrate a provider that doesn't implement OpenID Connect, or if you want to build and own the integration yourself.
...
Admin users, which are of the `user` [actor type](../../../../commerce-modules/auth/auth-identity-and-actor-types/page.mdx), can only authenticate with the `emailpass` provider by default. So, you must allow your custom provider for admin users before they can use it.

Set the `http.authMethodsPerActor` configuration in `medusa-config.ts` to enable the custom authentication provider for the `user` actor type. It's also recommended to enable it only for that actor type, since providers are otherwise available to every other actor type, such as customers:
...
Before [Medusa v2.21.1](https://github.com/medusajs/medusa/releases/tag/v2.21.1), admin users could authenticate with every registered provider by default, so this configuration was recommended rather than required.
