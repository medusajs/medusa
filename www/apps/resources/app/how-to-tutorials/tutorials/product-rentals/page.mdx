...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Rental Module, run the following command in the `apps/backend` directory:
...
export const upsertRentalConfigWorkflow = createWorkflow(
  ...
  (input: UpsertRentalConfigWorkflowInput) => {
    ...
    const createdConfig = when(
      "create-rental-configuration",
      { products },
      (data) => !data.products[0]?.rental_configuration
    ).then(() => {
...
1. Retrieve the product's details with its rental configuration using the `useQueryGraphStep`. This step uses [Query](!docs!/learn/fundamentals/query) under the hood, which retrieves data across modules.
...
You expose a `GET` API route at `/admin/products/:id/rental-config`. In the route handler, you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container and use it to query the rental configuration for the product.
...
Run the following command in the `apps/backend` directory to start the Medusa server:
...
```ts title="src/api/store/products/[id]/rental-availability/route.ts" collapsibleLines="1-10" expandButtonLabel="Show Imports"
...
### b. Allow Retrieving Rental Configuration

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, before the storefront can retrieve the rental configuration, you must add `rental_configuration` to the allowed fields of the `/store/products` route.
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("rental_configuration")],
    },
  ],
})
```
...
### c. Fetch Rental Configuration with Product Details
...
### d. Add Rental Availability Function
...
### e. Create Rental Date Picker Component
...
### f. Customize Product Price Component
...
### g. Show Rental Options on Product Details Page
...
### h. Test Rental Options in Storefront
...
export const addToCartWithRentalWorkflow = createWorkflow(
  ...
  (input: AddToCartWorkflowInput) => {
    ...
    const rentalConfiguration = transform(
      { variants },
      ({ variants }) => variants[0].product?.rental_configuration || null
    )
