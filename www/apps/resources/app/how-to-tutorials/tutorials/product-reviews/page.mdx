...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Review Module, run the following command in the `apps/backend` directory:
...
`useQueryGraphStep` uses [Query](!docs!/learn/fundamentals/query), which allows you to retrieve data across modules. For example, in the above snippet you're retrieving the product, which is managed in the [Product Module](../../../commerce-modules/product/page.mdx), by passing `id` to the `fields` array.
...
export const GetAdminReviewsHighlights = [
  ["8", "GetAdminReviewsSchema", "Define the query parameters schema."],
  ["10", "GET", "Expose a GET API route."],
  ["23", "query.graph", "Retrieve reviews with pagination."],
]
...
Then, you export a `GET` function, which exposes a `GET` API Route at the path `/admin/reviews`. In the route handler you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container, which allows you to retrieve data across modules.
...
{/* eslint-disable @medusajs/ui-route-config-via-define-route-config */}
...
Then, you export a `GET` function, and that exposes a `GET` API Route at the path `/store/products/[id]/reviews`. In the route handler you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container, which allows you to retrieve data across modules.
