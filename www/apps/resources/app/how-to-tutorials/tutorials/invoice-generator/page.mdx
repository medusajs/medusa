...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Invoice Generator Module, run the following command in the `apps/backend` directory:
...
export const retrieveInvoiceConfigHighlights = [
  ["6", "GET", "Expose a `GET` API route at `/admin/invoice-config`."],
  ["10", "query", "Resolve Query from the Medusa container."],
  ["14", "invoiceConfig", "Retrieve the default invoice configuration."],
  ["19", "json", "Return the invoice configuration in the response."]
]
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/query) from the Medusa container. It allows you to retrieve data across modules.
...
6. Define an `uploadLogo` function that uploads a logo using Medusa's [Upload API route](!api!/admin/uploads/upload-files).
...
Refer to the [Admin Components](../../../admin-components/guides/forms/page.mdx) documentation to learn more about creating forms in the Medusa Admin dashboard.
...
export const orderPlacedHighlights = [
  ["8", "orderPlacedHandler", "Subscriber function that handles the `order.placed` event."],
  ["21", "order", "Retrieve the order details using Query."],
  ["48", "generateInvoicePdfWorkflow", "Generate the invoice PDF using the workflow."],
  ["58", "binaryString", "Convert the PDF buffer to a binary string for email attachment."],
  ["62", "createNotifications", "Send the email with the invoice attached."]
]
