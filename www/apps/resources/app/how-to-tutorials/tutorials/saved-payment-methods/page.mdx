...
To test it out, start the Medusa application by running the following command in the `apps/backend` directory:
...
## Step 6: Delete Saved Payment Methods

<Note>

This step is available starting from Medusa [v2.13.6](https://github.com/medusajs/medusa/releases/tag/v2.13.6).

</Note>

You can also allow customers to delete their saved payment methods. The Payment Module provides a `deletePaymentMethods` method that removes saved payment methods from third-party providers like Stripe.

### Create Delete API Route

First, create an API route to delete a saved payment method. Create the file `src/api/store/payment-methods/[account_holder_id]/[payment_method_id]/route.ts`:

```ts title="src/api/store/payment-methods/[account_holder_id]/[payment_method_id]/route.ts"
import { MedusaError } from "@medusajs/framework/utils"
import { MedusaRequest, MedusaResponse } from "@medusajs/framework/http"
...
export async function DELETE(
  ...
  const { account_holder_id, payment_method_id } = req.params
  const query = req.scope.resolve("query")
  const paymentModuleService = req.scope.resolve("payment")
...
This API route accepts the account holder ID and payment method ID as path parameters, then uses the Payment Module's `deletePaymentMethods` method to remove the payment method from the third-party provider.

### Protect the Delete API Route

Add the delete route to your middleware protection in `src/api/middlewares.ts`:
...
export default defineMiddlewares({
  routes: [
    ...
    {
      matcher: "/store/payment-methods/:account_holder_id/:payment_method_id",
      method: "DELETE",
      middlewares: [
        authenticate("customer", ["bearer", "session"]),
      ],
...
### Add Delete Functionality to Storefront

In `src/lib/data/payment.ts`, add a function to delete saved payment methods:

```ts title="src/lib/data/payment.ts" badgeLabel="Storefront" badgeColor="blue"
export const deleteSavedPaymentMethod = async (
  ...
  return sdk.client.fetch<{ success: boolean }>(
    `/store/payment-methods/${accountHolderId}/${paymentMethodId}`,
    {
      method: "DELETE",
      headers,
    }
...
Then, update the `StripeSavedPaymentMethodsContainer` component in `src/modules/checkout/components/payment-container/index.tsx` to include delete functionality:

```tsx title="src/modules/checkout/components/payment-container/index.tsx" badgeLabel="Storefront" badgeColor="blue"
// Add this import
import { deleteSavedPaymentMethod } from "@lib/data/payment"

// Add this function inside StripeSavedPaymentMethodsContainer
const handleDelete = async (method: SavedPaymentMethod) => {
  ...
  try {
    await deleteSavedPaymentMethod(accountHolderId, method.id)
    // Remove the deleted method from the list
    setSavedPaymentMethods((prev) => 
      prev.filter((pm) => pm.id !== method.id)
    )
...
// Update the return statement to include delete button
return (
  <div className="flex flex-col gap-y-2">
    <Text className="txt-medium-plus text-ui-fg-base">
      Choose a saved payment method:
    ...
    {savedPaymentMethods.map((method) => (
      ...
      >
        <div 
          className="flex items-center gap-x-4 cursor-pointer flex-1"
          role="button"
          onClick={() => handleSelect(method)}
        >
          <input
...
This adds a delete button to each saved payment method that allows customers to remove methods they no longer want to use.
