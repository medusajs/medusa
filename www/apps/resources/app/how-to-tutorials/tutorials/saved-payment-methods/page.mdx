...
export const apiRouteHighlights = [
  ...
  ["16", "paymentModuleService", "Resolve the Payment Module's service from the Medusa container."],
  ["20", "accountHolder", "Retrieve the account holder by its ID."],
  ["38", "paymentMethods", "Retrieve the saved payment methods in the account holder's provider."]
]
...
- [Query](!docs!/learn/fundamentals/query), which is a tool that retrieves data across modules in the Medusa application.
...
During checkout, when the customer chooses a payment method, such as Stripe, the Next.js Starter Storefront creates a [payment session](../../../commerce-modules/payment/payment-session/page.mdx) in Medusa using the [Initialize Payment Session](!api!/store/payment-collections/initialize-payment-session) API route.
