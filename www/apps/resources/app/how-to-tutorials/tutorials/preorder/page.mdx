...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Preorder Module, run the following command in the `apps/backend` directory:
...
export const upsertProductVariantPreorderWorkflowHighlights = [
  ...
  ["42", "updatePreorderVariantStep", "Update the existing pre-order variant."],
  ["52", "when", "Check if there isn't an existing pre-order variant."],
  ["57", "createPreorderVariantStep", "Create a pre-order variant for the product variant."],
  ["59", "createRemoteLinkStep", "Link the pre-order variant to the product variant."],
  ["73", "transform", "Return either the created or updated pre-order variant."],
  ["80", "preorderVariant", "Return the pre-order variant to the workflow's executor."],
]
...
1. Try to retrieve the variant's details to confirm it exists. The [useQueryGraphStep](/references/helper-steps/useQueryGraphStep) uses [Query](!docs!/learn/fundamentals/query) which allows you to retrieve data across modules.
...
- `preorderVariant`: The pre-order variant. It's retrieved from Medusa's [Retrieve Variant](!api!/admin/products/get-variant) API route, which allows expanding linked records using the `fields` query parameter.
...
export const validateCartHookHighlights = [
  ["9", "isPreorderVariant", "Check if a pre-order variant is enabled and in the future."],
  ["19", "validate", "Consume the `validate` hook of the `addToCartWorkflow`."],
  ["25", "itemsInCart", "Retrieve all items in the cart."],
  ["37", "variantsToAdd", "Retrieve the pre-order variants of the new items being added to the cart."],
  ["47", "cartHasPreorderVariants", "Check if the cart has pre-order variants."],
  ["53", "newItemsHavePreorderVariants", "Check if the new items being added have pre-order variants."],
  ["60", "MedusaError", "Throw an error if the cart has mixed pre-order and available items."],
]
...
You'll also allow the storefront to retrieve pre-order variants from the Store API routes it uses.
...
### a. Allow Retrieving Pre-order Variants

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, before the storefront can retrieve pre-order variants, you must add the fields of the pre-order variant link to the allowed fields of the routes that the storefront uses.

You do that with middlewares that push the fields to the request's `allowed` array.

In `src/api/middlewares.ts`, add the following route objects to the array passed to `defineMiddlewares`:
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("variants.preorder_variant")],
    },
    {
      matcher: "/store/carts",
      middlewares: [
...
The storefront can now request the pre-order variant when it retrieves products, carts, and orders. The other fields that the storefront requests, such as `+items.total` and `*region`, are already allowed by default.

<Note title="Important">

The middlewares must be global, so don't set a `method` property on the route objects. Medusa runs a method-scoped middleware after it validates the `fields` query parameter, which is too late to change the allowed fields.

</Note>

Learn more about this behavior in the [Allowed Fields](!docs!/learn/fundamentals/api-routes/allowed-fields) documentation.

### b. Add Pre-order Types
...
### d. Show Pre-order Information in Product Page
...
### e. Show Pre-order Information in Cart Page
...
### f. Use Custom Complete Pre-order Cart API Route
...
### g. Show Pre-order Information in Order Confirmation Page
...
export const fulfillPreorderWorkflow = createWorkflow(
  ...
  (input: WorkflowInput) => {
    ...
    const paymentCollection = transform(
      { orders },
      ({ orders }) => orders[0].payment_collections?.[0]
    )

    const paymentId = transform(
      { orders },
      ({ orders }) => orders[0].payment_collections?.[0]?.payments[0].id
    )
...
export const fulfillPreordersJobHighlights = [
  ["11", "fulfillPreordersJob", "The scheduled job that fulfills pre-orders."],
  ["26", "schedule", "The cron schedule for the job."]
]
...
export const preorderNotificationHighlights = [
  ["10", "productCreateHandler", "The function that's executed when the event is emitted."],
  ["24", "preorderVariants", "Retrieve the pre-order variant details."],
  ["32", "order", "Retrieve the order details."],
  ["40", "notificationModuleService.createNotifications", "Send a notification to the customer."],
  ["42", "channel", "The channel to send the notification with."],
  ["42", `"feed"`, "The feed channel is useful for debugging."],
  ["52", "event", "The event that the subscriber is listening to."],
]
...
export const orderCanceledHighlights = [
  ["19", "workflowInput", "Prepare the input to pass to the workflow."],
  ["29", "preorders", "Retrieve the pre-orders of the order."],
  ["46", "push", "Add the pre-orders to the workflow input."],
  ["51", "cancelPreordersWorkflow", "Execute the cancel pre-orders workflow."],
]
...
export const handleOrderEditWorkflowHighlights = [
  ...
  ["29", "useQueryGraphStep", "Retrieve the pre-orders of the order."],
  ["38", "retrievePreorderUpdatesStep", "Retrieve the pre-orders to cancel and create."],
  ["43", "updatePreordersStep", "Update the pre-orders to cancel."],
  ["45", "createPreordersStep", "Create the pre-orders to create."],
  ["57", "emitEventStep", "Emit the `preorder.cancelled` event for canceled pre-orders."],
  ["62", "WorkflowResponse", "Return the created and canceled pre-orders."]
]
