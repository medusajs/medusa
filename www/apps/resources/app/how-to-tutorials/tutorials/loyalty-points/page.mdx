...
<Note title="New: Loyalty Plugin">

This guide was written before the release of the [Loyalty Plugin](../../../commerce-modules/loyalty/page.mdx), which adds features like gift cards and store credit. You can follow the approach of this guide, or use the [Store Credit Module](../../../commerce-modules/store-credit/page.mdx) from the Loyalty Plugin to implement a loyalty points system.
...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Loyalty Module, run the following command in the `apps/backend` directory:
...
- Use `useQueryGraphStep` to retrieve the order's details. You pass the order's ID as a filter to retrieve the order.
    - This step uses [Query](!docs!/learn/fundamentals/query), which is a tool that retrieves data across modules.
...
To test out the loyalty points flow, you'll use the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) that you installed in the first step. The storefront is available in the `apps/storefront` directory of your project.

So, run the following command in the `apps/backend` directory to start the Medusa server:
...
export const completeCartWorkflowHookHighlights = [
  ["10", "validate", "Consume the validate hook."],
  ["19", "graph", "Retrieve the cart's details."],
  ["37", "getCartLoyaltyPromotion", "Get the cart's loyalty promotion."],
  ["45", "getPoints", "Get the customer's loyalty points."],
  ["48", "requiredPoints", "Calculate the amount to be discounted based on the loyalty points."],
  ["54", "MedusaError", "Throw an error if the customer doesn't have enough loyalty points."],
]

```ts title="src/workflows/hooks/complete-cart.ts" highlights={completeCartWorkflowHookHighlights} collapsibleLines="1-9" expandButtonLabel="Show Imports"
