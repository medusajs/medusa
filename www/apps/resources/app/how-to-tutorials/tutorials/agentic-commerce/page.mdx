...
After that, the installation process will begin. This will install the Medusa application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory. If you chose to install the Next.js Starter Storefront, it'll be in the `apps/storefront` directory.
...
1. Retrieve products with pagination using [Query](!docs!/learn/fundamentals/query). Query allows you to retrieve data across modules. You retrieve the fields necessary for the product field.
...
- Map refund transactions to the `adjustments` array. Medusa stores refunds as negative amounts, so you use the absolute value.
...
Extract the payload-building logic into an exported function, such as `buildOrderWebhookEvent`, if you want to unit test the status and adjustment mapping without emitting events. You can find an example in the [tutorial's repository](https://github.com/medusajs/examples/tree/main/agentic-commerce).
