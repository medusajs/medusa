...
Medusa's CLI tool can generate migrations for you. To generate a migration for the Product Media Module, run the following command in the `apps/backend` directory:
...
In the API route, you resolve [Query](!docs!/learn/fundamentals/query), which retrieves data across modules.
...
### b. Allow Retrieving Category Images

Every Store API route defines the fields that clients can request in the `fields` query parameter. Medusa strips any other field from the response without throwing an error. So, before the storefront can retrieve category images, you must add `product_category_image` to the allowed fields of the `/store/product-categories` route.

You do that with a middleware that pushes the field to the request's `allowed` array.

In `src/api/middlewares.ts`, add a new route object passed to the array in `defineMiddlewares`:
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("product_category_image")],
    },
  ],
})
```
...
The middleware must be global, so don't set a `method` property on the route object. Medusa runs a method-scoped middleware after it validates the `fields` query parameter, which is too late to change the allowed fields.

</Note>

Learn more about this behavior in the [Allowed Fields](!docs!/learn/fundamentals/api-routes/allowed-fields) documentation.

### c. Retrieve Category Images
...
You add `*product_category_image` to the `fields` query parameter in both functions. The asterisk (`*`) indicates that you want to include all fields in the product category image record. The route allows the other fields, such as `*category_children` and `*parent_category`, by default, so the middleware you added earlier only allows `product_category_image`.

### d. Add Category Image Type
...
### f. Show Banner Image on Category Page
