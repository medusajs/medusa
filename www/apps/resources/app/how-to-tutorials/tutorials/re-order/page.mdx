...
In the workflow's constructor function, so far you use the `useQueryGraphStep` step to retrieve the order's details. This step uses [Query](!docs!/learn/fundamentals/query) under the hood, which allows you to query data across [modules](!docs!/learn/fundamentals/modules).

<Note>

Refer to the [Query documentation](!docs!/learn/fundamentals/query) to learn more about how to use it.
...
First, to start the Medusa application, run the following command in the `apps/backend` directory:
