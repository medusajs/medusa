...
export const validateCallbackHighlights = [
  ["9", "phone", "Extract phone from request query."],
  ["9", "otp", "Extract OTP from request query."],
  ["18", "retrieve", "Check that the user exists."],
  ["39", "verify", "Verify that the OTP is correct and hasn't expired."],
  ["57", "update", "Remove OTP from provider metadata."],
]
...
Next, you'll register the customer using the [Register Customer](!api!/store/customers/register-customer) API route. You'll pass the registration token you received in the previous step in the header of this request.
...
- The customer details in the request body with the ones you want to use. Use the same phone number you used in the previous step.
    - You pass the email because it's required by the [Register Customer](!api!/store/customers/register-customer) API route. You set it to the phone number with a `gmail.com` domain.
...
Next, you'll authenticate the customer using the [Authenticate Customer](!api!/store/auth/authenticate-customer) API route. This would send the customer an OTP to their phone number (which you'll implement in the next step).
...
So, to verify the OTP, you'll send a request to the [Verify Callback API route](!api!/store/auth/validate-authentication-callback):
...
You can use this token to authenticate the customer in the application. For example, you can use the token to [retrieve the customer's details](!api!/store/customers/get-customer).
...
export const sendHighlights = [
  ["13", "to", "Extract the phone number to send the SMS to."],
  ["13", "content", "Extract the content of the SMS."],
  ["13", "template", "Extract the template to use for the SMS."],
  ["13", "data", "Extract the data to use in the template."],
  ["14", "contentText", "Set the content text to either the text or template content."],
  ["14", "getTemplateContent", "Get the content for the template."],
  ["18", "message", "Send the SMS using Twilio's client."],
  ["29", "getTemplateContent", "A method to get the content for the template."],
]
    ...
    switch (template) {
      case "otp-template":
        if (!data?.otp) {
          throw new MedusaError(
            MedusaError.Types.INVALID_DATA,
            "OTP is required for OTP template"
          )
        }
...
Then, after setting the registration token in the cookies, you create a customer using the [Create Customer](!api!/store/customers/register-customer) API route. You pass the following properties in the request body:
