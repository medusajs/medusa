...
1. An object indicating the first data model in the link. You pass the link configurations for the `Tier` data model from the Tier Module. You also specify the `id` property as filterable, allowing you to filter customers by their tier later using the [Index Module](!docs!/learn/fundamentals/query/index-module).
...
- Use [when-then](!docs!/learn/fundamentals/workflows/conditions) to check whether the promotion ID is provided and retrieve the promotion to validate that it exists.
    ...
    - This step uses [Query](!docs!/learn/fundamentals/query) under the hood to retrieve data across modules.
...
Notice that you use the `query.index` method. This method is similar to `query.graph` but allows you to filter by linked records using the [Index Module](!docs!/learn/fundamentals/query/index-module).
...
The [Index Module](!docs!/learn/fundamentals/query/index-module) is a tool for performing high-performance queries across modules, such as filtering linked modules.
...
Refer to the [Index Module](!docs!/learn/fundamentals/query/index-module) documentation to learn more.
...
When a customer adds a promotion to the cart, the storefront sends a request to the [Add Promotions API Route](!api!/store/carts/add-promotions), which executes the [updateCartPromotionsWorkflow](/references/medusa-workflows/updateCartPromotionsWorkflow).
