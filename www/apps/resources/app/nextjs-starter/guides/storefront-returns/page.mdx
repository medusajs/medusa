...
The first function sends a request to the [List Return Reasons](!api!/store/return-reasons/list-return-reasons) API route. It fetches the available return reasons, which customers can select from when creating a return request.
...
Next, you'll add a function that sends a request to the [List Shipping Options](!api!/store/shipping-options/list-shipping-options-for-cart) API route. It fetches the available shipping options for returns, which customers can select from when creating a return request.
...
### Allow the Order's Cart Field

Medusa restricts the fields that you can request from a Store API route through an allowed-fields list. The order routes don't allow the `cart.id` field, so Medusa omits it from the response unless you allow it.

In the Medusa application, create the file `src/api/middlewares.ts` with the following content:
  ...
  routes: [
    {
      ...
      middlewares: [allowFields("cart.id")],
    },
  ],
})
```
...
Since you need the cart ID to retrieve data like return shipping options, you need to retrieve the cart along with the order data. You can do this by adding `+cart.id` to the `fields` query parameter when retrieving the order. This works only after you add the middleware from the previous section.
...
Refer to the [Create Return API reference](!api!/api/store/returns/create-return) to see the available parameters when creating a return request.
