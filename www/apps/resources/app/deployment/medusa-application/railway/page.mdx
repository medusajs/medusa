...
Learn more about the `worker_mode` configuration in [this document](/references/medusa-config#workermode).
...
Later, you’ll set different values of the `MEDUSA_WORKER_MODE` environment variable for each Medusa application.
...
Once you’re done, click the Update Variables button.
...
### Changes if Deploying with Admin
