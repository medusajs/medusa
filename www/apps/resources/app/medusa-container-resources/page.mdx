...
Use the `ContainerRegistrationKeys` enum imported from `@medusajs/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      - For Medusa's commerce modules, use the `Modules` enum imported from `@medusajs/utils`.
