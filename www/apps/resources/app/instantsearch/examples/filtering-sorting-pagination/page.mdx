...
- InstantSearch keeps the previous response in `results` while the next search runs, and `results.query` is the query that response was fetched for. Gate rendering on `results.query`, never on the query last sent. Otherwise the first real query renders the empty query's hits for a beat, which is the entire catalogue flashing on screen.
...
- Never render hits from a response fetched for a different query than the current one. Compare `results.query` to the current query.
...
- If the full list flashes before a query's results arrive, the render is gated on the query last sent rather than on `results.query`. Fix the gate; don't hide it with a loading state.
