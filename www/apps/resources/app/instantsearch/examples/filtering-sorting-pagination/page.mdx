import { Prerequisites, Note, Table, TypeList } from "docs-ui"
...
The product indexes in this guide's snippets are simplified to show one feature at a time. For examples of indexing prices in multiple currencies, option values, categories, and other product data, refer to the [Product Index Examples guide](../../../infrastructure-modules/search/product-index-examples/page.mdx).
...
- The adapter is a search client, not UI components, and the supported widgets each need configuration on it:
  ...
  - A product index often holds a price field per currency, such as `min_price_usd` and `min_price_eur`. Build each price widget's attribute from the region's currency code, and list every currency's range fields in `numericAttributes`. An `on_sale_{currency}` boolean, when the index holds one, is a toggle refinement rather than a range.
  ...
  types={[
    {
      ...
      type: "`string`",
      optional: false,
      description: "The `name` of the index the page lists from, which the adapter sends as the query's `entity`. For the product index that Medusa provides, use `product`."
    },
    {
      name: "searchClient",
      type: "`object`",
      optional: false,
      description: "The client you created in the [InstantSearch Adapter guide](../../page.mdx). Every surface imports the same one if they share the same options."
    },
    {
      name: "routing",
      type: "`boolean` \\| `object`",
      optional: true,
      description: "Writes the search state to the URL's query parameters and restores it on load, so a customer can share a filtered list, and the browser's back button undoes one refinement at a time."
    },
    {
      name: "Configure",
      type: "`React.Component`",
      optional: true,
      description: "Sets search parameters that the customer doesn't control, such as the page size."
    }
  ]}
  sectionTitle="Set Up the Search Context"
/>
  ...
  types={[
    {
      ...
      type: "`string`",
      optional: false,
      description: "The facetable field to filter by."
    },
    {
      name: "limit",
      type: "`number`",
      optional: true,
      description: "How many values to request."
    },
    {
      name: "sortBy",
      type: "`string[]`",
      optional: true,
      description: "How to order the values. `[\"count:desc\", \"name:asc\"]` shows the most common values first, with ties broken alphabetically."
    }
  ]}
  sectionTitle="Filter by a Field's Values"
/>
      ...
      children: [
        {
          ...
          type: "`string`",
          optional: false,
          description: "The value's human-readable label."
        },
        {
          name: "value",
          type: "`string`",
          optional: false,
          description: "The value to pass to `refine`."
        },
        {
          name: "count",
          type: "`number`",
          optional: false,
          description: "How many of the current results carry the value."
        },
        {
          name: "isRefined",
          type: "`boolean`",
          optional: false,
          description: "Whether the customer has already ticked the value."
        }
      ]
    },
    {
      ...
      optional: false,
      description: "A function that toggles a value in the search query's filters."
    }
  ]}
  sectionTitle="Filter by a Field's Values"
/>
...
export const statsFacetHighlights = [
  ["13", "facetable", "Report the field's range as a stats facet."],
]

```ts title="src/search/product.ts" highlights={statsFacetHighlights} badgeLabel="Medusa Application" badgeColor="green"
import {
  defineSearchIndex,
  search,
...
```ts title="src/lib/search-client.ts" badgeLabel="Storefront" badgeColor="blue"
...
```tsx title="src/components/filters.tsx" badgeLabel="Storefront" badgeColor="blue"
...
A search engine can't calculate a price at query time, so a product index holds a set of price fields per currency, such as `min_price_usd` and `min_price_eur`. Refer to the [Product Search Index Examples guide](../../../infrastructure-modules/search/product-index-examples/page.mdx#index-prices-for-multiple-currencies) for the index side of that.
...
export const priceAttributeHighlights = [
  ["1", "SEARCH_PRICE_CURRENCIES", "Keep in sync with the index definition."],
  ["12", "indexedCurrency", "Fall back to the first currency the index holds."],
  ["20", "priceAttribute", "Build a price field's name for a currency."],
  ["28", "numericAttributes", "Every currency's range fields, not one of them."],
]

```ts title="src/lib/search-client.ts" highlights={priceAttributeHighlights} badgeLabel="Storefront" badgeColor="blue"
...
// A region whose currency the index doesn't hold
// falls back to the first one, so the listing still
...
export const { searchClient } = createInstantSearchAdapter({
  sdk,
  path: "/store/search",
  numericAttributes: SEARCH_PRICE_CURRENCIES.flatMap(
    (currency) => [
      `min_price_${currency}`,
      `max_price_${currency}`,
    ]
...
For example, the following component renders a two-thumb slider, such as [Radix's](https://www.radix-ui.com/primitives/docs/components/slider), over the price field of the region's currency:

export const rangeHighlights = [
  ["13", "attribute", "The price field of the region's currency."],
  ["16", "range", "The field's bounds, from the stats facet."],
  ["18", "start", "Unrefined bounds are `-Infinity` and `Infinity`."],
  ["27", "useEffect", "Follow a refinement that changed elsewhere."],
  ["32", "canRefine", "Render nothing until the bounds differ."],
  ["42", "onValueChange", "Move the thumbs without searching."],
  ["43", "onValueCommit", "Refine on release, not on every pixel."],
]
...
const PriceSlider = ({
  ...
  const from = Number.isFinite(start[0])
    ? (start[0] as number)
    : min
  const to = Number.isFinite(start[1])
    ? (start[1] as number)
    : max
  ...
  // No stats yet, or every product costs the same.
  ...
  return (
    <Slider.Root
      ...
      onValueCommit={(committed) =>
        refine([committed[0], committed[1]])
      }
  ...
  types={[
    {
      ...
      type: "`object`",
      optional: false,
      description: "The field's lowest and highest values, as reported by the stats facet.",
      expandable: false,
      children: [
        {
          name: "min",
          type: "`number`",
          optional: true,
          description: "The field's lowest value."
        },
        {
          name: "max",
          type: "`number`",
          optional: true,
          description: "The field's highest value."
        }
      ]
    },
    {
      ...
      type: "`[number, number]`",
      optional: false,
      description: "The current selection, as a `[min, max]` tuple. An unset bound is `-Infinity` or `Infinity`, so check it with `Number.isFinite` before rendering it."
    },
    {
      name: "refine",
      type: "`(range: [number, number]) => void`",
      optional: false,
      description: "A function that sets a new `[min, max]` range. Pass `undefined` for a bound you want to leave open."
    },
    {
      name: "canRefine",
      type: "`boolean`",
      optional: false,
      description: "`false` when the lowest and highest values are equal, which includes the empty bounds before the first response arrives."
    }
  ]}
  sectionTitle="Build a Price Slider with useRange"
/>
...
An index that holds an `on_sale` field per currency, as the [Product Search Index Examples guide](../../../infrastructure-modules/search/product-index-examples/page.mdx#index-prices-for-multiple-currencies) shows, can filter down to the discounted products with a single checkbox.

Use the `useToggleRefinement` hook, which filters on one value of a field and reports how many documents carry it:

export const onSaleHighlights = [
  ["11", "attribute", "The `on_sale` field of the region's currency."],
  ["12", "on", "The value to filter on when the box is ticked."],
  ["15", "canRefine", "Hide the toggle when nothing is discounted."],
  ["27", "value.count", "The number of products the toggle would leave."],
]

```tsx title="src/components/on-sale-toggle.tsx" highlights={onSaleHighlights}
...
export const sortHighlights = [
  ...
  ["8", "/sort/created_at:desc", "The suffix the adapter turns into `pagination.order`."],
]
...
```tsx title="src/components/sort-products.tsx"
import {
  PRODUCT_INDEX_NAME,
  priceAttribute,
} from "../lib/search-client"

const getSortOptions = (currencyCode: string) => {
  const minPrice = priceAttribute(
    "min_price",
    currencyCode
  )
...
- `estimated` (default): the engine approximates the total. The last page number may shift as the customer moves through the list.
