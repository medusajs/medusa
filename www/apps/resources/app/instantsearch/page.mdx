...
7. Make the search feel responsive: don't send a request per keystroke, and don't let the results flicker or empty while the next search runs.
8. Give every state its own treatment: no query yet, search in flight, results, no results, and a failed search. An empty list must never stand in for any of the others.
9. Place the finished component where the user asked for it.
10. Report what you changed and how to test it.
</steps>
...
## How to Test
Numbered steps that end in an observable result, including one that checks the results don't flicker while typing.
...
- Each of the five states in step 8 shows something distinct from the others.
