...
<Prerequisites
  items={[
    ...
    {
      text: "`product` search index defined and allowed on the `/store/search` API route",
      link: "/infrastructure-modules/search#search-products"
    },
    {
      text: "Medusa v2.21.1+",
...
Filters you set in the storefront's client are visible to customers, who can change them, so use these options for presentation, such as choosing the fields to retrieve, and not to protect data. Scoping a search to what a customer may see, such as a sales channel or published products, is a middleware on the `/store/search` route in your Medusa application.
...
The `/store/search` route searches any index that a middleware allowed in your Medusa application, so an index you defined yourself needs no route of its own. Add its name to the `allowSearchIndexes` middleware:
  ...
  routes: [
    {
      ...
      middlewares: [
        allowSearchIndexes("product", "article"),
      ],
    },
  ],
})
...
Refer to the [Search Module](../infrastructure-modules/search/page.mdx) documentation to learn how to define an index.
