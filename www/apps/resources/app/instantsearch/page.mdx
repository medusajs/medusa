...
- Medusa exposes a `/store/products/search` API route with a default `product` index. The route scopes every query to the storefront's context, so you don't create a route for product search.
- `@medusajs/instantsearch-adapter` provides a search client, not UI components. It converts InstantSearch requests into Medusa search queries, sends them to `/store/products/search`, and converts the results back.
...
3. Create the search client in a shared module, such as `src/lib/search-client.ts`. Pass the existing JS SDK instance if there is one; otherwise create one from the storefront's backend URL and publishable API key environment variables. Set `path` to `/store/products/search`, and export the index name as a constant so the UI never repeats the string.
...
- Never modify the Medusa application's `/store/products/search` route.
- Create a custom API route only when searching an index other than the `product` index that `/store/products/search` exposes. In that route, build the hits from the `data` that `query.search` returns.
</constraints>
...
The `@medusajs/instantsearch-adapter` package provides a search client for Medusa. It translates each InstantSearch request into a Medusa-compatible search query, sends it to Medusa's `/store/products/search` API route, then maps the result back into the shape the widgets expect.
...
export const clientHighlights = [
  ["6", "PRODUCT_INDEX_NAME", "The name of the index to search."],
  ["9", "sdk", "The storefront's existing SDK instance."],
  ["10", "path", "Medusa's storefront search route."],
]
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        The path of the search route, or an absolute URL. Use `/store/products/search` unless you added a custom route. Required unless you pass `requester`.
      </Table.Cell>
      <Table.Cell>
        \-
      </Table.Cell>
...
Filters you set in the storefront's client are visible to customers, who can change them. The `/store/products/search` route already scopes queries to the storefront's context, so use these options for presentation, such as choosing the fields to retrieve, and not to protect data.
...
The `/store/products/search` route exposes the `product` index only, scoped to what's safe for customers. To search an index you defined yourself, add an API route for it and point the adapter's `path` option to it.
