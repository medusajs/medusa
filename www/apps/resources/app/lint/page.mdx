...
The Medusa ESLint plugin (`@medusajs/eslint-plugin`) is the official ESLint plugin for Medusa projects. It codifies Medusa's framework conventions into static checks that run while you write code in your editor and when you build or develop your application.
...
- `--lint`: Whether to run the linter before starting the development server. Accepts `--lint true` (default) or `--lint false`.
- `--fix`: Auto-fix lint issues where possible before starting.

For example, to start the development server without linting:

```bash
npx medusa develop --lint false
```
...
- `--lint`: Whether to run the linter before building. Accepts `--lint true` (default) or `--lint false`.
- `--fix`: Auto-fix lint issues where possible before building.
- `--quiet`: Report lint errors only, suppressing warnings.

For example, to build your project without linting:

```bash
npx medusa build --lint false
```
...
To change a rule's severity or disable it, add a configuration block after the preset in your `eslint.config.ts` file.
