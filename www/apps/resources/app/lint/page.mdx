...
```bash npm2yarn
npm install --save-dev @medusajs/eslint-plugin eslint
```
...
Then, in the Medusa application package, create an `eslint.config.ts` file that extends the root configuration:
...
Since `strict` is type aware, you must also install `jiti`:
