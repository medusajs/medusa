...
The framework only loads middlewares from `src/api/middlewares.ts` with a default export of `defineMiddlewares`. A file with another name or location is ignored, so its middlewares never run.
