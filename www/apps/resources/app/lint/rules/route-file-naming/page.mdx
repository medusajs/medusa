...
The framework only registers HTTP method handlers exported from `route.ts` or `route.js` files. Handlers placed in other files are never reached, so the route won't work as expected.
