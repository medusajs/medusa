...
7. Create a workflow that uses the module's main service in a step in the file `src/workflows/create-post.ts`:

```ts title="src/workflows/create-post.ts"
import {
  createStep,
  createWorkflow,
  StepResponse,
...
const createPostWorkflow = createWorkflow(
  "create-post",
...
8. Execute the workflow in an API route:
...
Learn more in [this documentation](!docs!/learn/fundamentals/query).
...
Learn more in [this documentation](!docs!/learn/fundamentals/query#retrieve-linked-records).
...
Learn more in [this documentation](!docs!/learn/fundamentals/query#apply-filters).
...
Learn more in [this documentation](!docs!/learn/fundamentals/query#sort-records).
...
To get prices of a product variant for a region and currency using [Query](!docs!/learn/fundamentals/query):
...
To get all prices of a product variant using [Query](!docs!/learn/fundamentals/query):
...
To get a variant's prices with taxes using [Query](!docs!/learn/fundamentals/query) and the [Tax Module](../commerce-modules/tax/page.mdx)
