...
const MyCustom = model.define("my_custom", {
  color: model.enum(["black", "white"]).default("black"),
...
const MyCustom = model.define("my_custom", {
  id: model.id().primaryKey(),
  name: model.text().index("IDX_MY_CUSTOM_NAME"),
})
...
```ts
const product = await helloModuleService.retrieveProduct("123", {
  relations: ["orders"],
})
...
const updatedProduct = await helloModuleService.updateProducts({
  id: product.id,
  // other properties...
  orders: [...product.orders.map((order) => order.id), "321"],
})
```
...
const product = await helloModuleService.retrieveProducts("123", {
  relations: ["orders"],
})
```
...
export const step2 = createStep("step-2", async ({ name }: StepInput) => {
...
const myWorkflow = createWorkflow("hello-world", function (input) {
  ...
  const str3 = transform({ str1, str2 }, (data) => `${data.str1}${data.str2}`)
...
const workflow = createWorkflow(
  "workflow",
  function (input: { is_active: boolean }) {
...
const workflow = createWorkflow("hello-world", async (input) => {
...
export const myWorkflow = createWorkflow("my-workflow", function (input) {
  ...
  const productCreatedHook = createHook("productCreated", {
...
const myWorkflow = createWorkflow("my-workflow", (input: WorkflowInput) => {
  const product = createProductStep(input)
...
```ts
const { transaction } = await myLongRunningWorkflow(req.scope).run()
```
...
export default async function myCustomJob(container: MedusaContainer) {
  ...
  console.log(`Time to check products! You have ${count} product(s)`)
}
...
const cart = await cartModuleService.retrieveCart("cart_123", {
  relations: ["items.adjustments", "shipping_methods.adjustments"],
})
...
cart.shipping_methods.forEach((shippingMethod) => {
  const filteredAdjustments = shippingMethod.adjustments?.filter(
    (adjustment) => adjustment.code !== undefined
  ) as unknown as ComputeActionAdjustmentLine[]
  if (filteredAdjustments.length) {
...
// compute actions
const actions = await promotionModuleService.computeActions(["promo_123"], {
...
const remoteLink = container.resolve(ContainerRegistrationKeys)
...
```ts
const invite = await userModuleService.validateInviteToken(inviteToken)
