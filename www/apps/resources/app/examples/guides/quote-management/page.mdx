...
This guide is based on the [B2B starter](https://github.com/medusajs/b2b-starter) explaining how to implement some of its quote management features. You can refer to the B2B starter for other features not covered in this guide.
...
<CardList items={[
  {
    href: "https://github.com/medusajs/examples/tree/main/quotes-management",
...
Afterwards, the installation process will start, which will install the Medusa application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory. If you chose to install the Next.js Starter Storefront, it'll be in the `apps/storefront` directory.
...
Medusa's CLI tool generates the migrations for you. To generate a migration for the Quote Module, run the following command in your `apps/backend` directory:
...
export const CreateQuote = z.strictObject({
...
Learn more about configuring Query for requests in the [Request Query Configurations documentation](!docs!/learn/fundamentals/query#request-query-configurations).
...
Before creating the customer, retrieve a registration token using the [Retrieve Registration JWT Token API route](!api!/store/auth/retrieve-registration-jwt-token):
...
Then, register the customer using the [Create Customer API route](!api!/store/customers/register-customer):
...
The customer is now registered. Lastly, you need to retrieve its authenticated token by sending a request to the [Authenticate Customer API route](!api!/store/auth/authenticate-customer):
...
A cart requires a region ID. You can retrieve a region ID using the [List Regions API route](!api!/store/regions/list-regions):
...
Then, create a cart for the customer using the [Create Cart API route](!api!/store/carts/create-cart):
...
You now need to add a product variant to the cart. You can retrieve a product variant ID using the [List Products API route](!api!/store/products/list-products):
...
Finally, to add the product variant to the cart, use the [Add Item to Cart API route](!api!/store/carts/add-line-item):
...
- Add shipping and billing addresses by [updating the cart](!api!/store/carts/update-a-cart).
- [Choose a shipping method](!api!/store/carts/add-shipping-method) for the cart.
- [Create a payment collection](!api!/store/payment-collections/create-payment-collection) for the cart.
- [Initialize payment session](!api!/store/payment-collections/initialize-payment-session) in the payment collection.
...
Learn more about paginating Query results in the [Query documentation](!docs!/learn/fundamentals/query#apply-pagination).
...
- Refer to the [B2B starter](https://github.com/medusajs/b2b-starter) for more quote-management related features, including how to add or remove items from a quote, and how to allow messages between the customer and the merchant.
