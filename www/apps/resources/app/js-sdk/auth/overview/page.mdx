...
When email verification is required for authentication, the JS SDK provides methods to handle the verification flow:

### Request Verification Token

Use the `auth.verification.request` method to request a new verification token:
...
Use the `auth.verification.confirm` method to verify the user's email with their token:
...
try {
  ...
  if (result.verified) {
    ...
    // User can now authenticate normally
  }
} catch (error) {
...
### Handle Verification-Required Registration

When registering with `returnVerification: true`, the register method can return verification details instead of throwing an error:
...
try {
  ...
  if (typeof result === "object" && result.verification_required) {
    ...
    console.log("Please verify your email to complete registration")
    console.log("Verification expires:", result.verification.expires_at)
  } else {
...
Learn more about implementing email verification in the [email verification guide](../../../commerce-modules/auth/email-verification/page.mdx).
