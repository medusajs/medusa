...
Since the pass calls the same `seed` function, your definition decides what it costs: read the `catchup` property to narrow your query and to yield a `delete` write for a record deleted mid-run. Refer to [Handle the Catch-Up Pass](../index-definitions/page.mdx#handle-the-catch-up-pass) for how to handle it, which the [`graphSeed`](../index-definitions/page.mdx#fill-the-index-from-query-with-graphseed) helper does for you.
