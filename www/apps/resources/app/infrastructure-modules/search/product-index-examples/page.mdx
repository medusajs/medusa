...
export const simpleHighlights = [
  ["20", "name", "The index name that a search query resolves against."],
  ["21", "entity", "The Query entity used to hydrate non-indexed fields."],
  ["24", "status", "Lets the Store Search API route scope the index to published products."],
  ["38", "events", "The events that change the data the index holds."],
  ["43", "consume", "Turn each of those events into document writes."],
  ["44", "seed", "Fill the index with every product."],
]
...
The index holds draft products too, but the [Store Search API route](../store-search/page.mdx) returns published products only, as long as the index declares a `status` field with the [`filterable` modifier](../index-definitions/modifiers/page.mdx#filterable). The field uses [`retrievable(false)`](../index-definitions/modifiers/page.mdx#retrievable), so the route filters on it without returning it in a hit. Unpublishing a product then drops it from the storefront's results as soon as its `product.updated` event arrives.
...
export const scopeHighlights = [
  ...
  ["22", "status", "Read by `/store/search`, never returned in a hit."],
  ["26", "sales_channel_ids", "Read by `/store/search`, never returned in a hit."],
]
...
export const completeHighlights = [
  ["14", "status", "Lets the Store Search API route scope the index to published products."],
  ["69", "transform", "Reshape each product into the document to index."],
  ["106", "settings", "Engine settings, supported by Medusa Search only."],
  ["124", "consume", "Turn each event into document writes."],
]
