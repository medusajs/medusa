...
So with the definition above, your `seed` and `consume` functions pass the text on `title_embedding` rather than an embedding:
...
A query then searches the field with plain text:
...
Medusa Search decides which embedding model produces the vector, so you don't choose one in the definition. Refer to [Semantic Search with Medusa Search](!cloud!/search#semantic-search-with-medusa-search) for more details.
