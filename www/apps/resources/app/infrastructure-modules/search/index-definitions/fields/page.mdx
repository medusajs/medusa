...
No provider that Medusa ships supports geo fields. Each of them rejects an index that declares one, so your application fails to start. Refer to [Compare Search Module Providers](../../providers/comparison/page.mdx#index-definition-support) for what each provider does with a `geo` field.
...
In this example, `title_embedding` is a `vector` field with 1536 dimensions, matching the output of OpenAI's `text-embedding-3-small` model.
