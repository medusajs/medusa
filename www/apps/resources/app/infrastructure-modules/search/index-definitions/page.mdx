...
export const transformAsyncHighlights = [
  ["11", "async", "The function can await other reads."],
  ["12", "container", "Resolve Query from the context's container."],
  ["15", "query.graph", "One extra read for the whole page."],
]

```ts title="src/search/product.ts" highlights={transformAsyncHighlights}
export const productIndex = defineSearchIndex({
  name: "product",
  entity: "product",
  fields: search.define({
...
Some fields only resolve when `query.graph` receives a [query context](!docs!/learn/fundamentals/query/query-context), such as `variants.calculated_price`, which needs a currency or a region to calculate a price for. Pass that context to `graphSeed` with the `context` option:
...
<TypeList
  types={[
    ...
    },
    {
      ...
      type: "`function`",
      optional: true,
      description: "Maps a page of records to the documents to index. Receives the records and the same context `seed` receives, and can be asynchronous. Every document must carry the record's primary key value as its `id`. Leave a record out of the returned array to keep it out of the index.",
      defaultValue: "Indexes the records as-is"
    },
    {
      name: "context",
      type: "`object` \\| `function`",
      optional: true,
      description: "The [query context](!docs!/learn/fundamentals/query/query-context) that `query.graph` reads the records with, such as the pricing context that `variants.calculated_price` needs. Pass a function to build it from the context `seed` receives. Applied to the full run, the catch-up pass, and `consume` alike."
    },
    {
      name: "batch_size",
      type: "`number`",
...
<TypeList
  types={[
    ...
    },
    {
      ...
      type: "`function`",
      optional: true,
      description: "Maps a page of records to the documents to index. Receives the records and the same context `consume` receives, and can be asynchronous. Every document must carry the record's primary key value as its `id`. Leave a record out of the returned array to delete its document instead.",
      defaultValue: "Indexes the records as-is"
    },
    {
      name: "context",
      type: "`object` \\| `function`",
      optional: true,
      description: "The [query context](!docs!/learn/fundamentals/query/query-context) that `query.graph` reads the records with, such as the pricing context that `variants.calculated_price` needs. Pass a function to build it from the context `consume` receives."
    },
    {
      name: "resolve_ids",
      type: "`function`",
  ...
  types={[
    {
      ...
      type: "`object`",
      optional: false,
      description: "A limited Medusa container exposing `query`, so you can read data with [Query](!docs!/learn/fundamentals/query), and `logger`, so you can log what the run is doing."
    },
    {
      name: "index",
      type: "`object`",
