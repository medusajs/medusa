...
export const highlights = [
  ["14", "notificationModuleService", "Resolve the Notification Module."],
  ["19", "product", "Retrieve the product's details."],
  ["27", "createNotifications", "Create the notification to be sent."],
  [
    "29",
    '"email"',
    "By specifying the `email` channel, SendGrid will be used to send the notification.",
  ],
  ["30", '"product-created"', "The ID of the template defined in SendGrid."],
  ["31", "data", "The data to pass to the template defined in SendGrid."],
]

```ts title="src/subscribers/product-created.ts" highlights={highlights} collapsibleLines="1-10" expandButtonLabel="Show Imports"
import type {
  SubscriberArgs,
  SubscriberConfig,
...
- Resolve the Notification Module's main service and [Query](!docs!/learn/fundamentals/query) from the [Medusa container](!docs!/learn/fundamentals/medusa-container).
...
And create a product either using the [API route](!api!/admin/products/create-product) or the [Medusa Admin](!user-guide!/products/create). This runs the subscriber and sends an email using SendGrid.
