...
You can cache data with the Caching Module in two ways: by caching data retrieved with [Query](!docs!/learn/fundamentals/query) or the [Index Module](!docs!/learn/fundamentals/query/index-module), or by directly using the [Caching Module's service](/references/caching-service).
...
Query and the Index Module accept other caching options. Learn more in the [Query](!docs!/learn/fundamentals/query#cache-query-results) and [Index Module](!docs!/learn/fundamentals/query/index-module#cache-index-module-results) guides.
