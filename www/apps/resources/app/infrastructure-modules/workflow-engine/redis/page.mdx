...
<Note title="Option shapes">

The options can also be passed at the top level of the module options (for example, `redisUrl` directly under `options`), matching the options table below and the other Redis infrastructure modules. If both shapes are provided, the nested `redis` object takes precedence.
