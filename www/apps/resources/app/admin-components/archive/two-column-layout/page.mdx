...
<Note title="Archived" type="warning">

Medusa exports a `LayoutComposer` component from the `@medusajs/dashboard` package since [v2.21.0](https://github.com/medusajs/medusa/releases/tag/v2.21.0). Use it with the `core:two-column` layout instead of building your own layout, as explained in the [LayoutComposer documentation](../../layouts/layout-composer/page.mdx). This guide is kept for reference only.
...
Use the `TwoColumnLayout` component in your UI routes that have two columns. For example:
...
This UI route also uses [Container](../../guides/container/page.mdx) and [Header](../../guides/header/page.mdx) custom components.