...
1. Retrieve the products from Medusa using the `useQueryGraphStep`.
    - This step uses [Query](!docs!/learn/fundamentals/query) to retrieve data across modules.
...
export const productsSyncPayloadSubscriberHighlights = [
  ["20", "products", "Retrieve paginated products from Medusa."],
  ["36", "filteredProducts", "Filter out the products that are already linked to Payload."],
  ["42", "createPayloadProductsWorkflow", "Execute the workflow to create products in Payload."]
]
...
In the subscriber, you use [Query](!docs!/learn/fundamentals/query) to retrieve all products from Medusa.
...
### a. Allow Retrieving Payload Data in Store Products

The Store API routes only return the fields that they allow in the `fields` query parameter. Any other field, including a field added by a custom link like `payload_product`, is removed from the response without an error.

So, before you can retrieve Payload data in the storefront, you must add `payload_product` to the allowed fields of the Products API route.

In your Medusa application, create the file `src/api/middlewares.ts` with the following content:
  ...
  routes: [
    {
      ...
      middlewares: [allowFields("payload_product")],
    },
  ],
})
```
...
This middleware pushes `payload_product` to the route's allowed fields, so Medusa keeps the field when a request asks for it.

<Note title="Important">

The middleware must be global, so don't set a `method` property in the route object. Medusa runs a middleware scoped to a method after it validates the `fields` query parameter, so it has no effect.

</Note>

Learn more in the [Allowed Fields](!docs!/learn/fundamentals/api-routes/allowed-fields) documentation.

### b. Fetch Payload Data with Product Data
...
Passing this field is possible because you [defined the virtual read-only link](#step-4-create-virtual-read-only-link-to-products) between the `Product` model in Medusa and the `Products` collection in Payload, and because you allowed the field in the previous section.
...
### d. Display Payload Product Title and Description
...
### e. Display Payload Product Images
...
### f. Display Product Options and Values
