...
- The user's auth identity was authenticated by Okta. You use [Query](!docs!/learn/fundamentals/query) to retrieve the user's auth identity with its provider identities, then check whether Okta is one of the providers. If not, you throw an unauthorized error.
