...
<Note title="Looking for a built-in integration?">

Medusa has an [OIDC Auth Module Provider](../../../enterprise/sso/page.mdx) that authenticates users with Okta, and any other identity provider that implements OpenID Connect, without writing a custom provider. The Medusa Admin dashboard also renders its login button for you. It's an [enterprise feature](../../../enterprise/page.mdx), so it requires a license key.

Follow this tutorial instead if you want to build and own the integration yourself.
...
### Allow the Okta Provider for Admin Users

Admin users can only authenticate with the `emailpass` provider by default, so you must also allow the Okta provider for the `user` [actor type](../../../commerce-modules/auth/auth-identity-and-actor-types/page.mdx).

To do that, set the `http.authMethodsPerActor` configuration in `medusa-config.ts`:
...
This allows admin users to authenticate with email and password or with Okta, and customers with email and password only.

<Note>

Before [Medusa v2.21.1](https://github.com/medusajs/medusa/releases/tag/v2.21.1), admin users could authenticate with every registered provider by default, so this configuration wasn't necessary.
