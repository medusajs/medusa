...
- [Medusa Search](!cloud!/search), the managed search service for Cloud projects. Refer to [Medusa Search vs Meilisearch](!cloud!/search/meilisearch) for a comparison.
- A [Search Module Provider](/references/search/provider) for Meilisearch, which keeps Meilisearch behind the Search Module's index definitions and `query.search`.

If you already followed this guide, refer to [Migrate from Meilisearch to the Search Module](#) for the migration steps.

{/* TODO add the link to the Meilisearch migration guide once it's written */}
...
1. Retrieve products from Medusa's database using `useQueryGraphStep`. This step uses Medusa's [Query](!docs!/learn/fundamentals/query) tool to retrieve data across modules. You pass it the pagination and filter parameters you received in the input.
