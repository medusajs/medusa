...
In your terminal, move to the `apps/backend` directory and run the following command:
...
In the step, you resolve from the Medusa container Sanity Module's service and [Query](!docs!/learn/fundamentals/query), which is a tool that allows you to retrieve data across modules and links.
...
Sanity has a CLI tool that helps you with the setup. First, change to the Next.js Starter's directory (`apps/storefront`).
...
You return the transaction ID of the workflow, which you can use to track the execution's progress since the workflow will run in the background. This is not implemented in this guide, but Medusa has a [Get Execution API route](!api!/admin/workflows-executions/get-exection) that you can use to get the details of a workflow's execution.
