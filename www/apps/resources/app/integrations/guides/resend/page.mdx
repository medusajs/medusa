...
<Note title="Important: Module Provider Resolve Path">

The `resolve` path must point to the **module directory** (like `./src/modules/resend`), not directly to a service file. Medusa's module loader looks for an `index.ts` file in this directory that exports the module definition using `ModuleProvider()`.

**Correct structure:**
```
src/modules/resend/
├── index.ts          # Exports ModuleProvider()
└── service.ts        # Contains the service class
```

**Common mistake:** Pointing `resolve` to `./src/modules/resend/service` will cause the error:
...
- `options`: An object of options to pass to the module. It has a `providers` property which is an array of module providers to register. Each module provider object has the following properties:
    - `resolve`: The path to the **module directory** (not the service file) to register in the application. It can also be the name of an NPM package.
