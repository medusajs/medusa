...
First, install the AvaTax SDK package to interact with Avalara's API. Run the following command in your `apps/backend` directory:
...
export const createTransactionStepHighlights = [
  ["34", "avalaraProviderService", "Retrieve the Avalara Tax Module Provider's service from the Tax Module."],
  ["38", "createTransaction", "Create a transaction in Avalara using the provider's createTransaction method."],
  ["51", "uncommitTransaction", "Uncommit the created transaction if an error occurs during the workflow's execution."]
]
...
So far, you retrieve the order's details using the `useQueryGraphStep`. It uses [Query](!docs!/learn/fundamentals/query) under the hood to retrieve data across modules.
...
export const updateItemStepHighlights = [
  ["24", "getItem", "Retrieve the original item before updating."],
  ["27", "updateItem", "Update the item in Avalara using the provider's updateItem method."],
  ["44", "updateItem", "Revert the updates by restoring the original values if an error occurs during the workflow's execution."]
]
...
export const updateProductItemWorkflowHighlights = [
  ...
  ["48", "updateItemStep", "Update the item in Avalara if it exists."],
  ["58", "transform", "Return either the created or updated item as the workflow's output."]
]
...
export const updateProductItemWorkflow = createWorkflow(
  ...
  (input: WorkflowInput) => {
    ...
    const updateResponse = when({ products }, ({ products }) => 
      ...
      .then(() => {
        const avalaraItemId = transform(
          { products },
          ({ products }) => products[0].metadata?.avalara_item_id as number
        )
...
export const deleteItemStepHighlights = [
  ["19", "getItem", "Retrieve the original item before deleting."],
  ["21", "deleteItem", "Delete the item in Avalara using the provider's deleteItem method."],
  ["42", "createItems", "Recreate the item using the original values if an error occurs during the workflow's execution."]
]
