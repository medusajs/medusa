...
1. Retrieve the product options in Medusa using the `useQueryGraphStep`.
    - This step uses [Query](!docs!/learn/fundamentals/query) to retrieve data in Medusa across modules.
...
### a. Allow Retrieving Strapi Data in Store Products

The Store API routes only return the fields that they allow in the `fields` query parameter. Any other field, including a field added by a custom link like `strapi_product`, is removed from the response without an error.

So, before you can retrieve Strapi data in the storefront, you must add `strapi_product` to the allowed fields of the Products API route.

In your Medusa application, add a route object to the `src/api/middlewares.ts` file that you created earlier:
...
export default defineMiddlewares({
  routes: [
    // ...
    {
      ...
      middlewares: [allowFields("strapi_product")],
    },
  ],
})
```
...
This middleware pushes `strapi_product` to the route's allowed fields, so Medusa keeps the field when a request asks for it. The other fields that you'll pass in the next section, such as `*variants.images`, are already among the route's allowed fields.
...
### b. Retrieve Strapi Product Data
...
### d. Add Strapi Product Utilities
...
### f. Customize Product Details Metadata
...
### g. Customize Product Details Page
