...
import { Card, Note, Prerequisites, Details, WorkflowDiagram } from "docs-ui"
...
<Note title="Deprecated" type="warning">

This guide is deprecated as of Medusa v2.20.0, which introduced the [Search Module](/infrastructure-modules/search). Rather than the custom module this guide builds, consider one of the following instead:

- [Medusa Search](!cloud!/search), the managed search service for [Cloud](!cloud!) projects. Refer to [Medusa Search vs Algolia](!cloud!/search/algolia) for a comparison.
- A [Search Module Provider](/references/search/provider) for Algolia, which keeps Algolia behind the Search Module's index definitions and `query.search`.

If you already followed this guide, refer to [Migrate from Algolia to the Search Module](#) for the migration steps.

{/* TODO add the link to the Algolia migration guide once it's written */}
...
1. Execute `useQueryGraphStep` to retrieve products from Medusa's database. This step uses Medusa's [Query](!docs!/learn/fundamentals/query) tool to retrieve data across modules. You pass it the pagination and filter parameters you received in the input.
