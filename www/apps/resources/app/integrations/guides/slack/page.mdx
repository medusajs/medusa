...
- `resolve`: The path to the **module directory** (like `./src/modules/slack`), not to the service file. Medusa's module loader looks for an `index.ts` file in this directory that exports the module definition.
...
The `resolve` path must point to the **module directory**, not directly to a service file. If you point to `./src/modules/slack/service`, you'll get the error:
