...
- You can now manage webhook endpoints in your organization's settings. Create an HTTPS endpoint that receives Cloud events, enable or disable it, rotate its signing secret, and delete it. Refer to [Webhook Endpoints in Cloud](/webhooks/endpoints) for more details.
- The new **Webhook Events** page shows the delivery history of your organization's webhook events, including each event's status, number of attempts, failure reason, and next automatic retry. You can also retry a delivery yourself and filter the events by project or environment. Refer to [Manage Webhook Events in Cloud](/webhooks/events) for more details.
