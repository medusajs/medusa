...
If you have any of these modules configured in your `medusa-config.ts`, remove them before deploying to Cloud. Keeping them causes configuration conflicts that result in deployment failures or in-memory fallback warnings at runtime. Also remove any `redisUrl` set in `projectConfig`, since Cloud manages Redis provisioning automatically.
