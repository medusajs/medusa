...
2. In the side window that opens:
    ...
    - **Branch**: Click the branch field to open the list of your repository's branches, then select the branch you want to associate with this environment. For example, `staging`. You can use the search field at the top of the list to find a branch by its name.
        - If a branch is already linked to another environment, the list shows a badge with that environment's name next to the branch, and you can't select it.
    ...
    - **Custom subdomain**: Set the custom subdomain for the Medusa application (and, subsequently, storefront). Medusa applications are a subdomain of `medusajs.app`, and their storefront is a subdomain of `medusajs.site`.
        - Refer to the [Subdomain Restrictions](../../projects/page.mdx#subdomain-restrictions-for-projects) guide for reserved subdomains and restrictions.
    - **Infrastructure Specs** (optional): By default, your long-lived environment will use the infrastructure specs of your organization's plan. You can also choose to upgrade or downgrade these specs for a specific environment. For example, you can downgrade your long-lived environment to **Develop** if you want to reduce costs. When you enable the Infrastructure Specs toggle, you can choose a different spec. Refer to the [Plans and Pricing](../../pricing/page.mdx) to learn more about the specs of each plan.
3. Once you're done, click the "Create" button.

If Cloud can't create the environment, such as when the branch is already in use, a notification shows the error message.
...
2. In the environment's dashboard, click **Environment variables** in the sidebar.
...
1. In the [project's dashboard](../../projects/page.mdx#open-project-dashboard), click on the environment you want to redeploy.
2. In the environment's dashboard, click on the "Redeploy" button to redeploy the environment with the new environment variables.
...
This will open the environment's dashboard.
...
Aside from the subdomain you set when [creating the environment](../../environments/long-lived/page.mdx#create-a-long-lived-environment-on-cloud), Medusa also provides a unique, randomly generated URL for each environment. Both of these URLs point to the same deployment.
...
You can promote a long-lived environment to Production in Cloud. This is useful when you want to make the environment's deployment the live application for your users, or when you want to switch the Production environment to a different branch.

<Note title="Caution" type="warning">

Before promoting an environment to Production, consider the following:

- By promoting a long-lived environment to Production, the Production environment will become a long-lived environment, and the promoted environment will become the new Production environment. The previous Production environment will no longer be the live application for your users, and its name will be changed to the name of the promoted environment.
- Promoting the environment doesn't retain the Production environment's settings or custom domain. If you want to keep certain settings the same, such as the same custom domain, make sure to set them up again after promotion.

</Note>

For example, if you have a Staging environment and you want to promote it to Production, the Staging environment will become the new Production environment, and the previous Production environment will be renamed to "Staging".

To promote a long-lived environment to Production:

1. In the [project's dashboard](../../projects/page.mdx#open-project-dashboard), find the long-lived environment you want to promote to Production.
2. Click on the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon at the bottom right of the environment's card.
3. Choose "Promote to Production" from the dropdown menu.

This will promote the environment's deployment to Production, making it the live application for your users.
...
Learn more in the [Monitoring guide](../../monitoring/page.mdx).

---

## Manage Environment Variables

In the environment's **Environment variables** settings, you can manage the environment variables for the environment's Medusa application and storefront. Environment variables are key-value pairs that store sensitive information, such as API keys, or configurations that your application needs to run.
...
- **Medusa-provided subdomains**: The default `medusajs.app` (backend) and `medusajs.site` (storefront) subdomains assigned to the environment. For example, `acme.medusajs.app` and `acme.medusajs.site`.
- **Custom domains**: Your own domain names to serve the backend and storefront instead of the Medusa-provided subdomains. For example, `acme.com`.

To update these domains, see the [Subdomains](../subdomains/page.mdx) and [Custom Domains](../custom-domains/page.mdx) guides.

---

## Change Environment's Branch

You can't change the branch of preview environments. You can only change it for long-lived environments, such as Production or Staging.

To change the branch associated with a long-lived environment:
...
5. In the "Linked branch" section, click the <InlineIcon Icon={PencilSquare} alt="pencil" /> icon to change the branch.
6. In the side window that opens, click the branch field and select the branch you want to link to the environment. You can use the search field at the top of the list to find a branch by its name. Branches already linked to another environment show a badge with that environment's name, and you can't select them.
7. Click **Save** to apply the changes. If Cloud can't update the branch, a notification shows the error message.

The next time you push a commit to the new branch, Medusa will create a new deployment for the environment. Pushes to the previous branch will no longer trigger new deployments for the environment.

---

## Import and Export Environment's Database Dump

In the environment's **Database** settings, you can export a database dump of the environment. This is useful for backing up the environment's data, debugging issues, or migrating the data to another environment.
...
## Change Infrastructure Specifications for Long-Lived Environment

<Note>

To change the infrastructure specs of the Production environment, see the [Projects](../../projects/page.mdx#change-project-infrastructure-specifications) guide.

</Note>

By default, long-lived environments that aren't the Production environment use the specs of the **Develop** plan. This is useful to reduce costs on those environments, but that means it may not be fitting for high-traffic or complex operations.

To change the infrastructure specs for a long-lived environment:

1. In the [environment's dashboard](#open-environment-dashboard), click **Deployment Rules** in the sidebar.
2. In the **Infrastructure Specs** section, select the plan you'd like to use its specs for the long-lived environment.
3. Click **Save Changes**.

The new specs for the environment will be applied afterward.
...
1. In the environment's dashboard, click **Deployment Rules** in the sidebar.
2. Click the **Delete** button in the "Delete Environment" section.
