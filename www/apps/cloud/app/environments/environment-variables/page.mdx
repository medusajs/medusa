...
A toolbar above the environment variables table lets you find and filter variables quickly:
...
- **System Variables toggle** (backend tab only): Show or hide system-managed variables. System variables are hidden by default.

---

## System Environment Variables Visibility

In addition to the [system environment variables](#system-environment-variables) documented above, the backend tab can now display all system-provided variables alongside your user-defined ones when the **System Variables** toggle is enabled.

System variables are visually distinguished from user-defined variables with a subtle background and muted text. Because they are managed by Cloud:
...
7. In the side window that opens, you can edit the variable's key, value, and configure its settings:
