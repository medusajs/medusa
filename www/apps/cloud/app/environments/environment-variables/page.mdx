...
The environment variables table adapts to different screen sizes for better usability:

- **Desktop view**: Shows all columns including Key, Value, Sensitive status, Build/Runtime scope, and Last Update timestamp.
- **Mobile view**: Automatically hides the "Last Update" column to provide more space for essential information.
- **Value display**: Non-secret variables show their values directly, while secret variables display masked values with a reveal button.
