...
- **Provisioning failed**: The domain could not be provisioned by the platform. The DNS detail page shows an alert explaining the failure instead of the DNS configuration instructions. Contact support if you see this status.
...
If the domain's status is **Provisioning failed**, the DNS detail page replaces the DNS configuration instructions with an alert explaining the failure. The **Check records** and **Restart verification** buttons are not available when provisioning has failed. Contact support if you encounter this state.
