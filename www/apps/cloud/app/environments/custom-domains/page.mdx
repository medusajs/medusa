...
6. In the modal that opens, enter your custom domain name (for example, `api.acme.com`) and click **Continue**.
...
You can add multiple backend custom domains. The **Add Domain** button is shown when your plan entitlement allows it. If a plan upgrade is required, a tooltip indicates whether you need to upgrade to add your first domain or to add more domains.
...
6. In the modal that opens, enter your custom domain name (for example, `shop.acme.com`) and click **Continue**.
...
You can add multiple storefront custom domains. The **Add Domain** button is shown when your plan entitlement allows it and your environment has a storefront. If a plan upgrade is required, a tooltip indicates whether you need to upgrade to add your first domain or to add more domains.
...
- **Failed**: DNS verification failed. Use the **Restart** button on the DNS detail page to retry.
...
In the **Domains** section of your environment settings, each custom domain is shown as a card with its domain name and DNS verification status. Click a domain card to open its DNS configuration detail page.

If an environment has more custom domains than the preview limit shown on the project overview, a **+N more** button appears on the environment card. Click it to expand and see all domains.
...
3. In the confirmation dialog, confirm the removal of the domain name shown.
...
If you've updated your DNS records and want to trigger an immediate check, open the DNS configuration detail page for your domain and click the **Verify records** button. This manually starts a fresh DNS check instead of waiting for the next automatic check. If the domain is in a failed state, click **Restart** instead to retry from the beginning.
...
2. **Re-verify**: Click the **Verify records** button on the DNS configuration detail page to trigger a fresh check. If the domain is in a failed state, click **Restart** instead.
