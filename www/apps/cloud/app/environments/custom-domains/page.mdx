...
Each section shows whether the corresponding DNS records have been verified. A propagation note below the sections reminds you that DNS changes may take up to 48 hours to take effect. The domain header also shows a **last checked** timestamp indicating when the most recent verification check ran.

### Verify or Restart DNS Checks

The action button on the DNS detail page adapts to the domain's current status:

- When the domain is in a **failed** state, the button shows **Restart** — click it to retry the verification from the beginning.
- Otherwise, the button shows **Verify** — click it to trigger an immediate DNS check.
...
2. **Re-verify**: Click the **Verify** button on the DNS configuration detail page to trigger a fresh check. If the domain is in a failed state, click **Restart** instead.
