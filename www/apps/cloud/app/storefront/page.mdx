...
Cloud deploys storefronts on Cloudflare via the OpenNext Cloudflare adapter, which only supports Edge runtime middleware. If you use Next.js middleware with the default Node.js runtime, your build will fail with the following error:
...
To fix this, add the Edge runtime export to your middleware file:
...
When running middleware in the Edge runtime, Node.js-specific APIs such as `fs`, `path`, and the Node.js `crypto` module are not available. Use Web Platform APIs instead. For example, replace `crypto.createHash` with the Web Crypto API:
...
Refer to the [Next.js Edge Runtime documentation](https://nextjs.org/docs/app/api-reference/edge) and the [OpenNext Cloudflare documentation](https://opennext.js.org/cloudflare) for more details.
