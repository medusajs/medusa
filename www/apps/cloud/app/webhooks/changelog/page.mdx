...
- Added the `build.created` event. Cloud delivers this event when a new build is created for an environment.
...
- Added the `deployment.created` event. Cloud delivers this event when a new deployment is created for an environment.
...
Cloud now delivers webhooks for build and deployment lifecycle events. Each request is a POST with a JSON body and the following headers: X-Medusa-Signature (HMAC-SHA256 signature in the format t={unix_timestamp},v1={hex_digest}, signed over the string {timestamp}.{raw_body} using the endpoint secret), X-Medusa-Event (the event type string), X-Medusa-Delivery (the webhook event ID), and User-Agent (MedusaCloud-Webhook/1.0). Cloud expects a 2xx response within 10 seconds; if none is received the delivery is marked failed. Failed deliveries are retried automatically with exponential backoff starting at 5 minutes and doubling on each attempt, up to 24 hours from the original event.
