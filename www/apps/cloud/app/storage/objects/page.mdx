...
In this guide, you'll learn about the managed object storage service that Medusa provides as part of your project environments on Cloud.

## Managed Object Storage Service on Cloud

Medusa offers a managed S3-compatible object storage service for your project environments on Cloud. Each environment has its own dedicated S3 bucket that is automatically provisioned when the environment is created.
...
### Access Object Storage Configurations
...
To access the object storage details of an environment:

<Note title="Tip">

You can also navigate to a project's object storage details from the [command palette](../../command-palette/page.mdx). Press <Kbd>{getOsShortcut()}</Kbd> + <Kbd>K</Kbd>, select **Storage**, then choose a project.
...
3. In the environment's dashboard, click on the **Storage** tab.
4. In the sidebar, click **Objects**.

### Object Storage Configuration

When object storage is configured, the **Object storage** section displays:

- Your full S3 file URL
- Your S3 endpoint
...
If object storage isn't configured for your environment, you'll see a message indicating that object storage is unavailable. This typically happens when:

- The S3 file URL or endpoint isn't configured
- Object storage hasn't been set up for the environment
- There's a configuration issue with the storage service

In this case, you may need to contact support or check your environment's configuration.

### Override Object Storage Configurations

If you need to override the default object storage configurations of your Cloud project, such as changing public access to assets, you can do so by setting up and managing your own S3 bucket externally.
...
To set the connection options of your external S3 instance, refer to the [Environment Variables](../../environments/environment-variables/page.mdx#add-environment-variables) guide to learn how to add environment variables. You can then use these variables in your `medusa-config.ts` file to connect to your S3 instance.
