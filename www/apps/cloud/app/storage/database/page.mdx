...
In this guide, you'll learn about the managed database storage service that Medusa provides as part of your project environments on Cloud.
...
Database backups are available to the Launch, Scale, and Enterprise plans only. Learn how to upgrade in the [Manage Plans](../../billing/plans/page.mdx#change-plan) guide.
...
- **Source**: The backup origin. **Scheduled** backups are created automatically; **Manual** backups are triggered by you; **Other** backups may be created during maintenance operations.
...
This will start a download of the backup file to your device. It's a `.dump` file that you can restore to a PostgreSQL database using tools like `pg_restore`. For example:
...
A manual backup is triggered when you export a database dump from the environment settings. Learn how to do that in the [Import/Export Database Dumps](#importexport-database-dumps) section below.
...
For production environments, you'll see additional warnings when uploading database dumps to help prevent accidental data overwrites.
