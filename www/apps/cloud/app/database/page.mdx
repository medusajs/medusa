...
5. In the "Database Read" section, copy the read-only connection string in the code block.
...
## Enable Database Write Access

For advanced use cases where you need write access to your environment's database, Medusa allows organization owners to enable database write access.
...
3. In the environment's dashboard, click on the "Settings" tab.
4. Choose the "Storage" tab from the sidebar.
5. In the "Database Write Access" section, toggle the switch to enable write access.
6. Once enabled, copy the write connection string from the code block that appears.

<Note title="Permission Requirements">

Only organization owners can enable or disable database write access. Non-owners can view the current state of the toggle but cannot modify it.

</Note>

You'll receive success or error messages when toggling the write access setting, confirming whether the operation completed successfully.
