import { getOsShortcut, Kbd, InlineIcon, Table } from "docs-ui"
...
You can upgrade your plan as explained in the [Manage Plans](../billing/plans/page.mdx#change-plan) guide.
...
You don't need to add a `predeploy` script to your `package.json`, or run `medusa db:migrate` in your `start` script, to apply migrations on Cloud. If you're moving a self-hosted application that runs migrations this way, you can rely on the deployment to run them instead.

### How Data Migration Scripts Run on Cloud

Each environment has its own database, so Medusa runs a data migration script once per environment. Medusa records a script in the `script_migrations` table of the environment's database only after the script runs to completion. So:
...
To run a one-off data task on a Cloud environment, such as seeding data or filling a configuration table, create a [data migration script](!docs!/learn/fundamentals/data-models/write-migration#data-migration-scripts) in the `src/migration-scripts` directory instead of a [custom CLI script](!docs!/learn/fundamentals/custom-cli-scripts) that you run with `medusa exec`.

For example:

```ts title="src/migration-scripts/seed-config.ts"
import { ExecArgs } from "@medusajs/framework/types"

export default async function seedConfig({
  container,
}: ExecArgs) {
  // perform the data changes, for example,
...
Test the script locally with `npx medusa db:migrate` and deploy it to a [preview environment](../environments/preview/page.mdx) before merging it to your production branch.
