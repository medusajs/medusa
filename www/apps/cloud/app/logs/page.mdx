...
By default, runtime logs are displayed in descending order, with the most recent logs shown first. You can change the sort order to ascending to view logs from oldest to newest.

To change the sort order, click the sort button in the **Timestamp** column header. The logs will update to reflect the new order, and the loading indicator moves to match the selected direction.
