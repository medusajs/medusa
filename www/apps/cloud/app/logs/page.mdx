...
You can sort the runtime logs by timestamp to view them in ascending (oldest first) or descending (newest first) order. This helps you analyze logs chronologically or find the most recent entries quickly.

To change the sort order, click the sort button next to the timestamp column header in the logs table. This will toggle between:

- **Descending order** (newest first): Shows the most recent logs at the top
- **Ascending order** (oldest first): Shows the oldest logs at the top

The loading indicator position adjusts based on your selected sort order - appearing at the top when sorted in descending order and at the bottom when sorted in ascending order.
