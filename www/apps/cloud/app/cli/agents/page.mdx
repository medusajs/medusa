import { Note, Table, CodeTab, CodeTabs } from "docs-ui"
...
- **Use `--yes` for irreversible operations.** [`mcloud projects delete`](../commands/projects/page.mdx#projects-delete), [`mcloud environments delete`](../commands/environments/page.mdx#environments-delete), and [`mcloud variables delete`](../commands/variables/page.mdx#variables-delete) require confirmation in interactive mode and refuse to run without `--yes` in non-interactive mode.
...
Both fields use the same value set: `created`, `building`, `built`, `deploying`, `deployed`, `build-failed`, `deployment-failed`, `timed-out` (backend only), `canceled`, and `idle`. See the full reference in the [deployments command guide](../commands/deployments/page.mdx#backend-and-storefront-statuses).
...
When `build-logs` alone isn't enough to pinpoint a build failure, reproduce the Cloud build on the local machine with [`mcloud local build`](../commands/local/page.mdx). It runs the same Docker build Cloud runs and infers the root path and build variables from the linked project and environment, so an agent can iterate on a fix without pushing to the tracked branch and waiting for a full Cloud build each time.

<Note>

`mcloud local build` requires Docker installed and running, and must run from inside the project's Git repository. Unlike every other command, it has **no `--json` flag**: it streams plaintext build output and signals the result through its exit code (`0` means the build succeeded). Do not parse its output as JSON. It reproduces `build-failed` failures only, not `deployment-failed` (runtime) failures.

</Note>

To match the failure exactly, check out the same commit the failed deployment built, then reproduce the build:
...
# Check out the same commit so the local build matches the failed one
git checkout "$COMMIT"

# Reproduce the backend build locally, then route on the exit code
if mcloud local build; then
  echo "Build succeeded locally; the failure is not reproducible from this commit."
else
  echo "Build failed locally; inspect the streamed output above for the failing step."
fi
...
If the failure only involves environment variables, reproduce and test a fix without editing code by overriding them:
...
Once the local build exits `0`, push the fix to the tracked branch and start a fresh Cloud build with [`mcloud environments trigger-build`](#rerun-a-deployment).
