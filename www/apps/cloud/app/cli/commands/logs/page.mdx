...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      `-e <id-or-handle>`, `--environment <id-or-handle>`

      </Table.Cell>
      <Table.Cell>

      [The ID or handle of the environment](../environments/page.mdx#find-environment-ids-and-handles) to fetch logs for.
      ...
      Falls back to the `MC_ENVIRONMENT_HANDLE` environment variable, if set.
    ...
    <Table.Row>
      ...
      Filter log lines to those belonging to a specific deployment.
...
<CodeTabs group="cli-auth">
  ...
  # MC_ENVIRONMENT_HANDLE=production
...
Set environment variables once to avoid repeating project and environment
flags across multiple commands in the same session:

```bash
export MC_PROJECT_HANDLE=my-store
export MC_ENVIRONMENT_HANDLE=production
...
its ID. Find the deployment ID from `mcloud environments get` or the Cloud
dashboard.
...
<CodeTabs group="cli-auth">
  ...
  mcloud logs --deployment-id dep_01ABC123
  ```
