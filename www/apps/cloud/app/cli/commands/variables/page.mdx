...
When creating a new variable, both `--build` and `--runtime` default to `false`. A variable with no scope is saved to the environment but is not injected into the build or the running container. Always pass `--runtime`, `--build`, or both when creating a new variable to ensure it is available where you need it.
