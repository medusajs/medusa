...
4. **Activity**: This section shows the build logs for the backend and storefront (if applicable). For storefronts, logs are categorized into separate build and deployment phases for clearer visibility into each stage of the process. These logs are useful to understand why a deployment failed. Errors from either the backend or storefront build or deployment processes will cause the deployment to show a failed status. Learn more in the [Logs](../logs/page.mdx) guide.
...
When a deployment or its build fails, you can use the "Fix with AI" feature to troubleshoot the issue with the help of Cloud's support agent. The AI agent will analyze the deployment and build logs, identify the root cause of the issue, and provide you with actionable steps to resolve it.
