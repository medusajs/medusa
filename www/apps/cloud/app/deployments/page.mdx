import { Table, InlineIcon, getOsShortcut, Kbd, Tabs, TabsList, TabsTrigger, TabsContent, TabsContentWrapper } from "docs-ui"
...
Storefront builds and deployments operate independently of the backend status. This means storefront activities can proceed successfully even if the backend build or deployment fails.
...
When a deployment or its build fails, you can use the "Fix with AI" feature to troubleshoot the issue with the help of Cloud's support agent. The agent support drawer is always available in the deployment activity section for failed deployments, allowing you to access assistance whenever issues occur. The AI agent will analyze the deployment and build logs, identify the root cause of the issue, and provide you with actionable steps to resolve it.
