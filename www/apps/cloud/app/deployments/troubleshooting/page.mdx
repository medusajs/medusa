...
3. In the environment's dashboard, click on the **Environment Variables** tab.
4. Check whether you have a `DATABASE_URL` variable set. If so, remove it and let Cloud manage the connection string.

### Verify the Deployment Completed Successfully

Database migration runs during deployment. If the deployment itself failed before migrations completed, the database state may be inconsistent. Trigger a new deployment after fixing any build errors to re-run migrations.

### Contact Support

If the issue persists after checking environment variables and re-deploying, contact support with the deployment ID so the team can investigate the database connection at the platform level.
