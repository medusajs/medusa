...
### pnpm Users: Environment Variable Expansion in .npmrc

pnpm does not expand environment variables in project-level `.npmrc` files. This is a deliberate pnpm security policy to prevent leaking secrets to attacker-controlled registries. If you use pnpm and add `${NPM_TOKEN}` to a project-level `.npmrc`, the token is not substituted and authentication fails.

To work around this, use the `cloud:preinstall` lifecycle hook to write the token to a user-level `~/.npmrc` file, where pnpm does expand environment variables:

1. Remove or leave empty the auth token line from your project-level `.npmrc`.
2. Add a `cloud:preinstall` script to your root `package.json`:

```json title="package.json"
{
  "scripts": {
    "cloud:preinstall": "echo '//registry.example.com/:_authToken=${NPM_TOKEN}' > ~/.npmrc"
  }
}
```

Replace `registry.example.com` with your registry hostname. The `cloud:preinstall` script runs before `pnpm install` in the Cloud build process, so the user-level `.npmrc` is in place when pnpm fetches packages.
...
- **npm registry rate limiting (HTTP 429)**: Unauthenticated requests to the npm registry are rate-limited, causing `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` errors for Medusa packages.
...
### Fix npm Registry Rate-Limit Errors (HTTP 429)

If the full build logs show `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` errors for Medusa packages such as `@medusajs/js-sdk`, `@medusajs/modules-sdk`, or `@medusajs/types`, the npm registry is rate-limiting unauthenticated requests. Authenticated requests have significantly higher rate limits.

To resolve this, add an `NPM_TOKEN` environment variable to your Cloud project and configure pnpm to use it:

1. [Add an environment variable](../../environments/environment-variables/page.mdx#add-environment-variables-on-cloud) named `NPM_TOKEN` in your Cloud project and mark it as available at build time.
2. Because pnpm does not expand environment variables in project-level `.npmrc` files, use the `cloud:preinstall` lifecycle hook to write the token to a user-level `~/.npmrc`:

```json title="package.json"
{
  "scripts": {
    "cloud:preinstall": "echo '//registry.npmjs.org/:_authToken=${NPM_TOKEN}' > ~/.npmrc"
  }
}
```

3. Commit the updated `package.json` and push to trigger a new deployment.

The `cloud:preinstall` script runs before `pnpm install` in the Cloud build process, so the authenticated `.npmrc` is in place when pnpm fetches packages from the npm registry.
...
Medusa prunes `turbo` monorepos down to the package it's building. The pruned output only includes the root `package.json`, the lockfile, `pnpm-workspace.yaml`, `turbo.json`, `.gitignore`, `.npmrc`, and the source code of the target package along with the workspace packages it depends on. So, a shared root file, such as a `tsconfig.base.json` file that other packages reference with `extends`, isn't copied into the build.

### Fix Missing Root Files

Move the shared file into a workspace package, then add that package as a dependency of your Medusa application or storefront. Medusa then includes the file in the build, since it builds the target package along with the workspace packages it depends on.

For files that your Medusa application needs at runtime, such as JSON files or fonts, keep them within your Medusa application and copy them to the `.medusa/server` directory as part of your `build` script. Learn more in the [Built Assets](!docs!/learn/build#built-assets) documentation.
