...
If your deployment fails with a database connection error such as `KnexTimeoutError` or `Pg connection failed to connect to the database`, the most common causes are a manually set `DATABASE_URL` environment variable or a hardcoded `databaseUrl` in `medusa-config.ts`.

<Note>

Medusa automatically provisions and manages the database for each environment on Cloud. You do not need to create or configure a database yourself. The database connection is set automatically by Cloud when your environment is created.

</Note>

### Remove databaseUrl from medusa-config.ts

If your `medusa-config.ts` file hardcodes the `databaseUrl` option, it overrides Cloud's automatically injected connection string and causes connection failures. This is a common mistake when carrying over a local development configuration to Cloud.

Open your `medusa-config.ts` and remove any hardcoded `databaseUrl` value so Cloud can manage the connection:
...
export default defineConfig({
  projectConfig: {
    // Do NOT set databaseUrl here on Cloud.
    // Cloud injects the database connection automatically.
  },
  // ...
})
```

Common symptoms of a hardcoded `databaseUrl` include:

- `KnexTimeoutError` or `Pg connection failed` errors in deployment logs
- Storefront builds failing with 500 errors when calling the backend API
- Warnings about a fake Redis fallback appearing in runtime logs
...
Do not rely on a custom `package.json` `start` script for application initialization on Cloud. Cloud bypasses this script and calls `medusa start` directly.
...
Cloud injects the following environment variables into every environment automatically. You cannot override them:
...
You can read these variables inside your loaders or middleware to apply environment-specific logic.
