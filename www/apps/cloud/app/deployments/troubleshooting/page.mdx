...
If your Yarn berry monorepo build fails with the following error, the `yarn.lock` file is not included in the Docker build context:
...
### Diagnose the Missing Lockfile

Check for these indicators in your build logs:

- The error message references `/yarn.lock: not found`
- The build fails early during the dependency installation step
- No other lockfile errors are shown
...
Always commit your lockfile to version control. Cloud requires the lockfile to install dependencies with a frozen install, ensuring reproducible builds. Add the following to your `.gitignore` to make sure the lockfile is not accidentally excluded:

Remove any line in your `.gitignore` that matches `yarn.lock` or `*.lock`.
