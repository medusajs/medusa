import { getOsShortcut, Kbd, Note } from "docs-ui"
...
If you maintain a single `medusa-config.ts` for both Cloud and non-Cloud deployments, use the `MEDUSA_CLOUD_ENVIRONMENT_TYPE` [system environment variable](../environments/environment-variables/page.mdx#system-environment-variables) to conditionally skip the S3 module registration. Cloud automatically sets this variable in all environments:
...
const isCloud = !!process.env.MEDUSA_CLOUD_ENVIRONMENT_TYPE
...
`MEDUSA_CLOUD_ENVIRONMENT_TYPE` is set to `production`, `preview-instance`, or `long-lived` depending on the environment. It is not set outside of Cloud, so checking for its presence is a reliable way to detect a Cloud deployment.
