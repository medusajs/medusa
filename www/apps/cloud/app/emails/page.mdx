...
import { CodeTabs, CodeTab, Note, Prerequisites, InlineIcon, getOsShortcut, Kbd } from "docs-ui"
...
2. Click **Projects** in the sidebar and select a project.
3. Click **Emails** -> **Sender Domains** from the sidebar.
4. Click the **Add sender domain** button.
5. In the form that opens, enter the domain you want to add (for example, `yourdomain.com`) and click **Continue**.

After submitting, you'll be taken to the domain's detail page where the required DNS records are shown. Add these records to your domain's DNS settings:

- **DKIM** (labeled **Required**): DomainKeys Identified Mail records that authenticate your emails. You must add these records to send emails from your domain.
- **SPF**: (Optional) Sender Policy Framework records that help receiving mail servers verify your domain. Adding SPF records improves deliverability but is not required.
- **DMARC**: (Optional) Domain-based Message Authentication records that build on DKIM and SPF to protect against email spoofing. Adding DMARC records is recommended for improved email security but is not required.

If you're unsure how to add DNS records, refer to your domain registrar's documentation or support.

The domain will then be verified periodically. This process may take a few minutes to several hours, depending on your DNS provider.

### View Domain Verification Status

After adding a sender domain, you can view its verification status on the domain detail page. To open it, click the domain name in the **Sender Domains** list.

The detail page is organized into three sections: **DKIM**, **SPF**, and **DMARC**. Each section shows whether the corresponding DNS records have been verified. The domain header also shows when the most recent verification check ran.

### Provisioning Failure

If the domain's status is **Provisioning failed**, the domain detail page replaces the DNS configuration instructions with an alert explaining the failure. The **Check records** and **Restart verification** buttons are not available when provisioning has failed. Contact support if you encounter this state.

### Verify DNS Records Manually

You can trigger an immediate DNS check at any time instead of waiting for the next automatic check by clicking the **Check records** button at the top right of the domain detail page.

### Restart DNS Verification

To restart the verification process from the beginning at any time, click the **Restart Verification** button at the top right of the domain detail page.

### Diagnose DNS Record Issues

On the domain detail page, you can click the <InlineIcon Icon={TriangleRightMini} alt="expand" /> icon for any record whose verification status is failing. This shows:
...
Use these diagnosis details to verify and fix any issues in your domain's DNS records. For example, if a different value is set for a record, make sure you set the correct value.
...
You can also navigate to the Email Activity page from the [command palette](../command-palette/page.mdx). Press <Kbd>{getOsShortcut()}</Kbd> + <Kbd>K</Kbd> and select **Sender Domains**.

</Note>

1. If you're in a different organization, [switch to the organization](../organizations/page.mdx#switch-organization).
2. Click **Projects** in the sidebar and select a project.
3. Click **Emails** -> **Activity** from the sidebar.
...
The earliest selectable start date depends on your organization's [plan](../pricing/page.mdx):

- **Develop plan**: You can query email logs from up to the past 12 hours.
- **Launch and Scale plans**: You can query email logs from up to the past 3 days.

If you select a start date earlier than what your plan allows, the picker automatically moves it forward to the earliest permitted time.
...
If the email activity and logs don't show a cause related to your application, [check the Cloud status page](../status/page.mdx) for ongoing Cloud incidents that may be affecting email delivery.
