...
6. In the form that opens, perform these two steps:
    ...
    - **DNS Records**: Add the provided DNS records to your domain's DNS settings. The form shows two categories of records:
...
The detail page shows the DNS records for each section (DKIM, SPF, and DMARC) with an inline status indicator per row showing whether verification succeeded, failed, or is still in progress. When all DKIM records pass but the domain is still pending platform confirmation, a **Finalizing** status is shown.
...
If you've updated your DNS records and want to trigger an immediate check, click the **Reverify records** button on the domain detail page. This manually starts a fresh DNS check rather than waiting for the next automatic check.
