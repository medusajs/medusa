...
import { ChevronUpDown, TriangleRightMini } from "@medusajs/icons"
...
6. In the form that opens, enter the domain you want to add (for example, `yourdomain.com`) and click **Continue**.

After submitting, you are navigated to the domain's detail page where the required DNS records are shown. Add these records to your domain's DNS settings:
...
- **SPF** (labeled **Optional**): Sender Policy Framework records that help receiving mail servers verify your domain. Adding SPF records improves deliverability but is not required.
- **DMARC** (labeled **Optional**): Domain-based Message Authentication records that build on DKIM and SPF to protect against email spoofing. Adding DMARC records is recommended for improved email security but is not required.
...
The detail page is organized into three sections: **DKIM**, **SPF**, and **DMARC**. Each section shows whether the corresponding DNS records have been verified.

When all DKIM records pass but the domain is still pending platform confirmation, a **Finalizing** status is shown. A propagation note below the sections reminds you that DNS changes may take up to 48 hours to take effect.

The domain header shows a **last checked** timestamp indicating when the most recent verification check ran.

<Note>

The DMARC section's informational alert is only shown when DMARC has passed verification.
...
### Verify or Restart DNS Checks

The action button on the domain detail page adapts to the domain's current status:

- When the domain is in a **failed** state, the button shows **Restart** — click it to retry the verification from the beginning.
- Otherwise, the button shows **Verify** — click it to trigger an immediate DNS check rather than waiting for the next automatic check.
