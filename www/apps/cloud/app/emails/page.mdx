...
The domain header shows when the most recent verification check ran.

### Restart Failed DNS Setup

When the domain is in a **failed** state, you can retry the verification from the beginning by clicking the **Restart** button at the top right. This button will only show up in the failed state.

### Verify DNS Records Manually

If your domain's status is still pending, you can trigger an immediate DNS check instead of waiting for the next automatic check by clicking the **Verify records** button at the top right.
