...
Medusa Emails is available for all Cloud organizations and projects, given you [don't have another Notification Module Provider for the `email` channel configured](../page.mdx#remove-other-email-notification-module-providers). You also optionally need to [verify your sending domain](../page.mdx#add-email-sender-domain-to-cloud) to send emails to users outside your organization. Verified sender domains are available for Launch, Scale, and Enterprise plans only.
...
If your Cloud deployment fails with `Cannot find module 'react/jsx-runtime'`, it means `react` and `react-dom` are not available at runtime. Cloud does not install `devDependencies` during deployment, so these packages must be in the `dependencies` section of your `package.json`.

To fix this, install them as production dependencies:
...
Otherwise, refer to the [Troubleshooting Medusa Emails on Cloud](../page.mdx#troubleshoot-email-delivery-issues-on-cloud) guide for help resolving common issues.
