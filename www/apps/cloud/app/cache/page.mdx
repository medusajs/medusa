...
Medusa Cache is a caching layer on the Medusa backend that improves your application's performance and reduces costs. It's available to all environments and plans out of the box at no additional cost.
...
Medusa Cache is built on the new [Caching Module](!resources!/infrastructure-modules/caching). The module has been integrated into [Query](!docs!/learn/fundamentals/query) so that `query.graph` calls can be cached easily.
