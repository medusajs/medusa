...
Replace `11.0.0` with your installed npm version (run `npm --version` to check). Turborepo requires this field when pruning with NPM workspaces. Without it, the build will fail with a `Missing 'packageManager' field` error.

<Note type="warning">

npm v10.9.x has a known crash in arborist dedupe that affects Medusa monorepos. Use npm v11 or later to avoid this issue.
