...
3. Click **Log Drains** in the sidebar.
...
- **Resource Attributes**: OpenTelemetry resource attribute key/value pairs attached to every exported log. Several Medusa environment attributes are pre-populated. You can add or clear rows. You can set the value to a static value, to an environment variable using the `${VAR_NAME}` syntax, or one of the following built-in variables:
  - `${_service_name}`: The name of the service that generated the log. For example, `medusa-server` for HTTP backend logs.
  - `${_service_version}`: The version of the service that generated the log. For example, for Medusa backend logs, this is the Medusa version.
  - `${_instance_id}`: The unique identifier of the service instance that generated the log. For example, for Medusa backend logs, this is the Medusa instance ID.
  ...
  - `${_project_handle}`: The unique handle of the project, which acts as an ID.
  - `${_project_name}`: The display name of the project. For example, `My Project`.
  - `${_environment_handle}`: The unique handle of the environment, which acts as an ID.
...
Click **Create log drain** to save the log drain. Afterwards, you must [redeploy](../../deployments/page.mdx#redeploy-a-deployment) the project's environments for the changes to take effect.
...
Once you save the log drain and deploy, you can start seeing logs in Sentry by going to **Explore** > **Logs** in the Sentry dashboard.
...
Once you save the log drain and deploy, you can start seeing logs in Datadog by going to **Logs** in the Datadog dashboard.
...
Once you save the log drain and deploy, go back to the OpenTelemetry setup page in Grafana. It should indicate that logs are being received. You can then view the logs in Grafana by going to **Explore** > **Logs**.
...
Once you save the log drain and deploy, you can start seeing logs in GCP by going to **Monitoring** > **Logs Explorer** in the GCP console.
...
The configured log drain for a project appears as a card on the **Log Drains** settings page, showing its name, provider, OTLP URL, log level, source toggles, header keys, and resource attribute keys.
...
After editing a log drain, you must [redeploy](../../deployments/page.mdx#redeploy-a-deployment) the project's environments for the changes to take effect.
