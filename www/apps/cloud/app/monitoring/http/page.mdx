...
In the [HTTP monitoring dashboard](#view-http-metrics), the Latency over Time chart shows the average latency of HTTP requests in milliseconds (ms) over time.

By monitoring this metric, you can ensure that your environment is responding to HTTP requests in a timely manner and identify potential issues in your application's request handling that may be causing increased latency.
...
In the [HTTP monitoring dashboard](#view-http-metrics), the Latency by Endpoint section shows a list of your application's endpoints, along with the average latency of HTTP requests they receive in the selected time range. Latency is measured in milliseconds (ms). The endpoints are listed in descending order, with the endpoints experiencing the highest latency at the top.
