...
- An index is only reachable through `POST /store/search`
  once a middleware allows it with `allowSearchIndexes`
  from `@medusajs/framework/http`. The route applies no
  scoping of its own, so narrowing a query to a sales
  channel or to published documents is another middleware
  on the route.
...
6. Allow every index the storefront searches on
   `/store/search` with the `allowSearchIndexes` middleware
   in `src/api/middlewares.ts`. An index the middleware
   doesn't name answers exactly like one that doesn't exist.
   The route applies no scoping of its own, so add a
   middleware of the project's own for a constraint such as
   a sales channel.
...
- Every index the storefront searches is named in an
  `allowSearchIndexes` middleware on `/store/search`.
...
An index isn't searchable through the route until a middleware allows it, so add the `allowSearchIndexes` middleware in `src/api/middlewares.ts`:
