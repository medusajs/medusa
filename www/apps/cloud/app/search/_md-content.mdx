...
9. Allow every index the storefront searches on
   ...
   in `src/api/middlewares.ts`, as shown in "The Store API
   search route". An index the middleware doesn't name
   answers exactly like one that doesn't exist. The route
   applies no scoping of its own, so add a middleware of the
   project's own for a constraint such as a sales channel.
