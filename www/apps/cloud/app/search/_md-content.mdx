...
- Medusa Search supports full-text search with per-field
  ...
  search is an Enterprise plan feature.
- Search requests are a metered resource. Requests past the
  plan's allowance count as Flex Usage.
</context>
...
- **Plan and usage.** The user checks their search request
  ...
  page in the Cloud dashboard, and upgrades to an Enterprise
  plan there if the project needs vector search.
</dashboard_handoffs>
...
- Do not use vector search without confirming the
  organization is on an Enterprise plan. Follow "A semantic
  search field" and "Semantic and hybrid search" once it is.
...
- If a search returns no results while the index is Ready,
  check that the query's fields are `searchable()` and that
  the products are published and in the request's sales
  channel, then report what you found.
