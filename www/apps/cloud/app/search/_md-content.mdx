...
- Search requests are a metered resource. Requests past the
  ...
  call costs one request for the hits, one for the count
  unless it passes `count: "none"`, one per value or range
  facet, and up to four per stats facet, so a page with many
  facets costs many requests.
