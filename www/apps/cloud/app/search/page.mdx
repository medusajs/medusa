...
Set the connection string as the `MEDUSA_CLOUD_SEARCH_ENDPOINT` environment variable in your local project, along with the handle of the environment you took it from:
...
Medusa reads both variables and registers Medusa Search as the default Search Module Provider, so you don't change `medusa-config.ts`. Your indexes then read from and write to your Cloud environment's search resources instead of your local PostgreSQL database.

<Note title="Warning" type="warning">

Don't also set `MEDUSA_CLOUD_API_KEY` for this. The connection string already carries the credentials, and passing them in both places throws an error when the provider initializes.
