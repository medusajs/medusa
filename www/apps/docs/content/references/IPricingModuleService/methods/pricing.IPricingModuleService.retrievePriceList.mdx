...
async function retrievePriceList(priceListId: string) {
  ...
  const priceList = await pricingService.retrievePriceList(priceListId)
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              type: "`string`",
              description:
                "The unique name used to later identify the rule\\_attribute. For example, it can be used in the `context` parameter of \nthe `calculatePrices` method to specify a rule for calculating the price.",
              optional: false,
              defaultValue: "",
              expandable: false,
              children: [],
