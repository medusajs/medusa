...
You can also set the following environment variables to configure Stripe's webhook functionality:

```bash
# The delay in milliseconds before processing the
# webhook event. By default, it's `5000` (5 seconds).
STRIPE_WEBHOOK_DELAY=3000
# The number of times to retry the webhook event processing
# in case of an error. By default, it's `3`.
STRIPE_WEBHOOK_RETRIES=5
```
