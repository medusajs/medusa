...
When a product variant is created and its `manage_inventory` attribute's value is `true`, the Medusa application uses the Inventory Module to create an inventory item associated with that product variant.
...
When an item in an order is fulfilled and the associated variant has its `manage_inventory` attribute set to `true`, the Medusa application uses the Inventory Module to:
...
When an item in an order is returned and the associated variant has its `manage_inventory` attribute set to `true`, the Medusa application uses the Inventor Module to increment the `stocked_quantity` of the inventory level associated with the inventory item with the returned quantity.
