...
In this document, you’ll learn about concepts of item fulfillment.
...
The fulfillment is also associated with a shipping option of that provider, which determines the shipping details.
...
The `Fulfillment` data model has a `data` attribute that holds any necessary data for the associated fulfillment provider to process the fulfillment.

For example, the `data` attribute can hold the ID of the fulfillment in the third-party service. The fulfillment provider then uses it whenever it retrieves the fulfillment’s details. 
...
The `Fulfillment` data model has three attributes to keep track of the current status of the fulfillment:

- `packed_at`: The date the fulfillment was packed.
- `shipped_at`: The date the fulfillment was shipped.
- `delivered_at`: The date the fulfillment was delivered.
