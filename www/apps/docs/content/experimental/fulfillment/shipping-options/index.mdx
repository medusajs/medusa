...
These rules are represented by the `ShippingOptionRule` data model. Its attributes define the custom rule:
...
- `operator`: The operator used in the condition. For example:
    ...
    - Check out more operators in [this reference](../../../references/fulfillment/types/fulfillment.RuleOperatorType.mdx).
