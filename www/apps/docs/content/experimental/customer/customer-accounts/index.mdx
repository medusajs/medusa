...
The `Customer` data model has a `has_account` attribute, which is a boolean that indicates whether a customer is registerd.
...
The above behavior means that two `Customer` records may exist of the same email. However, the main difference is the `has_account` attribute’s value.
