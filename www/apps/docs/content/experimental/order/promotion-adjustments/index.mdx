...
The `amount` attribute of the adjustment line indicates the amount to be discounted from the original amount. Also, the ID of the applied promotion can be stored in the `promotion_id` attribute of the adjustment line.
...
Learn more about the `computeActions` method in [this reference](../../../references/promotion/IPromotionModuleService/methods/promotion.IPromotionModuleService.computeActions.mdx).
...
The `LineItem` data model has an `is_discountable` attribute that indicates whether promotions can be applied to the line item. It’s enabled by default.
