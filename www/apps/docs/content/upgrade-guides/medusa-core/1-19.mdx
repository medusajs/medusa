...
In previous versions, if you attempt to complete a cart that's already completed, it resulted in a response error with code `409`.

In v1.19 of the Medusa backend, the order or swap associated with the completed cart is returned instead, with no response error:
