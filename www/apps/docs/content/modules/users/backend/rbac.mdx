...
After defining the middleware, you apply it on all endpoints starting with `/admin/`.
...
This creates the Create Role endpoint at `/admin/roles` that uses the `RoleService` to create a new role. Notice that validation of received body parameters is omitted for simplicity.
...
This creates the Associate User endpoint at `/admin/roles/[id]/user/[user_id]` that uses the `RoleService` to associate a role with a user.

To test the endpoints out, run the `build` command in the root directory of your Medusa backend project:
