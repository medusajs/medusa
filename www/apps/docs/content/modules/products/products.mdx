...
- `manage_inventory`: A boolean value indicating whether Medusa should handle the management of the product variant’s inventory. This would allow you to handle your inventory data more accurately across different locations. Learn more in [the Inventory Module documentation](../multiwarehouse/inventory-module.md).
- `inventory_items`: A relation that gives you access to the inventory items of a variant when `manage_inventory` is enabled. Learn more in [the Inventory Module documentation](../multiwarehouse/inventory-module.md).
