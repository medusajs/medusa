...
You can now create the endpoints. Create the file `src/api/admin/product-media/route.ts` with the following content:
...
- a `GET` endpoint at `/admin/product-media` that retrieves a list of product media records that have the type `main`. It also retrieves the relations `variant` and `variant.product` to access the product media’s variant and main product.
- a `POST` endpoint at `/admin/product-media` that accepts the necessary data to create a product media, then returns the created product media.
...
Then, add the `requireCustomerAuthentication` middleware to this endpoint in `src/api/middlewares.ts`:
...
This ensures that only logged-in customers can access this endpoint.
