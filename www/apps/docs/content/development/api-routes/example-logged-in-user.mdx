...
const registerLoggedInUser = async (
  ...
  if (req.headers["x-medusa-access-token"] && typeof req.headers["x-medusa-access-token"] === "string") {
    const userService = req.scope.resolve("userService") as UserService;
    loggedInUser = await userService.retrieveByApiToken(req.headers["x-medusa-access-token"]);
  }
