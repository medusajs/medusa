...
<Tabs groupId="execute-type" isCodeTabs={true}>
...
const updateProduct = createStep(
  ...
  async (input: WorkflowInput, context) => {
    ...
    const previousProductData = await productService.retrieve(id, {
    ...
    const productService: ProductService = context.container.resolve(
      "productService"
    )
