...
This creates a `registerLoggedInUser` middleware that handles registering the currently logged-in user in the dependency container.

It then applies this middleware on the `/admin/products` endpoint. You can change that to be the route of any other endpoint or a pattern that matches multiple routes.
