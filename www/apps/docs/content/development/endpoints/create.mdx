...
By default, endpoint routes prefixed by `/store` don't require customers to be authenticated to access the endpoint. However, you can still access the logged-in customer's ID by accessing the Request object's `user.customer_id`, which will be `undefined` if the customer is not logged in.
...
Routes prefixed by `/store/me`, on the other hand, require customers to be authenticated to access the endpoint. You can access the logged-in customer's ID by accessing the Request object's `user.customer_id`.

If you want to disable authentication requirement on your custom route prefixed with `/store/me`, export an `AUTHENTICATE` variable in your route file with its value set to `false`. For example:
...
Note that this will disable authentication requirement on all endpoint methods defined in the same file.

### Protect Admin Routes

By default, all endpoint routes prefixed by `/admin` require an admin user to be authenticated to access the endpoint. You can access the logged-in user's ID in the endpoint by accessing the Request object's `user.userId`.
...
If you want to disable authentication requirement on an admin route, export an `AUTHENTICATE` variable in your route file with its value set to `false`.
...
If you want to protect routes that aren't prefixed with `/store` or `/admin`, you can use one of the following middlewares defined in `@medusajs/medusa` for authenticating customers or users:

- `authenticate`: this middleware ensures that only authenticated admin users can access an endpoint. You can access the user's ID in the endpoint by accessing the Request object's `user.userId`. 
- `authenticateCustomer`: this middleware doesn't require a customer to be authenticated, but if a customer is logged in, it attaches their ID to the Request object passed to the endpoint under `user.customer_id`.
- `requireCustomerAuthentication`: this middleware ensures that only authenticated customers can access an endpoint. You can access the customer's ID in the endpoint by accessing the Request object's `user.customer_id`.
...
Medusa provides an `errorHandler` middleware that you can use on your custom endpoints to that your error handling is consistent across your Medusa backend. You can also create custom [middlewares](./add-middleware-express-route.mdx) to handle errors.

To handle errors using Medusa's middlewares, first, import the `errorHandler` middleware from `@medusajs/medusa` and apply it on your routes. For example:
...
Then, wrap the function handler of every endpoint method with the `wrapHandler` middleware imported from `@medusajs/medusa`. For example:
...
Now all errors thrown in your custom endpoints, or in resources you use within your endpoint such as services, will be caught and returned to the user.
...
The endpoint returns the following object error in the response:
...
You can retrieve [services](../services/create-service.mdx) in your endpoint using `req.scope.resolve` passing it the service’s registration name in the [dependency container](../fundamentals/dependency-injection.md). Services are registered as their camel-case name. So, for example, if you have a `PostService`, it's registered as `postService`.
...
Files and directories prefixed with `_` are ignored. This can be helpful if you want to implement endpoint methods in different files, then reference them in your `route.ts` file.
