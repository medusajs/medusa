...
<Note title="Restrict user permissions">

By default, all admin users have full access to the Medusa Admin. To restrict what a user can see and do, your developer must first enable role-based access control. You can then manage roles and their permissions from Settings → Roles. Refer to the [Role-Based Access Control](!resources!/commerce-modules/user/rbac) guide for details.
...
This feature is available since [Medusa v2.20.0](https://github.com/medusajs/medusa/releases/tag/v2.20.0). The option only appears for users that authenticate with an email and password. If you don't find the option to copy a reset password link, make sure to [update your Medusa application](!docs!/learn/update).

</Note>

To copy the reset password link for a user:

1. Go to the user's details page.
2. Click the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon in the first section's header.
3. Choose "Copy reset password link" from the dropdown.
