...
5. In the side window that opens, fill out the following fields:
    - **Settlement type:** If you have the [Loyalty Plugin](!resources!/commerce-modules/loyalty#1-install-the-loyalty-plugin) installed, you can choose how to refund the payment:
        ...
        - **Store credit:** Refund the payment to the customer's [store credit account](../../loyalty/store-credits/page.mdx). The customer can then use the store credit balance for future purchases. You can only choose this option if the customer is a registered customer and the order has an outstanding amount to settle, such as after creating a return or making an order edit.
