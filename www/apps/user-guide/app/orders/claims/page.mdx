...
4. In the form that opens:
    - To add items to be returned:
        ...
        - You can then change the quantity to return or manage the item's details by clicking the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon next to it. When managing item details, you can also add custom metadata fields to store additional information about the claim item.
...
## Manage Claim Item Metadata

<Note>

This feature is available since Medusa [v2.13.7](https://github.com/medusajs/medusa/releases/tag/v2.13.7).

</Note>

When adding items to a claim (both inbound items for returns and outbound items for replacements), you can add custom metadata to store additional information about specific claim items.

To manage claim item metadata:

1. When creating or managing a claim, add your items as described above.
2. Click the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon next to a claim item.
3. Choose "Edit" or "Manage details" from the dropdown.
4. In the form that opens, you'll find a "Metadata" section where you can:
   - Add custom key-value pairs to store additional information
   - Edit existing metadata entries
   - Remove metadata entries you no longer need

This metadata is stored with the claim item and can be useful for tracking special handling instructions, internal notes, or integration with external systems.
