...
<PermissionsBadge permissions={["order:read"]} />
...
<PermissionsBadge permissions={["order:update"]} />
...
4. In the form that opens:
    - To add an item, click the <InlineIcon Icon={Plus} alt="plus" /> button in the "Items" section's header. From the dropdown, you can either choose:
        - **Add items**: Add existing product variants to the draft order. This will open the product variant selection window. The table shows each variant's **Stock** column, which displays the available inventory quantity and number of stock locations, so you can check availability before adding a variant.
...
<PermissionsBadge permissions={["order:delete"]} />
