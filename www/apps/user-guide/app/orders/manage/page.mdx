...
You can transfer the ownership of an order to another registered customer. This changes the customer associated with the order and sends notifications for order updates to the new customer. You can also optionally update the order's email address to match the new customer's email.
...
4. In the side window that opens:
   1. Choose the new customer from the dropdown under the "New owner" section.
   2. Optionally, enable the "Update order email" toggle to change the order's email address to match the new customer's email address.
5. Once you're done, click on the Save button.
