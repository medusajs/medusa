...
This feature is available since [Medusa v2.21.1](https://github.com/medusajs/medusa/releases/tag/v2.21.1). If you don't see the Debit the account option in your Medusa Admin, request your technical team to [upgrade your Medusa application](!docs!/learn/update).

</Note>

To remove funds from a store credit account manually:

1. Go to the store credit account's details page.
2. Click the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon at the top end of the first section.
3. Choose **Debit the account** from the dropdown.
4. In the side window that opens, enter the following:
    - **Amount**: The amount to remove from the account. Must be greater than 0 and cannot exceed the current balance.
    - **Note**: Optionally add a note describing the reason for the debit.
5. Click the **Save** button.

The debited amount will appear in the account's transaction history and the current balance will update accordingly.

<Note title="Tip">

The **Debit the account** option is disabled when the account has no balance.
...
You can also create a new store credit account for the customer directly from this section. Click the **Create** button in the **Store Credit Accounts** section header. The creation form opens with the customer already pre-selected.
