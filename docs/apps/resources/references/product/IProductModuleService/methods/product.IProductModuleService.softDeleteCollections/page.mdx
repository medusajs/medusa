...
This method is used to delete product collections. Unlike the [deleteCollections](../product.IProductModuleService.deleteCollections/page.mdx) method, this method won't completely remove the collection. It can still be accessed or retrieved using methods like retrieveCollections if you pass the `withDeleted` property to the `config` object parameter.

The soft-deleted collections can be restored using the [restoreCollections](../product.IProductModuleService.restoreCollections/page.mdx) method.
