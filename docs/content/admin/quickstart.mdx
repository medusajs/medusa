...
1. Served alongside the Medusa backend: with this approach, the admin dashboard starts when you start the Medusa backend. This also means you can later deploy the Medusa backend along with the admin dashboard on the same hosting.
2. Served separately from the Medusa backend: with this approach, the admin dashboard starts separately from the Medusa backend. You still need the Medusa backend to be running as the admin uses its APIs. This is useful if you intend to later deploy the admin dashboard on a different hosting than the Medusa Backend, such as using Vercel.
...
- `dev:admin`: Used to run the development server of the admin. You can learn about other available options in [this section](#dev-command-options).
