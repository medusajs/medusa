...
This starts the Medusa Backend and the admin dashboard. By default, the admin will be available on the URL `localhost:7001`.
...
When you run the backend using the `develop` command:

```bash
npx @medusajs/medusa-cli develop
```

The admin dashboard run on `localhost:7001`.

However, when you run the backend using the `start` command, which is typically used in production:

```bash
npx @medusajs/medusa-cli start
```

The admin dashboard runs on `localhost:9000/app` by default. If you've changed the value of the `path` option, then it would be `localhost:9000/[path]`, where `[path]` is the path you've specified.
