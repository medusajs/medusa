...
Where `<YOUR_DATABASE_URL>` is the connection URL of your database. For example, `postgres://localhost/medusa-store`. You can learn about its format and other configurations [here](./development/backend/configurations.md).
...
Then, run the following command to migrate the schema to your database:
...
If you open your storefront now in your browser, everything should be working as expected. If you're not seeing any products, you can run the `seed` command as instructed earlier or install the [Medusa admin plugin](./admin/quickstart.mdx) to start adding products.
