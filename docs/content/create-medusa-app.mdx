...
Medusa provides developers with the toolkit to create a commerce application including a Node.js backend that encapsulates the core package and the commerce modules, an admin dashboard, and a storefront. Developers can set up and use these tools separately, giving them a full ownership over their tech stack.

In certain cases, it can make more sense to setup all the main building blocks together, such as when you're creating a fully fledged ecommerce store, or if you're curious about how Medusa works and what features it offers. For these cases, you can use the `create-medusa-app` command.

When you run the `create-medusa-app` command, you’ll install a Medusa backend, and optionally a Medusa admin and a storefront at the same time.

:::note

If you instead want to quickly install and setup only a Medusa backend, follow [this quickstart guide](./development/backend/install.mdx) instead.
