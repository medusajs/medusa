...
If the customer doesn’t receive an email after this request, make sure that you’ve set up a Notification provider like [SendGrid](../../../add-plugins/sendgrid.mdx) successfully. You also need to add a subscriber that handles the [customer.password_reset](../../../advanced/backend/subscribers/events-list.md#customer-events) event and sends the email.
...
- [Implement order-claim flow](../../../advanced/storefront/implement-claim-order.mdx)
