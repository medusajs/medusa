...
Where `session_data` would include the previously returned data from these methods. `update_requests` allows you to request from the core to update data of internal resources. Currently, it can only be used to update the `metadata` field of the customer entity.
