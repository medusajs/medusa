...
const BlogPost = () => {
  ...
  const { data: { post }, isLoading } = useAdminCustomQuery<AdminBlogPostQuery, AdminBlogPostRes>(
    `/blog/posts/${id}`,  // path
...
const AuthorsBlogPosts = () => {
  ...
  const { data: { posts }, isLoading } = useAdminCustomQuery<AdminBlogPostsQuery, AdminBlogPostsRes>(
    `/blog/posts`,  // path
...
const BlogPost = () => {
  ...
  const { mutate, isLoading } = useAdminCustomDelete<AdminBlogPostDeleteRes>(
    `/blog/posts/${id}`,  // path
    ["blog-posts"], // queryKey - we want to refetch the previous list queries
    {
      product: true
    },
   )
