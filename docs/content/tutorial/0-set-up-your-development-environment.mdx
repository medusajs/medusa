...
If you have Docker installed, you can quickly spin up a PostgreSQL instance with the following command:
...
`-e` creates environment variables POSTGRES_USER, POSTGRES_PASSWORD and POSTGRES_DB. These will be used to set up a database named `medusa-docker` with username and password being `postgres`.

`-p` maps the container port 5432 to the external host 5432. This allows you to connect to the database server from the outside.

`-d` enables Docker to run the container in the background.

The last seciton of the command, `postgres` grabs the latest postgres image from the Docker hub.
