...
### Adding aws config
Medusa also supports adding additional aws configurations by adding an option `aws_config_object` where you can also define `region`, `access_key_id` or `secret_access_key` in addition to any other properties defined [here](https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/Config.html): 
