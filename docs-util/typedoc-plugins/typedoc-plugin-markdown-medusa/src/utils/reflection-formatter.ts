import { Comment, DeclarationReflection, ParameterReflection, ReflectionType, TypeParameterReflection, SomeType } from "typedoc";
import * as Handlebars from 'handlebars';
import { stripLineBreaks } from "../utils";

type ReflectionParameterType = ParameterReflection | DeclarationReflection | TypeParameterReflection

const MAX_LEVEL = 3

export default function reflectionFomatter(reflection: ReflectionParameterType, level = 1) {
  if(reflection.name === "filters.q") {
    console.log(typeof reflection)
  }
  const prefix = `${Array(level - 1).fill("\t").join("")}-`
  let item = `${prefix} \`${reflection.name}\`: `
    const defaultValue = getDefaultValue(reflection)

    if (defaultValue || reflection.flags.isOptional) {
      item += `(${
        reflection.flags.isOptional ? "optional" : ""
      }${
