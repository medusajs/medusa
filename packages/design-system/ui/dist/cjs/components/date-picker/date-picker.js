...
const displayVariants = (0, cva_1.cva)({
    base: (0, clx_1.clx)("text-ui-fg-base bg-ui-bg-field transition-fg shadow-buttons-neutral flex w-full items-center gap-x-2 rounded-md outline-none", "hover:bg-ui-bg-field-hover", "focus-visible:shadow-borders-interactive-with-active data-[state=open]:shadow-borders-interactive-with-active", "disabled:bg-ui-bg-disabled disabled:text-ui-fg-disabled disabled:shadow-buttons-neutral", "aria-[invalid=true]:!shadow-borders-error"),
    variants: {
        size: {
...
currentValue, }) => {
    ...
    return (React.createElement("ul", { className: "flex flex-col items-start" }, presets.map((preset, index) => {
        return (React.createElement("li", { key: index, className: "w-full" },
            React.createElement("button", { className: (0, clx_1.clx)("txt-compact-small-plus w-full overflow-hidden text-ellipsis whitespace-nowrap rounded-md p-2 text-left", "text-ui-fg-subtle hover:bg-ui-bg-base-hover outline-none transition-all", "focus-visible:bg-ui-bg-base-hover", {
...
const formatDate = (date, includeTime) => {
    const usesAmPm = !(0, is_browser_locale_hour_cycle_24h_1.isBrowserLocaleClockType24h)();
    if (includeTime) {
        if (usesAmPm) {
            return (0, date_fns_1.format)(date, "MMM d, yyyy h:mm a");
        }
        return (0, date_fns_1.format)(date, "MMM d, yyyy HH:mm");
    }
            ...
            if ("date" in preset) {
                ...
                if (toDay) {
                    ...
                    if (presetDay > toDay.getDate()) {
                        throw new Error(`Preset ${preset.label} is after toDay ${(0, date_fns_1.format)(toDay, "MMM dd, yyyy")}.`);
                    }
                }
            }
            if ("dateRange" in preset) {
                ...
                if (fromMonth) {
                    ...
                    if (presetMonth && presetMonth < fromMonth.getMonth()) {
                        throw new Error(`Preset ${preset.label}'s 'from' is before fromMonth ${(0, date_fns_1.format)(fromMonth, "MMM, yyyy")}.`);
                    }
                }
                if (toMonth) {
                    const presetMonth = (_d = preset.dateRange.to) === null || _d === void 0 ? void 0 : _d.getMonth();
                    if (presetMonth && presetMonth > toMonth.getMonth()) {
                        throw new Error(`Preset ${preset.label}'s 'to' is after toMonth ${(0, date_fns_1.format)(toMonth, "MMM, yyyy")}.`);
                    }
                }
                if (fromDay) {
                    const presetDay = (_e = preset.dateRange.from) === null || _e === void 0 ? void 0 : _e.getDate();
                    if (presetDay && presetDay < fromDay.getDate()) {
                        throw new Error(`Preset ${preset.dateRange.from}'s 'from' is before fromDay ${(0, date_fns_1.format)(fromDay, "MMM dd, yyyy")}.`);
                    }
                }
                if (toDay) {
                    const presetDay = (_f = preset.dateRange.to) === null || _f === void 0 ? void 0 : _f.getDate();
                    if (presetDay && presetDay > toDay.getDate()) {
                        throw new Error(`Preset ${preset.label}'s 'to' is after toDay ${(0, date_fns_1.format)(toDay, "MMM dd, yyyy")}.`);
                    }
                }
            }
        });
