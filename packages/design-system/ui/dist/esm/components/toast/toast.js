...
dismissLabel = "Close", }) => {
    ...
    return (React.createElement("div", { className: "shadow-elevation-flyout bg-ui-bg-base flex w-fit min-w-[360px] max-w-[440px] overflow-hidden rounded-lg" },
        React.createElement("div", { className: clx("grid flex-1 items-start gap-3 py-3 pl-3", {
        ...
        hasActionables && (React.createElement("div", { className: clx("grid grid-cols-1", {
                "grid-rows-2": !!action && onDismiss,
                "grid-rows-1": !action || !onDismiss,
            }) },
            !!action && (React.createElement("button", { type: "button", className: clx("txt-compact-small-plus text-ui-fg-base bg-ui-bg-base hover:bg-ui-bg-base-hover active:bg-ui-bg-base-pressed flex items-center justify-center px-4 transition-colors", {
                    "border-ui-border-base border-b": onDismiss,
                    "text-ui-fg-error": action.variant === "destructive",
                }), onClick: action.onClick }, action.label)),
            onDismiss && (React.createElement("button", { type: "button", onClick: () => onDismiss(id), className: clx("txt-compact-small-plus text-ui-fg-subtle bg-ui-bg-base hover:bg-ui-bg-base-hover active:bg-ui-bg-base-pressed flex items-center justify-center px-4 transition-colors") }, dismissLabel))))));
};
