...
export class Migration20230929122253 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "price_set_money_amount" ("id" text not null, "title" text not null, "price_set_id" text not null, "money_amount_id" text not null, "number_rules" integer not null default 0, constraint "price_set_money_amount_pkey" primary key ("id"));');
    this.addSql('create index "IDX_price_set_money_amount_price_set_id" on "price_set_money_amount" ("price_set_id");');
    this.addSql('create index "IDX_price_set_money_amount_money_amount_id" on "price_set_money_amount" ("money_amount_id");');

    this.addSql('create table "rule_type" ("id" text not null, "name" text not null, "rule_attribute" text not null, "default_priority" integer not null default 0, constraint "rule_type_pkey" primary key ("id"));');
    ...
    this.addSql('create index "IDX_price_rule_price_set_id" on "price_rule" ("price_set_id");');
    this.addSql('create index "IDX_price_rule_rule_type_id" on "price_rule" ("rule_type_id");');
    this.addSql('create index "IDX_price_rule_price_set_money_amount_id" on "price_rule" ("price_set_money_amount_id");');
    ...
    this.addSql('alter table "price_set_money_amount_rules" add constraint "price_set_money_amount_rules_price_set_money_amount_id_foreign" foreign key ("price_set_money_amount_id") references "price_set_money_amount" ("id") on update cascade on delete cascade;');
