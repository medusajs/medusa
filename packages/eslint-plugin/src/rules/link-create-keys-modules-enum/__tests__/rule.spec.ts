...
ruleTester.run("link-create-keys-modules-enum", rule, {
  ...
  invalid: [
    // Identifier key for a known module → autofix + add import.
    {
      ...
      output: `import { Modules } from "@zjedene-medusa/framework/utils"
...
await link.create({ [Modules.PRODUCT]: { product_id: "1" } })`,
    ...
    // Existing framework/utils import → append `Modules` to specifier list.
    {
      ...
      errors: [{ messageId: "preferEnumKey" }],
      output: `
        import { defineLink, Modules } from "@zjedene-medusa/framework/utils"
        await link.create({ [Modules.PRODUCT]: { product_id: "1" } })
      `,
    },
    ...
    {
      code: `
        import { Modules as M } from "@zjedene-medusa/framework/utils"
    ...
    // Workflow step: createRemoteLinkStep.
    {
      code: `
        import { Modules } from "@zjedene-medusa/framework/utils"
        import { createRemoteLinkStep } from "@zjedene-medusa/medusa/core-flows"
    ...
    // Workflow step alias.
    {
      code: `
        import { Modules } from "@zjedene-medusa/framework/utils"
        import { dismissRemoteLinkStep as drls } from "@zjedene-medusa/medusa/core-flows"
        drls({ order: { id: "1" } })
      `,
      errors: [{ messageId: "preferEnumKey" }],
      output: `
