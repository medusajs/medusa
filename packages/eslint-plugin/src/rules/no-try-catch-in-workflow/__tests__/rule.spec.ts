...
ruleTester.run("no-try-catch-in-workflow", rule, {
  valid: [
    ...
    {
      code: `
        import { createWorkflow, createStep } from "@zjedene-medusa/framework/workflows-sdk"
        createWorkflow("my-workflow", (input) => {
    ...
    {
      code: `
        import { createWorkflow, transform } from "@zjedene-medusa/framework/workflows-sdk"
        createWorkflow("my-workflow", (input) => {
    ...
    {
      code: `
        import { createWorkflow, createStep, when } from "@zjedene-medusa/framework/workflows-sdk"
        createWorkflow("my-workflow", (input) => {
  ...
  invalid: [
    ...
    {
      code: `
        import { createWorkflow as cw } from "@zjedene-medusa/framework/workflows-sdk"
    ...
    {
      code: `
        import { createWorkflow, when } from "@zjedene-medusa/framework/workflows-sdk"
        createWorkflow("my-workflow", (input) => {
          when({ input }, (data) => data.input.foo).then(() => {
            try {
              return 1
    ...
    {
      code: `
        import { createWorkflow, when as w } from "@zjedene-medusa/framework/workflows-sdk"
        createWorkflow("my-workflow", (input) => {
