...
ruleTester.run("prefer-link-over-remote-link", rule, {
  valid: [
    ...
    {
      code: `import type { Link } from "@zjedene-medusa/framework/types"`,
    },
  ...
  invalid: [
    ...
    // Type import: RemoteLink from @zjedene-medusa/framework/types.
    {
      code: `import type { RemoteLink } from "@zjedene-medusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link } from "@zjedene-medusa/framework/types"`,
    },
    // Type import: IRemoteLink from @zjedene-medusa/types.
    {
      code: `import { IRemoteLink } from "@zjedene-medusa/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import { ILink } from "@zjedene-medusa/types"`,
    },
    // Mixed: rename only the deprecated specifier.
    {
      code: `import type { Link, RemoteLink } from "@zjedene-medusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link, Link } from "@zjedene-medusa/framework/types"`,
    },
    // Aliased import — rename only the `imported` part, keep alias.
    {
      code: `import type { RemoteLink as MyLink } from "@zjedene-medusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link as MyLink } from "@zjedene-medusa/framework/types"`,
    },
    // Multiple violations in one file.
    {
      code: `
        import { RemoteLink } from "@zjedene-medusa/framework/types"
      ...
      output: `
        import { Link } from "@zjedene-medusa/framework/types"
