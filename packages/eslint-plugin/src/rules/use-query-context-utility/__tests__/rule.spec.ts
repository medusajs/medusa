...
ruleTester.run("use-query-context-utility", rule, {
  valid: [
    ...
    {
      code: `
        import { QueryContext } from "@zjedene-medusa/framework/utils"
        query.graph({
          entity: "product",
          fields: ["*"],
    ...
    {
      code: `
        import { QueryContext as QC } from "@zjedene-medusa/framework/utils"
  ...
  invalid: [
    // Bare object context on first arg — no existing import.
    {
      ...
      output: `import { QueryContext } from "@zjedene-medusa/framework/utils"\nquery.graph({ entity: "product", context: QueryContext({ region_id: "us" }) })`,
    },
...
query.graph({ context: { region_id: "us" } })
      ...
      errors: [{ messageId: "useQueryContext" }],
      output: `
import { Modules, QueryContext } from "@zjedene-medusa/framework/utils"
