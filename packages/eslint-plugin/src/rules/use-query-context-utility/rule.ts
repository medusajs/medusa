...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Wrap `context` arguments to `query.graph` / `query.index` with `QueryContext(...)` from `@zjedene-medusa/framework/utils`.",
    },
    fixable: "code",
    messages: {
      useQueryContext:
        "Wrap the `context` value with `QueryContext(...)` from `@zjedene-medusa/framework/utils`.",
    },
    schema: [],
  },
  defaultOptions: [],
