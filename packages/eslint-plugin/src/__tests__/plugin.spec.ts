...
describe("@zjedene-medusa/eslint-plugin", () => {
  it("exposes a meta block with the plugin name", () => {
    expect(plugin.meta).toEqual({ name: "@zjedene-medusa/eslint-plugin" })
  })
  ...
  it("recommended preset includes a global-ignores block and a base TS block", () => {
    ...
    expect((baseBlock!.plugins as Record<string, unknown>)["@zjedene-medusa"]).toBe(plugin)
  })
})
