  ...
  testSuite: ({
    ...
    describe("Promotion Service: computeActions", () => {
      ...
      describe("when adjustments are present in the context", () => {
        ...
        // Custom, calculated adjustments are written without a code (the column
        // is nullable). They don't belong to any promotion, so computeActions
        // must ignore them entirely: no `removeItemAdjustment` /
        // `removeShippingMethodAdjustment` is emitted for them, which is what
        // keeps them on the cart across promotion re-computations.
