...
export class Migration20260724031923 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "rbac_role_assignment" drop constraint if exists "rbac_role_assignment_role_id_reference_reference_id_unique";`);
    this.addSql(`create table if not exists "rbac_role_assignment" ("id" text not null, "role_id" text not null, "reference" text not null, "reference_id" text not null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "rbac_role_assignment_pkey" primary key ("id"));`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_rbac_role_assignment_role_id_reference_reference_id_unique" ON "rbac_role_assignment" ("role_id", "reference", "reference_id") WHERE deleted_at IS NULL;`);
