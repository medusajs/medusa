...
export default class RedisEventBusService extends AbstractEventBusModuleService {
  ...
  async emit<T = unknown>(
    ...
    for (const [groupId, events] of groupEventsMap.entries()) {
      ...
      // pipeline as the push that creates it, so it is never a no-op against a key that
      // does not exist yet. This helps prevent stale data from staying in redis for too
      // long in the event the module fails to cleanup events. For long running workflows,
      // setting a much higher TTL or even skipping the TTL would be required
  ...
  async clearGroupedEvents(
    ...
    if (eventNames?.length) {
      ...
      // Add the remaining events to the list, if any - an RPUSH with no members is
      // a Redis error, and the list is already empty from the del above
