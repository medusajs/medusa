...
export default class RedisEventBusService extends AbstractEventBusModuleService {
  ...
  async emit<T = unknown>(
    ...
    for (const [groupId, events] of groupEventsMap.entries()) {
      ...
      // The TTL for the key of the list that is scoped to a group is set in the same
      // pipeline as the push that creates the key. This will be helpful in preventing
      // stale data from staying in redis for too long in the event the module fails to
      // cleanup events. For long running workflows, setting a much higher TTL or even
      // skipping the TTL would be required
      promises.push(
        this.groupEvents(groupId, eventsData, groupedEventsTTL)
      )
    }
  ...
  async clearGroupedEvents(
    ...
    if (eventNames?.length) {
      ...
      // Add the remaining events to the list. RPUSH requires at least one member, and
      // when every staged event matched eventNames there is nothing left to keep.
