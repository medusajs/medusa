...
calculate_order_change_1.OrderChangeProcessing.registerActionType(action_key_1.ChangeActionType.RECEIVE_DAMAGED_RETURN_ITEM, {
    isDeduction: true,
    ...
    operation({ action, currentOrder, previousEvents }) {
        ...
        (_a = existing.detail).return_dismissed_quantity ?? (_a.return_dismissed_quantity = 0);
    ...
    revert({ action, currentOrder, previousEvents }) {
        const existing = currentOrder.items.find((item) => item.id === action.details.reference_id);
        existing.detail.return_dismissed_quantity = utils_1.MathBN.sub(existing.detail.return_dismissed_quantity, action.details.quantity);
