...
import { ClaimItem, OrderClaim, Return } from "@models"
...
function createClaimItems(em, data, claimReference, returnReference, actions) {
  return data.claim_items?.map((item) => {
    actions.push({
...
async function processReturnShipping(
  ...
  if (data.return_shipping && data.type === ClaimType.REPLACE) {
...
export async function createClaim(
  ...
  claimReference.claim_items = createClaimItems(
    em,
    data,
    claimReference,
    returnReference,
