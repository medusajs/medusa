...
export class Migration20260616075929 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table if not exists "auth_verification" ("id" text not null, "auth_identity_id" text not null, "entity_id" text not null, "type" text not null, "provider" text not null, "verified_at" timestamptz null, "requested_at" timestamptz not null, "provider_metadata" jsonb null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "auth_verification_pkey" primary key ("id"));`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_auth_verification_unique_auth_identity_entity_id_type" ON "auth_verification" ("auth_identity_id", "entity_id", "type") WHERE deleted_at IS NULL;`);
