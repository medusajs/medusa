...
import ProductStatusSelector from "../../../../../components/molecules/product-status-selector"
...
const ProductDetails = ({ product }: Props) => (
  <div className="flex flex-col gap-y-3 mt-8">
    ...
    <Detail
      title="Default Price"
      value={
        (product.default_price || []).length > 0
          ? uniqBy(product.default_price, "amount")
    ...
    />
    <Detail title="Tax Category" value={product.tax_category?.name} />
    <Detail title="Type" value={product.type?.value} />
    <Detail title="Collection" value={product.collection?.title} />
    <Detail title="Discountable" value={product.discountable ? "Yes" : "No"} />
    <Detail title="Customer Prompt" value={product.customer_response_prompt} />
    <Detail title="Shipping Profile" value={product.profile.name} />
  </div>
)
...
const ProductTags = ({ product }: Props) => {
  ...
  return (
    <ul className="flex flex-wrap items-center gap-x-1 mt-4">
      {product.tags.map((t) => (
        <li key={t.id} className="m-1">
