...
const isRow = (element: HTMLElement | null): element is HTMLTableRowElement => {
  return !!element && element.nodeName === "TR"
}

/**
 * When using drag-to-fill we only support filling in horizontal or vertical
 * directions. This function attempts to find the nearest valid td to the
 * current target cell, that is within the accepted row and column range.
 * @param start - The start point
 * @param acceptedColumns - The accepted columns
 * @param acceptedRows - The accepted rows
 * @returns The nearest valid td or `null` if no valid td is found
 */
export const findNearestValidTd = (
  start: CellWithParentRow,
  acceptedColumns: number[],
  acceptedRows: number[]
) => {
  const previousSibling = start.previousElementSibling as HTMLElement | null

  if (previousSibling && isParentRow(previousSibling)) {
    const isColumnAccepted = acceptedColumns.includes(previousSibling.cellIndex)
  ...
  const rowSibling = start.parentElement
    .previousElementSibling as HTMLElement | null

  if (rowSibling && isRow(rowSibling)) {
    const aboveTd = rowSibling.children[start.cellIndex] as HTMLElement | null

    if (aboveTd && isParentRow(aboveTd)) {
      const isRowAccepted = acceptedRows.includes(rowSibling.rowIndex)
  ...
  const nextSibling = start.nextElementSibling as HTMLElement | null

  if (nextSibling && isParentRow(nextSibling)) {
    const isColumnAccepted = acceptedColumns.includes(nextSibling.cellIndex)
...
export const validateFillTarget = (
  target: HTMLElement | null,
  start: Point,
  end: Point
) => {
  ...
  if (!cell || !isNestedInRow) {
    return null
  }

  const targetCell = cell

  const acceptedRows = generateRangeArray(start.row, end.row)
  const acceptedCols = generateRangeArray(start.col, end.col)

  const isInAcceptedRowRange = acceptedRows.includes(
    targetCell.parentElement.rowIndex
  )
  const isInAcceptedColRange = acceptedCols.includes(targetCell.cellIndex)

  const isEditable = targetCell.getAttribute(EDITABLE_ATTRIBUTE) === "true"

  const isValid = isEditable && (isInAcceptedRowRange || isInAcceptedColRange)

  if (!isValid) {
    const nearestValidTd = findNearestValidTd(
      targetCell,
      acceptedCols,
      acceptedRows
    )
...
export const validateMouseOverTarget = (
  ...
  const isNestedInRow = isParentRow(targetCell)
