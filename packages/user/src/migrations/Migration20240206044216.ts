...
export class Migration20240206044216 extends Migration {
  ...
  async up(): Promise<void> {
    this.addSql('create table "invite" ("id" text not null, "user_identifier" text not null, "accepted" boolean not null, "token" text not null, "expires_at" timestamptz not null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "invite_pkey" primary key ("id"));');
    this.addSql('create index "IDX_invite_deleted_at" on "invite" ("deleted_at");');
    this.addSql('create unique index "invite_user_identifier_index" on "invite" ("user_identifier") where deleted_at is null;;');
