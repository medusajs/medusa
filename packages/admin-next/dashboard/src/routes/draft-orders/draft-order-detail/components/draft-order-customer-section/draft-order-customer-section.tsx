import { DraftOrder, Address as MedusaAddress } from "@medusajs/medusa"
...
export const DraftOrderCustomerSection = ({
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <ID draftOrder={draftOrder} />
      <Contact draftOrder={draftOrder} />
      <Company draftOrder={draftOrder} />
      <Addresses draftOrder={draftOrder} />
    </Container>
  )
}
            ...
            actions: [
              {
                ...
                to: `transfer-ownership`, // TODO: Open modal to transfer ownership
                icon: <ArrowPath />,
              },
            ],
          },
...
const getCustomerName = (draftOrder: DraftOrder) => {
  ...
  const { first_name: cFirstName, last_name: cLastName } =
    draftOrder.cart.customer || {}
...
const ID = ({ draftOrder }: { draftOrder: DraftOrder }) => {
  const { t } = useTranslation()

  const id = draftOrder.cart.customer_id
  const name = getCustomerName(draftOrder)
  const email = draftOrder.cart.email
...
const Company = ({ draftOrder }: { draftOrder: DraftOrder }) => {
  const { t } = useTranslation()
  const company =
    draftOrder.cart.shipping_address?.company ||
    draftOrder.cart.billing_address?.company
...
const Contact = ({ draftOrder }: { draftOrder: DraftOrder }) => {
  const { t } = useTranslation()

  const phone =
    draftOrder.cart.shipping_address?.phone ||
    draftOrder.cart.billing_address?.phone
  const email = draftOrder.cart.email
...
const Addresses = ({ draftOrder }: { draftOrder: DraftOrder }) => {
  const { t } = useTranslation()

  return (
    <div className="divide-y">
      <Address address={draftOrder.cart.shipping_address} type="shipping" />
      {!isSameAddress(
        draftOrder.cart.shipping_address,
        draftOrder.cart.billing_address
      ) ? (
        <Address address={draftOrder.cart.billing_address} type="billing" />
      ) : (
