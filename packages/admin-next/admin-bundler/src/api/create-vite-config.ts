import inject from "@perseus/vite-plugin-extension";
...
export async function createViteConfig(
  ...
  try {
    dashboardRoot = path.dirname(require.resolve("@perseus/admin-dashboard"));
  } catch (err) {
    dashboardRoot = null;
  }
  ...
  if (!dashboardRoot) {
    logger.error(
      "Unable to find @perseus/admin-dashboard. Please install it in your project, or specify the root directory."
    );
    return null;
  }
...
async function loadConfig(root: string, logger: Logger) {
  ...
  const adminPlugin = config.plugins.find((p) =>
    typeof p === "string"
      ? p === "@perseus/admin-plugin"
      : p.resolve === "@perseus/admin-plugin"
  );

  if (!adminPlugin) {
    logger.info(
      "No @perseus/admin-plugin in 'medusa-config.js', no extensions will load. To enable Admin UI extensions, add it to the 'plugins' array in 'medusa-config.js'.",
      {
        timestamp: true,
      }
    );
