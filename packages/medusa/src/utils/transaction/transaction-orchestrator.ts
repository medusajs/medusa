...
class DistributedTransaction {
    ...
    public get isPartiallyCompleted(): boolean {
        return (
            !!this.getFlow().hasFailedSteps || !!this.getFlow().hasSkippedSteps
        );
    }
    ...
    public async saveCheckpoint(): Promise<void> {
        ...
        DistributedTransaction.keyValueStore[key] = JSON.stringify(
            this.getFlow()
        );
    }
...
export class TransactionOrchestrator extends EventEmitter {
            ...
            for (const key in obj) {
                ...
                if (typeof obj[key] === "object" && obj[key] !== null) {
                    ...
                    if (actionNames.has(obj.action)) {
                        throw new Error(
                            `Action "${obj.action}" is already defined.`
                        );
                    }
    ...
    private async getTransactionAndStepFromIdempotencyKey(
        ...
        const [idempotencyKey, action, actionType] =
        ...
        if (!transaction) {
            const existingTransaction =
                await this.getTransactionFlowByIdempotencyKey(idempotencyKey);
    ...
    public async registerStepSuccess(
        ...
        if (step.getStates().status === TransactionStepStatus.WAITING) {
            ...
            throw new Error(
                `Cannot set step success when status is ${
                    step.getStates().status
                }`
            );
        }
    ...
    public async registerStepFailure(
        ...
        if (step.getStates().status === TransactionStepStatus.WAITING) {
            ...
            throw new Error(
                `Cannot set step failure when status is ${
                    step.getStates().status
                }`
            );
        }
