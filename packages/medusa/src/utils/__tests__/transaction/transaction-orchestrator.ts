...
describe("Transaction Orchestrator", () => {
    ...
    it("Should hold the status COMPENSATING while the transaction hasn't finished compensating", async () => {
        ...
        expect(resumedTransaction.getState()).toBe(
...
describe("Transaction Orchestrator - Error handling", () => {
    it("Should store error objects for all the failed steps", async () => {
        async function handler(
            actionId: string,
            functionHandlerType: TransactionHandlerType,
            payload: TransactionPayload
            ...
            const command = {
                ...
                secondMethod: {
                    [TransactionHandlerType.INVOKE]: () => {
                        throw new Error("Method 2 error");
                    },
                    [TransactionHandlerType.COMPENSATE]: () => {},
                },
            };
        ...
        expect(transaction.errors[0]).toEqual({
            action: "secondMethod",
            error: expect.objectContaining({
                message: "Method 2 error",
            }),
            handlerType: "invoke",
        });
    });

    it("Should store error objects for all the failed steps - multiple errors", async () => {
        async function handler(
            actionId: string,
            functionHandlerType: TransactionHandlerType,
            payload: TransactionPayload
            ...
            const command = {
                firstMethod: {
                    ...
                    [TransactionHandlerType.COMPENSATE]: () => {
                        throw new Error("Error compensating Method 1");
                    },
                },
                secondMethod: {
                    [TransactionHandlerType.INVOKE]: () => {
                    ...
                    [TransactionHandlerType.COMPENSATE]: () => {
                        throw new Error("Error compensating Method 2");
                    },
                },
                thirdMethod: {
                    [TransactionHandlerType.INVOKE]: () => {
                        throw new Error("Method 3 error");
                    },
                    [TransactionHandlerType.COMPENSATE]: () => {
                        throw new Error("Error compensating Method 3");
                    },
                },
            };
            return command[actionId][functionHandlerType](payload);
        ...
        expect(transaction.getState()).toBe(TransactionState.FAILED);
        expect(transaction.errors).toHaveLength(4);

        expect(transaction.errors).toEqual([
            {
                action: "thirdMethod",
                error: expect.objectContaining({
                    message: "Method 3 error",
                }),
                handlerType: "invoke",
            },
            {
                action: "thirdMethod",
                error: expect.objectContaining({
                    message: "Error compensating Method 3",
                }),
            ...
            },
            {
                action: "secondMethod",
                error: expect.objectContaining({
                    message: "Error compensating Method 2",
                }),
                handlerType: "compensate",
            },
            {
                action: "firstMethod",
                error: expect.objectContaining({
                    message: "Error compensating Method 1",
                }),
                handlerType: "compensate",
            },
        ]);
