...
export class orderEditing1662982509900 implements MigrationInterface {
  name = "orderEditing1662982509900"
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "order_edit" ("id" character varying NOT NULL, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, "order_id" character varying, "internal_note" character varying, "created_by" character varying, "requested_by" character varying, "requested_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "confirmed_by" character varying, "confirmed_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "declined_by" character varying, "declined_reason" character varying, "declined_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "canceled_by" character varying, "canceled_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), CONSTRAINT "PK_58ab6acf2e84b4e827f5f846f7a" PRIMARY KEY ("id"))`
    )
    ...
    await queryRunner.query(
      `ALTER TABLE "order_edit" ADD CONSTRAINT "FK_d0578a4a0a2baf6f853f573b44b" FOREIGN KEY ("requested_by") REFERENCES "user"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "order_edit" DROP CONSTRAINT "FK_d0578a4a0a2baf6f853f573b44b"`
    )
    await queryRunner.query(
