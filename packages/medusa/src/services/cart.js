...
class CartService extends BaseService {
  ...
  async create(data) {
    return this.atomicPhase_(async manager => {
      ...
      // when creating carts with a specific user, we should add
      // the address to that customer if not already existing.
      if (data.shipping_address) {
        data.shipping_address.customer_id = data.customer_id
      }
  ...
  async addShippingMethod(cartId, optionId, data) {
    return this.atomicPhase_(async manager => {
      ...
      const cartCustomShippingOptions = await this.customShippingOptionService_.list(
        { cart_id: cart.id }
      )
