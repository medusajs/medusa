...
describe("OrderEditService", () => {
  ...
  describe("complete", () => {
    ...
    it("complete an order edit", async () => {
      const id = IdMap.getId("requested-order-edit")
      const userId = IdMap.getId("user-id")
    ...
    */
    ...
    it("returns early in case of an already confirmed order edit", async () => {
      ...
      const result = await orderEditService.complete(id, {
        loggedInUserId: userId,
      })
    ...
    it("fails to complete an order edit that is not requested", async () => {
      const id = IdMap.getId(`order-edit-to-update`)
      const userId = IdMap.getId("user-id")
