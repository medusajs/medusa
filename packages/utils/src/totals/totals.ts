...
type CalculateLineItemDTO = {
  unit_price: number
  raw_unit_price: Record<string, any>
  quantity: number
  tax_lines: any[]
  adjustments: { raw_amount: Record<string, any>; amount: number }[]
...
export default class NewTotalsService extends TransactionBaseService {
  ...
  protected async getLineItemTotals_(
    ...
    // TODO: account for adjustments
    const discountBigNum = BigNumber("0")
    const discount_total = discountBigNum.toNumber()

    const unitPriceBigNum = BigNumber(item.raw_unit_price.value)

    let subtotalBigNum = unitPriceBigNum.multipliedBy(item.quantity)
    let totalBigNum = subtotalBigNum.minus(discount_total)

    let taxTotal = 0

    const totals: LineItemTotals = {
      unit_price: unitPriceBigNum.toNumber(),
      quantity: item.quantity,
      subtotal: subtotalBigNum.toNumber(),
      discount_total,
      total: totalBigNum.toNumber(),
      original_total: subtotalBigNum.toNumber(),
    ...
    const isTaxInclusive = includeTax ?? !!item.is_tax_inclusive // TODO: account for tax inclusivity
