...
export default async ({ repoUrl = "", seed, boilerplate, skipDb, dbUrl, browser, migrations, directoryPath, withNextjsStarter = false, verbose = false, v2 = false, }) => {
    ...
    const adminEmail = !skipDb && migrations ? await askForAdminEmail(seed, boilerplate) : "";
    ...
    }).then(async () => open(inviteToken
        ? `http://localhost:7001/invite?token=${inviteToken}&first_run=true`
        : "http://localhost:7001"));
};
async function askForProjectName(directoryPath) {
...
async function askForAdminEmail(seed, boilerplate) {
...
function showSuccessMessage(projectName, inviteToken, nextjsDirectory) {
    logMessage({
        message: boxen(chalk.green(
...
function getProjectPath(projectName, directoryPath) {
