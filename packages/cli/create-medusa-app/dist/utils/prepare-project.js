...
export default async ({ directory, dbConnectionString, admin, seed, boilerplate, spinner, processManager, abortController, skipDb, migrations, onboardingType = "default", nextjsDirectory = "", client, verbose = false, v2 = false, }) => {
    ...
    if (!skipDb && migrations) {
        ...
        await processManager.runProcess({
            process: async () => {
                const proc = await execute(["npx @medusajs/medusa-cli@latest migrations run", npxOptions], { verbose, needOutput: true });
                if (client) {
                    // check the migrations table is in the database
                    // to ensure that migrations ran
                    let errorOccurred = false;
                    try {
                        const migrations = await client.query(`SELECT * FROM "${v2 ? "mikro_orm_migrations" : "migrations"}"`);
                    ...
                    if (errorOccurred && (proc.stderr || proc.stdout)) {
                        throw new Error(`An error occurred while running migrations: ${proc.stderr || proc.stdout}`);
                    }
                }
            },
        });
    ...
    if (!skipDb && migrations) {
        if (seed || !boilerplate) {
            ...
            if (!fs.existsSync(path.join(directory, "data", "seed.json"))) {
                spinner
                    ?.warn(chalk.yellow("Seed file was not found in the project. Skipping seeding..."))
                    .start();
                return inviteToken;
            }
            await processManager.runProcess({
                process: async () => {
                    await execute([
                        `npx @medusajs/medusa-cli@latest seed --seed-file=${path.join("data", "seed.json")}`,
                        npxOptions,
                    ], { verbose });
                },
            });
        ...
        else if (fs.existsSync(path.join(directory, "data", "seed-onboarding.json"))) {
            ...
            await processManager.runProcess({
                process: async () => {
                    await execute([
                        `npx @medusajs/medusa-cli@latest seed --seed-file=${path.join("data", "seed-onboarding.json")}`,
                        npxOptions,
                    ], { verbose });
                },
            });
