...
export class Migration20240307115701 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table if not exists "stock_location" ("id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, "name" text not null, "inventory_item_id" text null, "address_id" text null, "metadata" jsonb null, constraint "stock_location_pkey" primary key ("id"));');
    ...
    this.addSql('alter table if exists "stock_location" add constraint "stock_location_inventory_item_id_foreign" foreign key ("inventory_item_id") references "stock_location_address" ("id") on update cascade on delete cascade;');
  ...
  async down(): Promise<void> {
    this.addSql('alter table if exists "stock_location" drop constraint if exists "stock_location_inventory_item_id_foreign";');
