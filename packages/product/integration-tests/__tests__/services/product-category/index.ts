...
describe("ProductCategory Service", () => {
  ...
  describe("list", () => {
    beforeEach(async () => {
      ...
      productCategories = await createProductCategories(testManager, productCategoriesData)
    })
    ...
    it("scopes product categories by parent_category_id", async () => {
      let productCategoryResults = await service.list({ parent_category_id: null }, {
        select: ['id']
      })
      ...
      productCategoryResults = await service.list({ parent_category_id: "category-0" })
      ...
      productCategoryResults = await service.list({ parent_category_id: ["category-1-b", "category-0"] })
    ...
    it("includes the entire list of descendants when include_descendants_tree is true", async () => {
      ...
      const serializedObject = JSON.parse(JSON.stringify(productCategoryResults))
