 ...
 *
 * The step performs a single update operation and fails if the payment session
 * no longer exists. Callers that need to guard against a session being deleted
 * concurrently (e.g. a parallel refresh or re-init) should serialise those
 * operations with a lock.
 *
 * @example
...
export const updatePaymentSessionStep = createStep(
  ...
  async (input: UpdatePaymentSessionStepInput, { container }) => {
    ...
    // Retrieve the existing session to capture its pre-update amount (for
    // compensation) and its `data` (so the provider targets the same underlying
    // payment). A NOT_FOUND here propagates and fails the step.
    ...
    // POST-update provider data. Reverting must pass the post-update data (new
    // amount) with the old amount so a provider no-op guard (data amount ===
    // target) sees a delta and actually resets the provider payment.
    return new StepResponse<PaymentSessionDTO, RevertData>(updated, {
      id: session.id,
  ...
  async (revert: RevertData | undefined, { container }) => {
    ...
    // is reset to its previous amount.
    try {
