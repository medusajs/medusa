...
/**
 * This workflow updates one or more product variants. It's used by the [Update Product Variant Admin API Route](https://docs.medusajs.com/api/admin/products/update-variant).
 *
    ...
    const variantsToCreateInventory = transform(
      ...
      (data) => {
        ...
        if ("product_variants" in data.input) {
          ...
          data.input.update &&
          data.input.update?.manage_inventory === true
        ) {
          variantIds.push(...data.updatedVariants.map((v) => v.id))
        }
