import { ModuleResolution } from "@zjedene-medusa/types"
...
describe("modules loader", () => {
  ...
  it("should throw an error if the default package isn't found and the module is required", async () => {
    ...
    const moduleResolutions: Record<string, ModuleResolution> = {
      testService: {
        resolutionPath: "@zjedene-medusa/testService",
        definition: {
          key: "testService",
          defaultPackage: "@zjedene-medusa/testService",
          label: "TestService",
          isRequired: true,
          defaultModuleDeclaration: {
            scope: MODULE_SCOPE.INTERNAL,
    ...
    try {
      ...
      expect(err.message).toEqual(
        `Make sure you have installed the default package: @zjedene-medusa/testService`
      )
    }
  })
