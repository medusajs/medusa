 ...
 * @ignore
 */
export type Pluralize<Singular extends string> = Singular extends `${infer R}y` ? `${R}ies` : Singular extends `${infer R}es` ? `${Singular}` : Singular extends `${infer R}ss` | `${infer R}sh` | `${infer R}ch` | `${infer R}x` | `${infer R}z` | `${infer R}o` ? `${Singular}es` : `${Singular}s`;
